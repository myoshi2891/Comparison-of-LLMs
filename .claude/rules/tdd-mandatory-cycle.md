...
4. **`plans/NNN-*.md` に紐づく作業は、該当プランの Status 更新までを 1 サイクルに含める。** 実装を終えても `plans/README.md` の Status 行が TODO / IN PROGRESS のままなら、そのサイクルは未完了である。
</ai_agent_directive>
...
作業種別に応じて **(a)** か **(b)** のいずれかを適用する。どちらにも当てはまらない場合（バグ修正・スタイル微調整・ユーティリティの独立変更など）は本ステップを省略してよい。
