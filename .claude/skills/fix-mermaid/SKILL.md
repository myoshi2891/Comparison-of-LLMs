...
| ノードラベル `["..."]` | **全角括弧 `（）`** 例: `名前（サブタイトル）` | コロン区切りに変換: `名前：サブタイトル` |
| ノードラベル `["..."]` | **全角スラッシュ `／`** 例: `役割A／役割B` | 中黒に置換: `役割A・役割B` |
| ノードラベル `["..."]` | 全角波ダッシュ `〜` 例: `20%〜50%` | `から` 等の日本語に置換: `20%から50%` |
| ノードラベル `["..."]` | 半角スラッシュ `path/to` | `-` またはスペースに置換 |
| 菱形ノード `{}` | クォートなし日本語 `{新しいファイル}` | `{"新しいファイル"}` とクォートする |

> **⚠️ 「再三修正が必要」なパターン（2026-09-17 実績）**
>
> 日本語の注釈を `（）`（全角括弧）でノードラベルに添える書き方は、
> HTML フォーマッタがインラインで保存すると Mermaid の括弧構文と衝突する。
> **移行前に必ず原本 HTML を grep して全角括弧・全角スラッシュを事前クリーニングすること。**
>
> ```bash
> # 全角括弧・全角スラッシュを含む mermaid ノードラベルを検出
> grep -n '[（）／〜]' <原本HTML> | grep -E 'class="mermaid|\["'
> ```

---

## Part 1-b: 移行前の原本 HTML クリーニング（2026-09-17 追記）

> **背景**: 複数回の移行作業で「全角括弧 `（）`」「全角スラッシュ `／`」を含む
> ノードラベルが繰り返し Syntax Error を引き起こした。移行前に必ず下記の手順を実施すること。

### 対象パターンと変換規則

| 検出パターン | 変換規則 | 例 |
|---|---|---|
| `"テキスト（注釈）"` | `"テキスト：注釈"` | `"バッチ（学習用）"` → `"バッチ：学習用"` |
| `"役割A／役割B"` | `"役割A・役割B"` | `"SRE／エンジニア"` → `"SRE・エンジニア"` |
| `"X%〜Y%"` | `"XパーセントからYパーセント"` または `"X%からY%"` | `"20%〜50%"` → `"20%から50%"` |
| `subgraph LABEL["（）含む"]` | `subgraph LABEL["コロン区切り"]` | `"オフライン（バッチ）"` → `"オフライン：バッチ"` |

### 検出コマンド

```bash
# 1. 全角括弧・全角スラッシュ・全角波ダッシュを含むノードを全件検出
grep -n '（\|）\|／\|〜' <原本HTML> | grep -v '^\s*//' | head -50

# 2. class="mermaid" ブロック内のみに絞る（awk を使用）
awk '/class="mermaid"/,/<\/div>/' <原本HTML> | grep -n '[（）／〜]'
```

### 修正手順

1. 上記コマンドで問題行を全件特定する（見落としゼロを保証）
2. `multi_replace_file_content` で **原本 HTML を先に修正**してからアーカイブへ移動する
   （`git mv` 後に修正すると diff が肥大化するため、移動前に修正する）
3. 修正後、再度 grep して検出数が 0 になることを確認する:

   ```bash
   # class="mermaid" ブロック内のみに絞ってカウントする（手順2と同じ awk 抽出）。
   # ファイル全体を対象にすると、Mermaid 外の通常の日本語本文にある全角括弧等まで
   # ヒットしてしまい、0 に収束しない。
   awk '/class="mermaid"/,/<\/div>/' <原本HTML> | grep -c '[（）／〜]'  # 0 になるまで修正を繰り返す
   ```

4. `page.tsx` の Mermaid テンプレートリテラルには **クリーニング済みの構文**を転写する

> **注**: この手順を省略して `page.tsx` の転写時に直接修正しようとすると、
> 原本との diff が混在して監査スクリプトの exit=0 が取れなくなる場合がある。
> 必ず原本 HTML → アーカイブ移動 → `page.tsx` 転写の順で作業する。
