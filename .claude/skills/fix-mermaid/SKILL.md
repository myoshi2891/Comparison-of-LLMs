...
> **⚠️ 全角括弧・全角/半角スラッシュ・全角波ダッシュは「実際にエラーを再現したときだけ」直す（2026-09-19 改訂）**
>
> 過去に日本語の注釈を `（）`（全角括弧）でノードラベルに添えた際、HTML フォーマッタがインラインで
> 保存した結果 Mermaid の括弧構文と衝突し Syntax Error になった実績がある。この文字種自体が既知の
> リスク要因であることは変わらないが、**含まれているというだけで機械的に全置換してはならない**。
> 原本の文言を投機的に書き換えると、ガイド移行の「100% Faithful」原則（CLAUDE.md）に反し、
> かつ実際には Mermaid 10.9.8 で問題なく描画されるケースまで不要に改変してしまう。
>
> **原則**: 原本の Mermaid ソースはそのまま `MermaidDiagram` の `chart` に渡す。ブラウザで実際に
> Syntax Error を再現した場合のみ、該当箇所を最小限修正し、何をどう直したか・どう再現したかを
> コミットメッセージか `docs/PROGRESS.md` に明記する。
>
> ```bash
> # 該当箇所を後から追いやすくするための洗い出し用（検出 = 即修正ではない）
