...
| ID | 内容 |
|---|---|
| Q-1 | `TocObserver.test.tsx` で **`TocObserver.tsx` が持つ全ての公開挙動**を単体検証する。`fireEvent.scroll` によるアクティブ見出しの遷移だけでなく、モバイルサイドバー/メニューの開閉（トグルボタン・scrim クリック・Escape キー）、リンククリック時の自動クローズ、resize によるブレークポイント跨ぎの挙動、`inert` の付与/解除など、ファイルが実装している分岐をすべてカバーする（§6-a も参照） |
...
### 6-a. `TocObserver.tsx` 等のクライアントコンポーネントにロジックを追加・変更する場合（必須・繰り返し発生した不具合）

> [!CAUTION]
> **背景（2026-09-18 発生）**: 既に移行済みの 3 ページ（`books/ai-engineering-guide`、
> `books/generative-ai-for-software-development`、`governance/itil-ai-governance-v5`）の
> `TocObserver.tsx` へ a11y フォーカス管理コード（`sidebar.inert = hidden` を設定する直前に
> フォーカスをトグルボタンへ退避させるガード）を追加した際、対応するテストが 1 件も
> 追加されなかった。当該 3 ファイルはブランチ全体では新規追加扱いのため、SonarQube の
> Quality Gate（`Coverage on New Code ≥ 80%`）がブランチ全体で Failed になった
> （実測 52.8%、3 ファイルとも 41〜68%）。同種の指摘が複数回入っている。

**したがって、`TocObserver.tsx`（または同種の `"use client"` インタラクティブコンポーネント）へ
新しい条件分岐・イベントハンドラ・DOM 操作を追加または変更した場合は、
実装コミットと同一の作業単位で、対応するテストケースを必ず `TocObserver.test.tsx` へ追加すること。**

- 対象ファイルに `TocObserver.test.tsx` が**存在しない**場合（今回のケース。移行時に
  作成されたはずのテストが何らかの理由で欠落している、または既存ページ移行が
  Q-1 未整備の時期に行われた）は、新規ロジックの分岐だけでなく、Q-1 が要求する
  **既存の全公開挙動**（モバイルサイドバー/メニューの開閉、scrim クリック、Escape キー、
  リンククリック時の自動クローズ、resize によるブレークポイント跨ぎ）を含めた
  テストスイートを新規作成する。差分の3行だけをテストしても、ファイル全体が
  新規追加扱いのブランチでは New Code Coverage を満たせない。
- 既に `TocObserver.test.tsx` が存在する場合は、追加した分岐を検証するテストケースを
  既存スイートへ追記する。
- `bun run test:coverage -- <path>` で対象ファイルの line coverage が新規/変更行を
  含めて実質的に検証されていることを確認してからコミットする。
- 省略してよいのは、ロジックを変えない純粋なリファクタリング（変数名変更・コメント追加等）のみ。
...
> `TocObserver.tsx` 等のクライアントコンポーネントへロジックを追加する際のテスト要件は
> 禁止事項ではなく §6-a の必須要件として定義する（肯定文の方が読み違えにくいため）。
