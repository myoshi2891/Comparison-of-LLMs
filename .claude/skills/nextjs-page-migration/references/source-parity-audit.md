...
| `listItems` / `codeBlocks` / `tableRows` の件数 | **参考値のみ** | 移行先は `<li>` を使わずカード / `div` で組むため件数は一致しない |

> 件数を判定に使わないのは意図的な設計である。`<li>` 件数でゲートすると
> 「原本 18 / ページ 0」のような正当なケースで常時 Red になり、ゲートが形骸化する。
