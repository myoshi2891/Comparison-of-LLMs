...
書籍が刊行された2024年以降、AIを使ったコーディングは「一部の先進的な開発者の実験」から「業界標準のワークフロー」へと急速に変化しました。Stack Overflowが2025年に実施した大規模な開発者調査（177カ国・約5万人が回答）では、AIコーディングツールを「利用している、または利用予定」と回答した割合は84%に達した一方、AIの出力を「信頼している」と答えた開発者はわずか33%にとどまり、前年の40%から大きく低下しています。デバッグに時間がかかる（45%）、出力が「惜しいけれど微妙に間違っている」（66%）といった不満が上位を占めており、**「使ってはいるが無条件には信じない」**という姿勢が定着しつつあります。
...
| Step 9 | 第9章 | GPT-ing on the go | GPT4Allで実行するLlama 2等のローカルLLMとの比較、オフライン活用の判断基準 |
...
書籍が「ソフトウェア工学における重要な側面」と位置づけるテストの章です。単体テスト・統合テスト・振る舞いテスト（Behavior Testing）の3種類をLLMに書かせる方法、コード品質・複雑度の評価、バグ発見、コードカバレッジの確認、さらにコードを言葉で説明することや、プログラミング言語間の翻訳までを扱います。
...
書籍では、PythonとFastAPIで書かれたISAM（Information Systems Asset Management、書籍内でのアプリケーションのセキュリティ文脈での呼称）を題材に、ChatGPTを脅威モデリング・脆弱性評価・セキュリティのベストプラクティス適用・保存時および通信時のデータ暗号化に活用する方法を解説しています。
...
「海外のAIカンファレンスに向かう飛行機の中で、機内Wi-Fiが遅くて高いとき、手元のノートPCでLLMをオフラインで動かせたら」という導入から始まる章です。GPT4AllでLlama 2などのモデルをローカル環境で動かし、ChatGPTの回答と比較しながら、いつオフラインモデルで十分か、いつクラウドのモデルが必要かを判断する材料を提供しています。

### ローカルLLM活用の判断フロー

```mermaid
flowchart TB
    Q1{"機密データを扱う／オフライン環境か？"} -->|はい| LOCAL["ローカルLLMを使う<br/>（要: 実行環境の機密性検証）"]
...
> **注意**: 「ローカルLLMを使う」はクラウドへの送信を避けられるだけであり、機密性そのものを自動的に保証するものではありません。実行環境が本当に閉じているかは別途、以下を確認する必要があります。
>
> - サーバーがループバック(localhost)のみで待ち受けているか、外部到達可能な口を開けていないか
> - アクセス制御（認証・認可）が設定されているか
> - モデルやツールが外部への通信（アップデート確認・テレメトリ等）を行っていないか
> - ログ・キャッシュ・応答履歴の保存先と保持期間
> - 呼び出すツール（関数呼び出し）やテレメトリ経由でデータが外部に漏れないか
> - モデル自体（配布元・ライセンス・改変有無）の信頼性

### 書籍刊行時点（2024年）と2026年時点の比較

| 観点 | 2024年（書籍刊行時） | 2026年（現在） |
|---|---|---|
| 代表的なツール | GPT4All（Llama 2等のモデルを実行） | Ollama、LM Studio |
| 実行環境 | CLIやデスクトップアプリが中心 | OpenAI互換APIを備えたローカルサーバーとして、IDEやコーディングエージェントから直接呼び出せる |
| 実用性の目安 | クラウドモデルとの性能差が大きく、限定的な用途向け | 8B〜35B前後のオープンウェイトモデルは、量子化方式・VRAMまたはユニファイドメモリ容量・コンテキスト長・推論バックエンド・モデル自体の特性に依存し、ハードウェアと設定が適合すれば実用的な速度と品質を達成できる場合がある |
| 主な用途 | オフライン時の代替手段 | 機密コードの解析、コスト削減、レイテンシ削減、プライバシー保護が必要な場面での積極的な選択肢 |

### 2026年アップデート

ローカルLLMは、単なる「オフライン時の代替」から一歩進み、**機密性の高いコードや社内データを外部に送りたくない場合の第一選択肢**としても定着しています。ただし「ローカルで動かす」こと自体はクラウドへの送信を避けるだけであり、機密性を自動的に保証しません。ループバックのみの待受けか、アクセス制御・アウトバウンド通信・ログ／キャッシュの保持期間・呼び出すツールやテレメトリの経路・モデル自体の信頼性を個別に検証したうえで採用することが前提です。Ollamaは OpenAI互換API と Anthropic Messages API の一部を提供しており、対応するコーディングツールであればエンドポイントの向き先をローカルサーバーへ変更するだけで接続できる場合があります。ただし対応範囲は API 全体を網羅しているわけではなく、認証方式やツール（関数）呼び出しのサポート状況はツール・モデルの組み合わせごとに異なるため、実際に利用する前に対象のコーディングツールとの互換性を個別に確認・設定する必要があります。
...
2025年2月、著名なAI研究者Andrej Karpathy（元Tesla AI部門責任者、OpenAI創設メンバーの一人）が「vibe coding（バイブコーディング）」という言葉をSNSに投稿し、一気に広まりました。これは、コードの中身をほとんど確認せず、AIの提案を次々に受け入れながら「とにかく動くものを作る」スタイルを指します。
...
Stack Overflowの2025年開発者調査[14]によると、AIツールを「利用している、または利用予定」と回答した割合は84%まで拡大した一方、出力を信頼すると答えた開発者は33%にとどまりました。ソフトウェアエンジニアリングブログの著者Josh Collinsworth[21]も、LLMの支援効果は「定型的な作業」や「専門外の領域」にとって特に大きく、その範囲を外れるほど効果が薄れるという傾向を指摘しています。
...
書籍第9章で紹介されているGPT4Allや、そこで動かされていたLlama 2のようなモデルは、2026年時点ではOllamaやLM Studioといったツールを通じて、より手軽かつ実用的に使えるようになっています。特にコーディング用途に最適化されたオープンウェイトモデルが充実しており、量子化方式・VRAMまたはユニファイドメモリ容量・コンテキスト長・推論バックエンド・モデル自体の特性とハードウェア・設定の組み合わせ次第では、十分な速度と品質でローカル実行できる場面が広がっています。
