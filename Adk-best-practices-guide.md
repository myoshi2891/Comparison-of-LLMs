...
> ⚠️ **A2AプロトコルおよびSDKの各言語サポート状況（2026年7月時点）**:
> ADKにおけるA2Aプロトコル（`A2AServer` や `RemoteA2aAgent` を含む実装）は発展途上にあり、使用する言語のSDKごとに以下のサポートレベルが設定されています。実装例を参照する際はサポートステータスにご注意ください。
> - **Java / TypeScript (Stable/Beta)**: 比較的一貫したAPIが提供されており、初期評価向けの検証が可能です。
> - **Go (Alpha)**: 基本プロトコルのみのサポートで、実用的なエージェント機能は未整備です。
> - **Python (Experimental / 実験的)**: Python SDKで提供されるA2Aクイックスタート (`A2A quickstart`) および `RemoteA2aAgent` 等の実装例はすべて **Experimental** 扱いです。API仕様の破壊的変更が頻繁に行われる可能性があり、現段階での本番導入は非推奨です。
