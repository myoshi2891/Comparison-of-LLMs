...
|---|---|---|
| 出発点 | 自社データを起点に、モデルアーキテクチャ設計から学習パイプライン構築・評価・提供までを担う（ゼロからの学習はその一例で、既存モデルの適応や継続学習も含む） | 既存の基盤モデルから始め、プロンプトやRAGで製品要件に適応させる |
| 主な作業 | モデルアーキテクチャ設計、学習パイプライン構築、評価、モデル提供（サービング） | プロンプト設計、評価設計、コンテキスト構築、インターフェース開発 |
...
LLM-as-a-judgeは人手評価に比べて大幅に低コスト・高速に評価をスケールできる一方、既知のバイアスがあります。これらを体系的に測定した代表的な一次研究がZheng et al., "Judging LLM-as-a-Judge with MT-Bench and Chatbot Arena"（NeurIPS 2023 Datasets and Benchmarks, arXiv:2306.05685）で、同論文は ①提示順序を入れ替えると判定が変わる**位置バイアス（position bias）**、②長い出力を過大評価する**冗長性バイアス（verbosity bias）**の2つを、実験によって定量的に報告しています。さらに ③判定側が自分自身の生成した応答を優遇する**自己贔屓バイアス（self-enhancement bias）**にも言及していますが、こちらは一部の判定モデルで自己生成応答を優遇する例が観察されたという**限られたデータに基づく予備的な結果**であり、バイアスの有無を確定できるものではないと同論文自身が断っています。
...
MCPが登場する以前は、AIをそれぞれの外部サービスに接続するために毎回個別の統合コードを書く必要がありました。MCPは、クライアント（AIアプリ側）とサーバー（ツール側）の間のやり取り——ツールの一覧取得と呼び出し、リソースやプロンプトの提供など——を単一のプロトコルとして定義しています。仕様は継続的に改定されており、本ガイドは2026年7月28日改定版（`2026-07-28`、GA）を基準にしています。同改定でMCPはステートフルな双方向プロトコルから、リクエスト/レスポンス型のステートレスなプロトコルへと大きく転換しました。旧仕様でサーバーからクライアントへの要求として使われていたサンプリング（`sampling/createMessage`）とRootsは非推奨となりました。一方、Elicitation（サーバーがユーザーへ追加入力を求める仕組み）は現行のクライアント機能として維持されています。ただしサーバー起点のリクエストという形は廃止され、追加入力が必要な場合はサーバーが`InputRequiredResult`（`resultType: "input_required"`）を返し、クライアントが入力を添えて元のリクエストを再送する**Multi Round-Trip Requests（MRTR）**パターンで処理されます。ツール側がMCPサーバーとして機能を公開しておけば、MCPに対応したクライアントは、認証・権限付与・トランスポート設定といった接続条件を満たしたうえで、その機能を共通の方法で利用できます。サービスごとに個別の統合コードを書き直す必要がなくなる点が評価されています。

2026年時点でMCPは急速に業界標準化が進んでいます。普及規模については、2025年12月9日のMCP公式（Anthropic）による発表が、月間SDKダウンロード数9,700万超・**アクティブなMCPサーバー**1万超という数字を示しています（いずれも同発表時点の値であり、測定方法は公表元の定義に依存します）。同じく2025年12月9日にAnthropicがMCPをLinux Foundation傘下の新団体「Agentic AI Foundation（AAIF）」に寄贈し、Anthropic・Block・OpenAIが共同創設メンバーとして、AWS・Google・Microsoft・Cloudflareなどが支援企業として参加したことで、単一企業の技術から業界横断のオープンスタンダードへと移行しました。Cursor・VS Codeをはじめ、MCPに対応するクライアントや開発ツールも増え続けています。
...
- **量子化（Quantization）**：モデルの重みを低精度で表現する手法で、メモリ使用量や推論速度の改善につながることがありますが、無条件に効果があるわけではありません。バックエンド・カーネル実装・モデル構造・ハードウェア・バッチ設定によっては効果がない、あるいはかえって速度が低下する場合があり、精度が低下するリスクも伴います。モデルの層ごとに異なる量子化戦略を組み合わせ、ベンチマーク品質を保ちながらスループットを改善できたという報告例もありますが、採用前に必ず対象環境で品質と速度の両方をベンチマークしてください。
...
- **保持期間と削除手段**: トレース・監査ログごとに保持期間を定め、期限到来時に自動削除されるようにする。あわせて、削除請求（GDPRの消去権など）に応じて特定ユーザーのレコードを検索・削除できる手段をあらかじめ用意しておく。削除対象はログだけではない。原文書の削除・アクセス権限の変更・保持期限の到来時には、そこから派生したデータ——検索チャンク（インデックスと埋め込み）、保存済みプロンプト、生成された回答、評価用トレース——までを対象に、一括で無効化または削除する。この伝播は用途別（デバッグ用途・監査用途など）に定めた保持期間ごとに最大伝播遅延（許容する上限時間）を設けて運用し、伝播が完了するまでは当該派生データへの参照（読み取り）を拒否する。訴訟ホールド（リーガルホールド）等で削除を保留する場合は、保留の理由・対象範囲・解除条件を記録し、期限のない例外として放置しない
...
- MCP公式仕様「2026-07-28」変更点まとめ（ステートレス化・MRTR導入・Roots/Sampling非推奨などの一次出典）— https://modelcontextprotocol.io/specification/2026-07-28/changelog
- MCP公式ブログ「The 2026-07-28 Specification」（2026年7月28日。GA公開の一次出典）— https://blog.modelcontextprotocol.io/posts/2026-07-28/
...
> ⚠️ AI分野の情報は非常に速く更新されます。特にツールの比較表やロードマップの期間感は執筆時点（2026年9月9日）のスナップショットであり、MCPのエコシステム統計は各出典の公表・測定時点（2025年12月9日）の値です。実践に移す前に、必ず一次情報（各社公式ドキュメントやブログ）を直接確認してください。
