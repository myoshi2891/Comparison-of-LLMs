...
なお、Gemini 2.5やGPT-4.1のように100万トークン級の長いコンテキストウィンドウを持つモデルが普及したことで、「知識ベース全体をそのままコンテキストに入れる（フルコンテキスト方式）」という選択肢が現実的になりました。フルコンテキスト方式とRAGのどちらを採るかは、**知識ベースの総トークン数・コスト・プロンプトキャッシュの利用可否・モデルのコンテキスト長・検索品質**を見て決めるべきで、どちらかが常に既定というわけではありません。目安として、知識ベースが20万トークン程度に収まり更新頻度も低いのであれば、検索パイプラインを持たないフルコンテキスト方式のほうが構成が単純で、かつ検索漏れが原理的に起きない分だけ精度で上回ることもあります。一方、知識ベースがそれを大きく超える規模になる、リクエストごとに長大なコンテキストを送るコストが見合わない、長いコンテキストでは中間に埋もれた情報の再現率が落ちる、といった条件に当てはまる場合はRAGが有利になります。
...
Anthropicは「AI Engineering Architecture」に関する解説の中で、コンテキストを充実させる、ガードレールを設ける、モデルルーターとゲートウェイを追加する、キャッシュでレイテンシを削減する、エージェントパターンを追加する、という段階的な拡張ステップを紹介しており、これは前述の「まずシンプルに始め、必要な場合にのみ複雑さを足す」という思想の延長線上にあります。
