...
2026年時点でMCPは急速に業界標準化が進んでいます。普及規模については、2025年12月9日のMCP公式（Anthropic）による発表が、月間SDKダウンロード数9,700万超・**アクティブなMCPサーバー**1万超という数字を示しています（いずれも同発表時点の値であり、測定方法は公表元の定義に依存します）。同じく2025年12月9日にAnthropicがMCPをLinux Foundation傘下の新団体「Agentic AI Foundation（AAIF）」に寄贈し、Block・OpenAIが共同創設メンバーとして、AWS・Google・Microsoft・Cloudflareなどが支援企業として参加したことで、単一企業の技術から業界横断のオープンスタンダードへと移行しました。Cursor・VS Codeをはじめ、MCPに対応するクライアントや開発ツールも増え続けています。
