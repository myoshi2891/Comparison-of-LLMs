...
- **リランキング（Re-ranking）:** 一次検索の上位候補をさらに精査するモデルを挟む手法。精度が常に向上するとは限らず、効果はリランカーへ渡す候補数（一次検索が返した上位何件をリランク対象にするか）、リランカーのモデル、対象データの性質、評価に使う指標に依存する。リランク対象の候補数が少なすぎれば並べ替える余地がなく、ドメインの離れたリランカーを使えば一次検索より検索品質が下がることもある（レイテンシとコストも増える）。導入は自社データと評価セットで一次検索のみの構成と比較し、実測で効果を確認したうえで判断すること
...
AIエージェントは必ずしもLLMを前提とするわけではなく、ルールベースの自動化や従来型MLを用いた構成も存在しますが、2026年時点で主流となっているLLMベースの生成AIエージェントは、LLMを「頭脳」として以下の要素を組み合わせて構成されます。
...
LLMは「モデル単体」、AIエージェントは「モデルを含むシステム構成」であり、両者は同じ能力軸上の両端として比較すると理解しやすくなります。対応モダリティ（テキスト・画像など、モデルにより異なる）は入出力の形式の話であり、以下の能力軸（ツール利用・状態保持・複数ステップ実行・自律性・外部副作用）とは別の観点である点に注意してください。

| 能力軸 | LLM単体 | AIエージェント（能力を有効化した構成） |
|---|---|---|
| ツール／API利用 | ツール呼び出しの要求を出力に含めることはできるが、実行や結果の反映は行わない | ツール・API・他システムを実際に呼び出して実行し、結果を次の判断に反映する |
...
生成AI・LLMアプリケーション特有のセキュリティリスクを体系化した代表的な業界ガイドが **OWASP Top 10 for LLM Applications** です（規格や認証基準ではなく、コミュニティが整備する啓発・チェックリスト文書です）。現行版は **2026年版**（OWASP GenAI Security Project、2026年8月公開）で、実インシデントのデータを順位付けに組み込んだ点が大きな変更です。
...
- 第三者AIベンダーへ個人データを提供する場合、個人データの処理有無・ベンダーの役割（processor、service provider、contractor、独立したcontrollerのいずれか）・適用法を確認したうえで、必要なデータ処理契約（DPA: Data Processing Agreement）または契約条項を締結し、データの学習利用可否・保存期間・データ所在地（レジデンシー）を明確化すること（ドメイン4.6のベンダー選定にも直結）
...
|---|---|---|---|
| Stable（安定） | 実績のある枯れた技術・パターンを使う。予測可能性が高い | 標準的な運用ルールで足りるのは低リスク用途に限られる。影響度・機微データの扱い・規制要件・リスク許容度に応じて、追加の統制（アクセス制御の強化、監査ログ、人間によるレビュー等）を上乗せする | 既存プロセスへのチャットボット導入、定型文書の要約 |
...
| 評価軸 | 確認すべきポイント |
|---|---|
| データセキュリティ・プライバシー | 入力データの学習利用有無、データ保存場所（レジデンシー）、第三者保証の取得状況（例: SOC 2）。なお **SOC 2 は「認証」ではなく、独立した公認会計士事務所が発行する保証報告書（attestation report）** であり、報告書そのものを受領して内容を読む必要がある。確認すべきは、指定日時点のシステム記述および統制のデザイン（設計）のみを対象とする **Type 1** か、一定期間にわたる統制の運用有効性まで対象とする **Type 2** か、そしてその **監査対象期間（examination period）が現在も有効といえる範囲か** の3点 |
