...
- **リランキング（Re-ranking）:** 一次検索の上位候補をさらに精査するモデルを挟む手法。精度が常に向上するとは限らず、効果は一次検索に渡す候補数、リランカーのモデル、対象データの性質、評価に使う指標に依存する。候補数が少なすぎれば並べ替える余地がなく、ドメインの離れたリランカーを使えば一次検索より検索品質が下がることもある（レイテンシとコストも増える）。導入は自社データと評価セットで一次検索のみの構成と比較し、実測で効果を確認したうえで判断すること
