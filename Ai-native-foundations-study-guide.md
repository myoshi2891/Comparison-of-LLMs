...
**Agentic AI（エージェント型AI）**とは、単に質問に一問一答で答える生成AIとは異なり、**目標（ゴール）を与えられると、自ら状況を認識し、計画を立て、複数のステップにわたって行動し、結果を観察しながら軌道修正していくAI**を指します。

従来の生成AI（例: チャットボットに1回質問して1回答えが返る）と、Agentic AIの本質的な違いは「**ループ構造を持つかどうか**」です。Agentic AIは以下のループを自律的に回します。
...
**RAGとは何か:** LLM単体は学習時点の知識しか持たず、社内文書のような非公開情報や最新情報には対応できません。RAGは、**質問に関連する情報を外部の知識ベース（文書・データベース）から検索し、その内容をプロンプトに含めた上でLLMに回答を生成させる**アーキテクチャです。これにより、ハルシネーションの低減、最新情報への対応、根拠（出典）の提示が可能になります。
...
- **出典の明示:** 生成した回答に、参照した文書名・箇所を必ず添えることで検証可能性を担保する
- **「わからない場合は分からないと答える」指示:** 検索結果に答えが含まれない場合、無理に回答を捏造させず「情報が見つからない」と回答させるようプロンプトで明示すること（ドメイン3.2でも詳述）
...
- **可観測性（Observability）:** エージェントが「なぜその行動を選んだか」のログを必ず記録し、事後検証できるようにする
...
両者とも「人を置き換える」のではなく「人の作業を補助する」ことを主眼にしている点で、1.6節の自律的なAIエージェントとは設計思想が異なります。
...
| 観点 | LLM | AIエージェント |
|---|---|---|
| 基本動作 | 1回の入力に対し1回の出力を返す（単発の予測） | 目標達成まで複数ステップの行動を自律的に繰り返す |
| 状態の保持 | 基本的に単一のやり取り内でのみ文脈を保持 | メモリを持ち、長期的なタスク状態を保持できる |
| 外部世界との関わり | テキストの入出力のみ | ツール・API・他システムを能動的に呼び出して行動する |
| 意思決定 | 次のトークンを予測するのみ | 計画を立て、行動を選択し、結果を評価する |
...
**ベストプラクティス**
- 「AIエージェント」という言葉が使われていても、実際にはループ構造を持たない単発のLLM呼び出しに過ぎない製品も多いため、導入前に実態を確認すること
...
責任あるAIの国際的な拠り所となっているのが、**OECD AI原則**（2019年策定、2024年5月改訂）です。47の国・地域が支持しており、EU AI ActやNIST AI RMFなど各国の枠組みの土台になっています。
...
**Dos（推奨事項）**
- LLMの出力は「信頼できない外部入力」として扱い、後続処理の前に必ず検証・サニタイズすること（特にLLM05対策）
...
- エージェントに与える機能・権限は最小限にし、重要な操作には承認ステップを設けること（LLM06対策）
- RAGで参照する外部ドキュメントの改ざん検知・アクセス制御を行うこと（LLM08対策）
- レート制限・利用上限を設定し、コストとリソースの暴走を防ぐこと（LLM10対策）
...
|---|---|---|
| 適用対象 | EU域内の拠点の活動に関連する処理、またはEU域外の組織によるEU域内の個人への商品・サービス提供・行動監視に関連する処理（Article 3） | カリフォルニア州住民の個人情報を扱う**営利事業者**のうち、次のいずれか1つ以上を満たす場合（CPPA公表基準）: ① 前暦年の年間総収益が2,662.5万ドル以上（2025年1月1日発効のインフレ調整後の額。以後も定期的に改定される）、② カリフォルニア州住民または世帯10万人分以上の個人情報を購入・販売・共有している、③ 年間収益の50%以上をカリフォルニア州住民の個人情報の販売・共有から得ている |
...
- **責任あるAI・セキュリティは「原則名」と「中身」をセットで覚える:** OECD AI原則、NIST AI RMFのGovern/Map/Measure/Manage、OWASP LLM Top10の代表的なリスク名を混同しないようにする
