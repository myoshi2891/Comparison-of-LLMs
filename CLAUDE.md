...
│   │   └── {claude,gemini,codex,copilot}/{skill,agent}/ および /gemini/agent-harness-engineering/、/claude/managed-agents/、/claude/code-slash-commands/   Phase B–C および追加移行済みルート（詳細は [`docs/archive/MIGRATION_PROGRESS.md`](docs/archive/MIGRATION_PROGRESS.md)）
│   ├── components/
...
- **Next.js 16 App Router + SSG**: `output: 'export'` で pure 静的エクスポート → Netlify CDN 配信。`@netlify/plugin-nextjs` 不要。Phase 1–14 でコスト計算機ホームが移行済、Phase A–F で残 18 ガイドページも全移行完了（計画書は `docs/archive/` に保存）。さらに `/gemini/agent-harness-engineering` や `/claude/managed-agents`、`/claude/code-slash-commands` ページを追加
