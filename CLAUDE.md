...
2. `cd web-next && bun run test` が完全に成功（177 files / 1639 tests pass。収集失敗もブロッキング失敗として原因を調査する）
