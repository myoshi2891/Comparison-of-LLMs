...
│   │   └── {claude,gemini,codex,copilot}/{skill,agent}/ および /gemini/agent-harness-engineering/、/claude/managed-agents/、/claude/code-slash-commands/、/code-review/coderabbit-guide/、/code-review/copilot-code-review/、/code-review/sonar-qube/   Phase B–C および追加移行済みルート（詳細は [`docs/archive/MIGRATION_PROGRESS.md`](docs/archive/MIGRATION_PROGRESS.md)）
│   ├── components/
...
- **Next.js 16 App Router + SSG**: `output: 'export'` で pure 静的エクスポート → Netlify CDN 配信。`@netlify/plugin-nextjs` 不要。Phase 1–14 でコスト計算機ホームが移行済、Phase A–F で残 18 ガイドページも全移行完了（計画書は `docs/archive/` に保存）。さらに `/gemini/agent-harness-engineering` や `/claude/managed-agents`、`/claude/code-slash-commands`、`/code-review/coderabbit-guide`、`/code-review/copilot-code-review`、`/code-review/sonar-qube` ページを追加
...
- **SonarQube Cloud（public 無料プラン）**: モノレポ単一プロジェクトで TS（vitest→lcov）と Python（pytest→coverage.xml）を解析。CI（`.github/workflows/sonarqube.yml`）+ ローカル（`make sonar` / Docker scanner CLI）で実行。**モノレポのパス整合**が要点で、scanner 実行前に lcov の `SF:` へ `web-next/` を、coverage.xml の `<source>` を `scraper/src/scraper` へ補正する（未補正だとカバレッジ 0%）。Python は `relative_files=true`。既存 `test.yaml` は不変
