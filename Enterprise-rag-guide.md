...
|---|---|---|---|
| **プロンプトに全部詰め込む（Long Context）** | 関連文書をまるごとプロンプトに入れる | 知識ベースが小さい（目安：50万トークン未満、約500ページ以下） | 知識ベースが大きくなるとコスト・レイテンシが跳ね上がる |
...
Anthropicの公式ガイダンスでも、知識ベースが小さい場合は無理にRAGを組まず、[プロンプトキャッシュ](https://docs.anthropic.com/en/docs/build-with-claude/prompt-caching)を使って全文をコンテキストに入れる方がシンプルで効果的だとされています。知識ベースがスケールしてきた段階で、初めてRAGが本当に必要になります。
...
flowchart TD
    Q[ユーザークエリ] --> QR[クエリ書き換え・拡張]
    QR --> VS[ベクトル検索<br/>意味的類似度]
    QR --> BM[BM25検索<br/>キーワード一致]
    VS --> RRF[結果統合<br/>Reciprocal Rank Fusion]
    BM --> RRF
    RRF --> ACL{アクセス権限で<br/>フィルタリング}
...
ここで新しく登場した「アクセス権限でフィルタリング」については、Step 9で詳しく解説します。
...
- **プロンプトインジェクション対策**：外部文書やユーザー入力に紛れ込んだ悪意ある指示を検出・無害化する。システムプロンプトと検索コンテキスト、ユーザー入力を明確に分離するアーキテクチャ設計も有効
...
6. **段階的にロールアウトする**：モデルルーティング、キャッシュ、レート制限などを整備し、SLO（サービス品質目標）を定めたうえで全社展開する

また、データソースは一度取り込んだら終わりではなく、継続的に更新されます。ロバストなデータパイプラインは、データを一度だけロードする場合よりも、ソースデータが常に変化し続ける本番環境でこそ重要になるとLlamaIndexのコミュニティでも指摘されています。増分更新（差分だけを再インデックスする仕組み）や、削除された文書をベクトルDBからも確実に取り除く仕組みは、地味ですが本番運用で最も事故が起きやすいポイントの一つです。
