...
- **URL取得を許可する場合のSSRF対策**：検索結果のURLを実際にフェッチする構成にするなら、取得先を宛先allowlistで限定したうえで、リダイレクト追跡後の最終宛先も再検証し、内部ネットワーク（プライベートIP、ループバック、リンクローカル、クラウドのメタデータエンドポイント等）への到達をDNS解決結果のレベルで拒否する。取得サイズ・タイムアウトの上限も設ける
...
| LLMのプロンプトキャッシュ（prompt cache） | 該当文書を含むキャッシュ接頭辞を再利用しない。接頭辞にバージョン識別子（例: コーパスのリビジョンID）を埋め込んで更新時に切り替える、TTLを短く設定する、または反映が完了するまで当該接頭辞のキャッシュ利用を停止する、のいずれかを必ず実施する。あわせて、キャッシュ接頭辞またはキャッシュキーには**テナントIDと、有効な認可コンテキスト**（ユーザーID、または当該ユーザーに適用される権限セットのバージョン識別子）を必ず含める |
