...
会話履歴とツール定義はターンを重ねるほど膨らむため、ヘッドルーム（例：上限の10〜20%）を確保しておかないと、パイロットでは通っていたリクエストが本番の長い会話で上限超過エラーになります。閾値はプロバイダと対象モデルのコンテキスト上限に依存するため、実際に採用するモデルの上限に対してこの合計値が収まることを必ず確認してください。知識ベースがスケールしてきた段階に加え、内容が頻繁に更新される、回答に出典の提示が求められる、検索時にユーザー単位の認可制御（アクセス権限に応じて参照できる文書を絞り込む）が必要になる、といった条件が一つでも当てはまる段階で、RAGの導入を検討すべきです。
...
- **テナント単位・ユーザー単位の行レベル認可**（RLS等）をデータベース側で強制し、アプリ側の条件付与だけに依存しない。PostgreSQLの場合、テーブルの所有者・superuser・`BYPASSRLS`権限を持つロールはRLSポリシーを素通りするため、実行接続には**所有者でも`BYPASSRLS`権限も持たない専用の実効ロール**（`NOBYPASSRLS`）を割り当てる。他のDBMSでも同様に、行レベルセキュリティを回避しうる管理者権限・所有者権限を実行ロールから排除する。RLSを適用できないテーブル・接続構成では、その対象への実行を拒否する
...
| **Context Precision（コンテキスト精度）** | 取得したコンテキスト（`retrieved_contexts`）のうち、関連するチャンクがどれだけ上位に順位付けされているか。関連ラベルが1であるランクの precision@k を合計し、関連チャンクの総数で割って算出される（同じ関連チャンク数でも、上位に来るほどスコアが高くなる） | 検索 |
...
5. **キャッシュを認可コンテキストごとに分離する（パイロット開始の前提条件）**：レスポンスキャッシュと検索キャッシュのキーには、テナントIDと**有効な認可コンテキスト（ユーザーIDまたは権限バージョン）**に加えて、**出力結果に影響しうる入力すべて**——クエリ本文、会話履歴、システムプロンプト、使用モデル、ロケール、フィーチャーフラグ、検索設定（Top-K・フィルタ条件など）——を含め、これらのいずれかが異なるリクエスト間でキャッシュが誤って再利用されないようにする。あわせて、**ソースの更新・削除時**と**権限変更時**には該当するキャッシュを無効化する。パーソナライズされた応答（利用者固有の文脈を含む回答）はそもそもキャッシュせず`no-store`とすることを既定とし、キャッシュキーが上記の入力を漏れなく含んでいること、およびパーティショニングの不変条件（権限境界を越えて共有されないこと）をドキュメント化したうえで検証する。この分離と無効化は**実ユーザーを入れる前に完了させる**こと——パイロットであっても、権限の異なる利用者間でキャッシュが共有されれば実データの漏えいになるため、「小規模だから後回し」は成立しない
...
flowchart TB
    ...
    R4 -->|イベントID・メトリクス・参照IDなどのマスキング済みメタデータのみを送出（未保護の本文・生成内容は含めない）| OPS
    SE4 -.->|統制の証跡| OPS
    OPS -.->|改善のフィードバック| ING
```
