...
```bash
(cd web-next && bun run test)        # 176 files / 1621 tests pass（2026-09-17 実測。全 Green ✅）
(cd web-next && bun run typecheck)   # OK（2026-09-17 実測）
(cd web-next && bun run build)       # ✅ Compiled successfully / 90 静的ページ生成（2026-09-17 実測）
(cd web-next && bun run lint)        # OK（489 files / 0 diagnostics。2026-09-17 実測）
