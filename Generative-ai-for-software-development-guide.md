...
2025年初頭、AI研究者のAndrej Karpathy氏が提唱した「vibe coding(バイブコーディング)」という言葉は、自然言語での指示だけでアプリケーションを作り上げる開発スタイルを指す言葉として一気に広まりました。2026年時点では、世界で本番稼働しているコードのうち4割前後がAI支援によって生成されているという業界記事による推計もあり、AIファーストのアプリビルダー市場は前年から大きく成長しています<sup>[7], [8]</sup>。
...
| ツール | 提供元 | 強み |
|---|---|---|
| Bolt.new | StackBlitz | ブラウザ完結のフルIDE。標準バックエンドはBolt Database(Bolt Cloud)で、Supabaseへの接続もオプションとして選択可能。直接コード編集も可能<sup>[17], [18]</sup> |
...
- **バックエンドの制約を理解する**: Bolt.newは標準バックエンドのBolt Database(Supabaseへの接続もオプションとして選択可能)、Lovableは同様にSupabase中心の統合になっているなど、特定のバックエンドサービスに依存する設計のツールが多く、後から別のデータベースに移行する場合は追加の作業が発生します<sup>[17]</sup>。
...
DORAの2025年レポートでは、AI活用度はソフトウェアデリバリーのスループットやプロダクトパフォーマンスの向上と正の相関を示す一方、デリバリーの安定性とは負の相関を示すと報告されています<sup>[5], [6]</sup>。つまり「AIがコードを書く速度」の向上が、テストや構成管理といった土台を伴わないまま変更量だけを増やすと、品質が不安定化しかねないということです。この課題に対応する形で、AIによる自動コードレビューツールの利用が急速に広がりました。業界調査では、2026年時点で開発チームの4割超が何らかのAIコードレビューツールを導入しているという報告があります<sup>[22], [23]</sup>。
...
これまでソフトウェア開発の課題は「十分なドキュメントが書かれないこと」でした。しかし2026年になると、AIがドキュメントを素早く生成できるようになった結果、課題は「AIが書いたコードとAIが書いたドキュメントの整合性をどう保つか」、すなわち生成されたドキュメントが実装からすぐに陳腐化してしまう「ドキュメント負債」をどう防ぐかに移っています<sup>[31]</sup>。
...
Pieter Levels氏は、Nomad List、Remote OK、Photo AIなど複数のプロダクトを一人で開発・運営してきたインディーハッカーの代表格です。2025年2月には、ブラウザ上で動くフライトシミュレーター「fly.pieter.com」を一人でAIツール(CursorとThree.jsを利用)を使って開発し、公開から17日でARR(年間経常収益)100万ドルに到達したと報告されています<sup>[37]</sup>。同氏のポートフォリオ全体では、複数プロダクト合算で月間20万ドル前後の収益(自己申告ベース)を継続的に生み出しているとされています<sup>[38]</sup>。
...
DORAの2025年調査では、AI活用度の高いチームほどソフトウェアデリバリーのスループット(変更のリードタイムやデプロイ頻度)やプロダクト性能が向上する一方で、デリバリーの安定性(変更失敗率や障害からの復旧時間)は低下する傾向が報告されています<sup>[5], [6]</sup>。これは「AIによって開発が速くなった分、品質管理のプロセスも同じだけ強化しないと、安定性の低下という形でしっぺ返しを受ける」ことを意味します。
...
CodeRabbitが470件のオープンソースGitHub PR(AI共同作成320件、人間のみ150件)を分析した調査では、AI共同作成PRの平均指摘事項数(バグ・脆弱性・スタイル逸脱等)が10.83件だったのに対し、人間のみのPRでは6.45件と、約1.7倍の差があったと報告されています<sup>[45]</sup>。これは「AI生成コードのレビューはむしろ強化すべき」という考え方の裏付けとしてよく引用される数字です。AIにコードを書かせることと、AIが書いたコードを別の仕組み(あるいは人間)でチェックすることは、常にセットで運用する必要があります。
...
45. CodeRabbit: State of AI vs Human Code Generation Report
