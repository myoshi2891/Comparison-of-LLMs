...
- **バックエンドの制約を理解する**: Bolt.newは標準バックエンドのBolt Database(SupabaseはオプションのScript接続)、Lovableは同様にSupabase中心の統合になっているなど、特定のバックエンドサービスに依存する設計のツールが多く、後から別のデータベースに移行する場合は追加の作業が発生します<sup>[17]</sup>。
