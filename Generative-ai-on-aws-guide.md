...
推論時は、凍結した元の重みによる計算 `W x` と、低ランク行列によるアダプターの計算 `B(A x)` にスケーリング係数 `α/r`（αはアダプター出力の大きさを調整する係数、rはランク。両者の比がアダプターの実効的な影響度を決める）を掛けた項を**加算**し、その和が出力になります（`W x + (α/r)B(A x)`）。元の重みは書き換えず、スケーリングされた差分だけを足し込む構造です。なお、この`α`はoptimizerの学習率（重み更新1ステップあたりの歩幅）とは別物で、学習が終わった後も推論時の出力スケールを決める固定パラメータとして残ります。
...
| QLoRA | 低ランク行列のみ（基盤モデルは4bit量子化のまま凍結） | 非常に高い | メモリはさらに節約できるが学習時間はやや増加する傾向 |
...
flowchart TB
    ...
    A -->|"初期化<br/>重みをコピー"| E["ポリシーモデルが<br/>プロンプトへの応答を生成"]
...
flowchart TB
    subgraph FWD["前方拡散プロセス Forward Diffusion<br/>（潜在拡散モデル＝Stable Diffusion等における学習プロセス）"]
        ...
        C --> D["ノイズを加えた潜在表現<br/>＝ノイズ予測の学習教材"]
    end
    subgraph REV["逆拡散プロセス Reverse Diffusion<br/>（生成時）"]
        subgraph UNET["U-Net"]
            K["U-Netへの入力<br/>（ノイズ潜在表現）"] --> G["ノイズを予測"]
            F["クロスアテンション層で<br/>テキスト条件を内部に注入"] --> G
        end
        E["テキストプロンプト<br/>条件付け"] --> F
        G --> H["予測ノイズを除去して<br/>1ステップ更新<br/>（潜在空間）"]
        H -->|"未完了ならステップを反復"| K
        H -->|"反復完了後"| J["最終的な潜在表現<br/>（潜在空間）"]
    end
    N["初期ノイズ<br/>ガウス分布からサンプリング<br/>（潜在空間のノイズ）"] --> K
    D -.->|ノイズ予測器の学習<br/>（学習時の入力）| K
    J -->|"VAEデコーダ"| I["生成された画像<br/>（ピクセル空間）"]
```

拡散モデルは、元画像にあたる**潜在表現**に少しずつノイズを加えていく「前方拡散プロセス」と、そのノイズを少しずつ取り除きながら潜在表現を復元していく「逆拡散プロセス」という2つのプロセスから成り立ちます。Stable Diffusionはピクセル空間ではなく、VAE（Variational Autoencoder）エンコーダで圧縮した潜在空間上でノイズの付加・除去を行う「潜在拡散モデル（Latent Diffusion Model）」であり、これによりピクセル空間で直接拡散を行うより計算コストを大きく削減できます。ここで注意したいのは、前方拡散が**学習時の処理**だという点です。学習データの画像をVAEエンコーダで潜在表現に変換したうえで段階的にノイズ化し、「どのノイズが加えられたか」をU-Netに当てさせることでノイズ予測器を訓練します。一方、テキストから画像を生成する**推論時には元画像は不要**で、ガウス分布からサンプリングした純粋なノイズ（潜在空間上のノイズ）を出発点として逆拡散を回します（上図でも、学習時の入力である「ノイズを加えた潜在表現」は点線でU-Netの入口へ入り、推論時は前方拡散の出力ではなく独立した「初期ノイズ」ノードから逆拡散へ接続しています）。テキストから画像を生成する場合は、この逆拡散プロセスの各ステップで、テキストプロンプトの情報を「クロスアテンション」という仕組みを通じてU-Netに注入し、ノイズ除去の方向をテキストの意味に沿ったものへと導きます。逆拡散で得られた最終的な潜在表現は、VAEデコーダによってピクセル空間の画像へ変換され、これが生成結果として出力されます。なお、Image-to-ImageやInpaintingのように入力画像を出発点とする推論では、その入力画像もVAEエンコーダで潜在表現に変換し、そこに対して（学習時とは別の目的で）前方拡散でノイズを加えてから逆拡散を開始することがあります。

本章ではStable Diffusion 2とStable Diffusion XLという2つのアーキテクチャの違い（XLでは条件付けの仕組みが強化され、加えてRefinerモデルが用意されている点など）も具体的に解説されています。XLのBaseモデルはそれ単体で（VAEデコーダによるピクセル空間への変換まで含め）画像を生成でき、Refinerは**Baseモデルが生成した潜在表現に対して**最終的なデノイズ段階を**任意で**追加適用し仕上げる別モデルです（ピクセル空間の画像を直接処理するわけではなく、必須の後段工程でもありません）。また、画像だけでなく「Inpainting（画像の一部を修復・置換）」「Outpainting（画像の外側を拡張）」「Depth-to-Image（深度情報から画像を生成）」といった応用的な画像編集タスクも扱われます。
...
> 上表の各行は、末尾に示したAWS公式資料に基づいています。**利用可能なモデル・リージョン、アカウントごとの提供条件（Bedrock Agents Classic の許可リスト判定）、Intelligent Prompt Routing の対象となりうるモデルファミリは、いずれもリージョンとアカウントによって異なります**。導入前に必ず参考文献18〜21の公式ドキュメントで自分のリージョン・アカウントの条件を確認してください。これに対し、実際のルーティング条件は可用性の話とは別に、**カスタムプロンプトルーター側で `responseQualityDifference`（許容する応答品質差）と `fallbackModel`（フォールバック先モデル）をユースケースに応じて設定する**ことで決まります。
...
- **LoRA/QLoRAの実践的チューニング**：Sebastian Raschka氏によるLoRA・QLoRAの実験ブログは、ハイパーパラメータ選定の実務的な勘所を数値付きで示しており、第6章の理解を補完します（参考文献6・7・23）。
...
| 18 | AWS公式ドキュメント「Models at a glance」（モデルID・リージョン対応状況を掲載） | https://docs.aws.amazon.com/bedrock/latest/userguide/model-cards.html |
...
| 24 | AWS公式ドキュメント「Observability for managed knowledge bases」（X-Rayトレース配信がRetrieve APIのみ対象であることの原典） | https://docs.aws.amazon.com/bedrock/latest/userguide/kb-managed-observability.html |
