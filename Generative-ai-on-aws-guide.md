...
推論時は、凍結した元の重みによる計算 `W x` と、低ランク行列によるアダプターの計算 `B(A x)` にスケーリング係数 `α/r`（αは学習率的な役割を持つハイパーパラメータ、rはランク）を掛けた項を**加算**し、その和が出力になります（`W x + (α/r)B(A x)`）。元の重みは書き換えず、スケーリングされた差分だけを足し込む構造です。
