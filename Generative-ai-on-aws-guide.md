...
LoRAとQLoRAの実践的なチューニングについては、機械学習分野で著名な研究者・教育者であるSebastian Raschka氏が、数百〜数千回規模のLoRA/QLoRA実験を行った知見を公開しており、「QLoRAはメモリを約33%削減できる一方で学習時間が約33%増加するというトレードオフがある」といった具体的な指摘は今でも広く引用されています（参考文献6・7）。本書の該当章と合わせて読むことで、理論だけでなく実務上の勘所もつかめます。
...
| エージェント機能 | 「Agents」として第9章・第12章で紹介 | 旧来の「Bedrock Agents」は2026年7月30日付けで新規利用停止となり「Bedrock Agents Classic」として保守モードへ。後継の「Amazon Bedrock AgentCore」がエージェント構築の中心的な仕組みに |
...
| ルーティング／コスト最適化 | 明示的なモデル選択が前提 | 簡単な質問は軽量モデルへ、複雑な推論は高性能モデルへ自動的に振り分ける「Intelligent Prompt Routing」などの最適化機能が追加 |
