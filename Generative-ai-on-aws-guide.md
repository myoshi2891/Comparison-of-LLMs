...
```mermaid
flowchart LR
    A["元の重み行列 W<br/>凍結・更新なし"] --> D["出力"]
    B["低ランク行列 A"] --> C["低ランク行列 B"]
    C --> D
    E["入力"] --> A
    E --> B
```
...
```mermaid
flowchart TB
    subgraph FWD["前方拡散プロセス Forward Diffusion"]
        A["元画像"] --> B["少しノイズを加える"]
...
拡散モデルは、元画像に少しずつノイズを加えていく「前方拡散プロセス」と、そのノイズを少しずつ取り除きながら画像を復元していく「逆拡散プロセス」という2つのプロセスから成り立ちます。テキストから画像を生成する場合は、この逆拡散プロセスの各ステップで、テキストプロンプトの情報を「クロスアテンション」という仕組みを通じてU-Netに注入し、ノイズ除去の方向をテキストの意味に沿ったものへと導きます。

本章ではStable Diffusion 2とStable Diffusion XLという2つのアーキテクチャの違い（XLではRefinerモデルが追加され、条件付けの仕組みも強化されている点など）も具体的に解説されています。また、画像だけでなく「Inpainting（画像の一部を修復・置換）」「Outpainting（画像の外側を拡張）」「Depth-to-Image（深度情報から画像を生成）」といった応用的な画像編集タスクも扱われます。
