...
flowchart TB
    subgraph QLoRA["QLoRAの構成"]
        Input["入力（隠れ状態）"] --> Base["事前学習済みモデルの重み<br/>（4bitに量子化し凍結）"]
        Base --> Frozen["推論時もこの重みは<br/>更新されない"]
...
| 発展方向 | 概要 |
|---|---|
| ハイブリッド検索の普及 | 密検索とBM25のようなキーワード検索をReciprocal Rank Fusion等で組み合わせ、単独手法より高い検索精度を得る構成が有力な構成の一つとして広がっている（普及度を示す確認可能な出典は本ガイドでは提示できていません） |
...
12章で扱われたDPOはその後も選好チューニングの代表的な手法の一つとして使われ続けていますが、「アライメントの実務上のデフォルト」と呼べるだけの採用率データは本ガイドでは確認できていません。SimPOやORPOといった、より計算効率を高めた派生手法も提案されていますが、その普及度についても確認可能な出典を本ガイドでは提示できていません。加えて2024年にDeepSeek社がDeepSeekMath論文（参考文献14）で提案したGroup Relative Policy Optimization（GRPO）は、数学やコード生成のように答えの正誤が機械的に検証できるタスクにおいて、別途の報酬モデルを訓練することなく強化学習で推論能力を直接鍛える手法として注目を集めました。その後「推論モデル」と呼ばれるモデル群の学習に採用された例が知られていますが、採用時期や普及の程度を直接裏付ける日付付きの出典は参考文献14（GRPOの提案論文自体）だけでは示せないため、本ガイドでは「2025年以降に急速に普及した」とは断定しません。これは12章の「報酬モデルを使わない選好チューニング」という論点の延長線上にある技術的進展の一例として押さえておくとよいでしょう。
