...
本書の全コード例は公式GitHubリポジトリでJupyter Notebookとして公開されており、実行環境としてはGoogle Colabの利用が推奨されています（利用可能な場合）。ただし**Colabの無料枠でGPUが必ず割り当てられる保証はなく、割り当てられるGPUの種類・VRAM容量・連続利用時間はいずれも保証されません**。実行できない、あるいは途中で切断される場合は、有料プランやローカルGPU、クラウドGPUインスタンスへの切り替えを検討してください。本ガイドはコード再掲を行いませんが、実際に手を動かす際は以下の環境整備が前提になります。
...
本書は、LLM登場以前から使われてきたWord2Vecアルゴリズムにも触れています。Word2Vecは「周囲に出現する単語が似ている単語は、意味も似ている」という分布仮説に基づき、CBOW（周囲の単語から中心の単語を予測）またはSkip-gram（中心の単語から周囲の単語を予測）という予測タスクを解くことで単語埋め込みを獲得します。実装上よく使われるネガティブサンプリング版のみが、正解ペアのスコアを上げ無作為抽出した不正解ペアのスコアを下げるという対照学習に近い目的関数を採用しており、Word2Vec全体が「似た文脈の単語ペアを近づけ、無関係な単語ペアを遠ざける」対照学習だと一般化することはできません。とはいえこのネガティブサンプリングの考え方は、後の章で登場するテキスト埋め込みモデルの訓練（10章）やCLIPのようなマルチモーダル埋め込み（9章）で使われる対照学習とも技術的につながっており、LLM時代を通じて重要な学習パラダイムの源流の一つです。
...
3章は、1章で概念的に触れたTransformerの内部構造に踏み込みます。自己回帰的にテキストを生成する訓練済みデコーダ専用Transformer LLMは、トークンID列を入力として受け取り、「次に来る可能性が最も高いトークン」の確率分布を出力します（BERTのようなエンコーダ専用モデルはこの次トークン予測を行いません）。この「次のトークンを予測する」という単純な仕組みを繰り返し適用することで、一見複雑な文章生成が実現されています。
...
flowchart TB
    ...
    subgraph Agents["Agents<br/>（必要に応じてモデル・チェーン・<br/>メモリを組み合わせて使う）"]
        A1["ReAct<br/>推論と行動を交互に繰り返す"]
        A2["ツール呼び出し<br/>（検索・計算等の外部機能）"]
    end

    Agents -.必要に応じて利用.-> ModelIO
    Agents -.必要に応じて利用.-> Chains
    Agents -.必要に応じて利用.-> Memory

    classDef groupStyle fill:#1e3a5f,stroke:#7c9eff,color:#eaf1ff
    class M1,C1,C2,Mem1,Mem2,Mem3,A1,A2 groupStyle
```

上図は必須の実行順序を表すものではなく、LangChainが提供する主要コンポーネントの一例です。Agentsは目的に応じてModel I/O・Chains・Memory・ツール・検索器などを任意に組み合わせて利用します。
...
第3部からは、既製モデルを使うだけでなく、モデル自体を訓練・調整する段階に入ります。10章はまず、2章・9章で繰り返し登場した「対照学習（Contrastive Learning）」を、共有された事前学習済みBERTエンコーダを対照学習でファインチューニングして埋め込みモデルを作る手法として掘り下げます。
...
flowchart TB
    subgraph QLoRA["QLoRAの構成"]
        Input["入力（隠れ状態）"] --> Base["凍結済み基盤モデル<br/>（4bitに量子化された重み、<br/>推論時も更新されない）"]
        Base --> BaseOut["基盤モデルの出力"]
        Input --> LoRA_A["低ランク行列A<br/>（訓練対象）"]
        LoRA_A --> LoRA_B["低ランク行列B<br/>（訓練対象）"]
        BaseOut --> Combine["基盤モデルの出力 + 低ランク行列による<br/>差分を加算"]
        LoRA_B --> Combine
    ...
    class Base,BaseOut frozenStyle
...
flowchart TB
    ...
    subgraph DPO["DPOのパイプライン"]
        ...
        D3["凍結した参照ポリシー<br/>（SFT済みモデルのコピー）<br/>の対数尤度と比較"] --> D2
    end
    ...
    class D1,D2,D3 dpoStyle
```

RLHFは「報酬モデルを訓練し、その報酬を強化学習で最大化する」という2段構えのプロセスであり、実装が複雑で訓練が不安定になりやすいという課題がありました。DPOはこれを、報酬モデルも強化学習のロールアウトも用いずに、訓練対象のポリシーと凍結した参照ポリシー（SFT済みモデルのコピー）それぞれの対数尤度の比を用いて、選好データそのものから直接ポリシーを最適化する分類問題として再定式化する手法であり、実装のシンプルさと訓練の安定性を特徴としています（採用率を示す確認可能な出典は本ガイドでは提示できていません）。本書はさらに、報酬モデルの入出力設計や、報酬モデルを使わない選好チューニングの選択肢についても整理しています。この選好チューニング手法の系譜（DPOからその派生手法、そして推論モデル向けの強化学習手法へ）は、2024年以降さらに大きく発展しており、第4部で詳しく取り上げます。
...
本書2章はサブワードトークナイザーの限界（多言語間の不公平な圧縮効率、未知の文字列への弱さなど）を指摘していましたが、この課題への根本的な解決策として、テキストをトークナイザーを介さず生のバイト列として直接扱う「バイトレベルモデル」の研究が2024年末以降大きく進展しています。Meta社が発表したByte Latent Transformer（BLT）は、バイトのエントロピー（予測の難しさ）に応じて動的に可変長の「パッチ」へグルーピングする仕組みを採用し、同等の計算量で従来のBPEベースモデルと同等以上の性能を達成しつつ、誤字や低リソース言語への頑健性を高めています。2026年に入ってからも、BLT以降の研究が継続的に進展していますが、その方向性は一様ではありません。例えば1B〜8Bパラメータ規模のモデルを対象に、トークナイザーの異なる教師モデルと学生モデルの間で知識蒸留を行う際、両者の出力分布を共通の「バイトレベル・インターフェース」へ変換して比較する手法「Byte-Level Distillation（BLD）」が提案されています（参考文献25）。具体的には、教師モデルの出力分布をバイトレベル確率に変換したうえで、学生モデルに軽量なバイトレベル出力ヘッドを一時的に付与して蒸留を行い、蒸留完了後はこのヘッドを取り除きます。そのため学生モデル自体はバイトレベルモデルにはならず、通常のトークンレベルモデルのまま残ります。つまりBLDは「トークナイザーフリーな最終モデル」を作る手法ではなく、異なるトークナイザー間の知識移転を単純化するための蒸留手法という位置づけです。報告されている性能（比較対象となるより複雑なクロストークナイザー蒸留手法と同等ないしそれを上回るとされる）や実運用への適用可能性は論文ごとに前提（対象モデル・評価ベンチマーク・計算予算）が異なるため、「トークナイザーフリーが実用段階に入った」根拠として一般化せず、採用を検討する際は個々の一次文献で実験条件を確認してください。

### 埋め込みモデルとMTEBの現在地（2章・10章関連）

埋め込みモデルの性能を測る標準的なベンチマークがMTEB（Massive Text Embedding Benchmark）です。近年は、Qwen系・Gemini Embedding・NVIDIAのLlama-Embed-Nemotron等、大規模言語モデルをベースに対照学習でファインチューニングした「LLMベース埋め込みモデル」が注目を集めています。ただしMTEBには言語カテゴリ（英語・多言語など）ごとに複数のリーダーボードとタスクセットがあり、順位は更新のたびに入れ替わります。本ガイドでは特定時点の順位を断定せず、**公式リーダーボード（参考文献21）で、自分が対象とする言語カテゴリ・タスクセットと参照日時点のスコアを必ず自分で確認する**ことを推奨します。一方で実務上は、MTEBの総合順位が必ずしも特定ドメイン（法律、金融など）での性能を保証しないという指摘もあり、汎用ベンチマークのスコアだけでなく自社データでの評価を重視すべきだという考え方が提唱されています（採用率を示す確認可能な出典は本ガイドでは提示できていません）。またMatryoshka表現学習により、1つの埋め込みモデルから用途に応じて次元数を柔軟に切り詰められる手法も登場し、ストレージとレイテンシのトレードオフを調整しやすくなっています（普及度を示す確認可能な出典は本ガイドでは提示できていません）。
...
| 発展方向 | 概要 |
|---|---|
| ハイブリッド検索 | 密検索とBM25のようなキーワード検索をReciprocal Rank Fusion等で組み合わせ、単独手法より高い検索精度を得る構成が有力な構成の一つとして挙げられる（普及度を示す確認可能な出典は本ガイドでは提示できていません） |
| Contextual Retrieval | Anthropicが2024年9月に発表した手法で、文書を分割する際に各チャンクへ文書全体を踏まえた説明文を付与する。同社の評価実験（コードベース・小説・ArXiv論文など複数分野のデータセット、Gemini/Voyage等の埋め込みモデル、top-20チャンク検索の条件下）では、Contextual Embeddings単体で失敗率を35%（5.7%→3.7%）、Contextual BM25と組み合わせて49%（5.7%→2.9%）削減したと報告されている。これは当該実験条件下での測定値であり、一般的な保証ではない |
| GraphRAG | Microsoft Researchが公開した、文書からナレッジグラフを構築し、グラフ構造を辿って複数文書にまたがる複雑な質問に答えることを狙う手法（有効性はデータ構造とタスクに依存し、本ガイドでは普及度を示す確認可能な出典を提示できていません） |
| Agentic RAG | 検索をパイプラインの前段に固定するのではなく、エージェントのループの中に組み込み、検索結果が不十分ならクエリを書き換えて再検索したり、追加の検索ラウンドを自律的に判断させたりする構成（提唱されている設計パターンであり、本ガイドでは普及度・効果を示す確認可能な出典を提示できていません） |

これらの発展は、8章で紹介されていた「密検索＋リランキング」という基本形の延長線上にあります。「タスクの複雑さに応じて検索パイプラインの複雑さを動的に切り替える」適応的な設計は、こうした流れの中で語られる**有力な設計パターンの一つ**ですが、普及度を示す日付付きの利用状況データを本ガイドでは確認できていないため、「主流」と断定はしません。採用の判断は自社のタスクと評価セットでの実測に基づいて行ってください。

### ファインチューニング手法の標準化：QLoRA・DoRA・統合ライブラリ（12章関連）

12章で扱われたQLoRAは、単一GPUでのファインチューニングにおいて広く使われる選択肢の一つです（「事実上のデフォルト」と呼べるだけの採用率データは本ガイドでは確認できていません）。LoRAの改良版である**DoRA（Weight-Decomposed LoRA）**、すなわち重み更新を「大きさ」と「方向」に分解しLoRAを方向成分のみに適用する手法も提案されていますが、どのフレームワークで利用できるかは実装ごとに異なるため、採用前に使用するライブラリのドキュメントで対応状況を確認してください。またHugging Face社のTRLライブラリが2026年3月31日にバージョン1.0に到達し、SFT・DPO・KTO・ORPO・GRPOという多様な訓練手法を1つのライブラリに統合したことで、12章で紹介された各種手法を実装レベルで一貫した形で扱える環境が整いました。

### 選好チューニングの多様化とGRPOの台頭（12章関連）

12章で扱われたDPOはその後も選好チューニングの代表的な手法の一つとして使われ続けていますが、「アライメントの実務上のデフォルト」と呼べるだけの採用率データは本ガイドでは確認できていません。SimPOやORPOといった、より計算効率を高めた派生手法も提案されていますが、その普及度についても確認可能な出典を本ガイドでは提示できていません。加えて2024年にDeepSeek社がDeepSeekMath論文（参考文献14）で提案したGroup Relative Policy Optimization（GRPO）は、PPO（Proximal Policy Optimization）を基礎とする強化学習手法の一種で、価値関数（クリティック）を省き、同一プロンプトに対する複数出力のグループ内相対評価から報酬のベースラインを求める点を特徴とします。DPOのような「報酬モデルを使わない選好チューニング」とは別系統の手法であり、数学やコード生成のように答えの正誤が機械的に検証できるタスク（verifiable rewards）はその代表的な適用例として知られています。具体例として、DeepSeek-AIが2025年1月に発表したDeepSeek-R1はGRPOを用いた強化学習で推論能力を獲得しています（参考文献26）。ただし、これ以降の「推論モデル」全般への採用時期や普及の程度を直接裏付ける日付付きの出典は本ガイドでは示せないため、「2025年以降に急速に普及した」とは断定しません。
...
| GRPO | critic（価値モデル）を使わず、同一プロンプトへの複数応答をグループ化してその中の相対スコアから基準値を推定する強化学習手法（検証可能な報酬を使う推論タスクはその代表的な利用例の一つ） |
...
25. Singh, Wu, Cioba, Bernacchia, Buffelli「Cross-Tokenizer LLM Distillation through a Byte-Level Interface」（2026年4月。1B〜8Bパラメータ規模のモデルを対象に、異なるトークナイザー間の知識蒸留を共通のバイトレベル・インターフェースで行う手法「Byte-Level Distillation（BLD）」の一次出典。蒸留後は学生モデルの出力ヘッドを通常のトークンレベルへ戻すため、学生モデル自体はバイトレベルモデルにはならない） — https://arxiv.org/abs/2604.07466
26. DeepSeek-AI「DeepSeek-R1: Incentivizing Reasoning Capability in LLMs via Reinforcement Learning」（2025年1月。GRPOを用いた推論モデル学習の一次出典） — https://arxiv.org/abs/2501.12948
