...
7. [章立てで見る本書の構成(全11章と付録)](#章立てで見る本書の構成全11章と付録)
...
- **保存期間と派生データの削除**: 保持期間は生データだけでなく、そこから派生したすべてのデータに適用します。元の投稿が削除された場合、利用許諾が撤回された場合、保存期限が到来した場合は、以下を漏れなく無効化・削除してください（LLM Twinでは1件の生データがMongoDB→チャンク→埋め込み→データセット→モデル→監視ログへと多段に複製されるため、生データだけ消しても実質的には残り続けます）
  ...
  - 当該データで学習したファインチューニング済みモデル（モデルレジストリ上で退役・削除するか、削除済みデータで再学習したモデルへ置き換える。学習済み重みにはデータが残るため、データセットの再生成だけでは消えない）
  - 当該モデルを配信している推論エンドポイント（旧モデルを配信し続けないよう停止するか、再学習済みモデルへ差し替える）
  - プロンプト監視・トレースに保存された入出力ログ（Cometなどの監視基盤側の保持設定も確認する）
  - 削除後は、削除対象の`author_id`や文書IDでMongoDB・Qdrant・データセット・監視ログをそれぞれ再検索してヒットが0件であること、**および当該データで学習した旧モデルがどの推論エンドポイントからも配信されていないこと**（エンドポイントに紐づくモデルバージョンを実際に照会して確認する）を確かめ、記録に残す。旧モデルの退役・削除・置換を実施した場合は、推論エンドポイント確認と同様に**モデルレジストリ側の状態も照会し**（退役・削除済み、または置換後の新モデルに切り替わっていること）、その確認結果もあわせて完了記録へ含める
...
本書は全11章と付録で構成されています。以下、各章の狙いを初学者向けに要約します。
...
| Google Chrome | Seleniumドライバと互換のバージョン | データ収集パイプラインのブラウザ自動操作 |
...
10. AWS SageMakerを使う場合は、`poetry install --with aws` で追加インストールしたうえで、**デプロイ前にAWS側の設定を済ませる**:
    ...
    - 各コマンドはリポジトリルートにJSONファイルを出力する。`create-sagemaker-role` は `sagemaker_user_credentials.json` に新規作成される専用ユーザー`sagemaker-deployer`のアクセスキー・シークレットキーを、`create-sagemaker-execution-role` は `sagemaker_execution_role.json` に実行ロールのARNを、それぞれ出力する。これらの値を `.env` の `AWS_ACCESS_KEY` / `AWS_SECRET_KEY`（`sagemaker_user_credentials.json` 由来）と `AWS_ARN_ROLE`（`sagemaker_execution_role.json` 由来）に**書き換える**（ブートストラップに使った管理者キーをそのまま使い続けない）。**`sagemaker_user_credentials.json` にはアクセスキー・シークレットキーが平文で書き出されるため、値を `.env` へ転記し終えたら、この2ファイルをリポジトリルートから必ず削除する**。誤コミットを防ぐため、事前にルート `.gitignore` へ `/sagemaker_user_credentials.json` と `/sagemaker_execution_role.json` を追加しておくことを推奨する
