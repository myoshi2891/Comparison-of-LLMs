...
- **最新 HEAD**: Phase B-1 完了（`8515ec3`）。`legacy/claude/skill.html`（2149 行）を `web-next/app/claude/skill/page.tsx` + `page.module.css` として Server Component で移植済
- **次の作業**: Phase B-2（`legacy/gemini/skill.html` → `web-next/app/gemini/skill/page.tsx`）。区切りのため新セッションで再開予定
...
| B | Provider skill.html × 4 (`claude` / `gemini` / `codex` / `copilot`) | 進行中（B-1 完了 / B-2〜B-4 未着手） | B-1: Red `d4735b4` / Green `8515ec3` / chore `b984f16` |
...
上記を読んで現在地を把握した上で、Phase A Green に進んでください。

Phase A Green の目標:
- web-next/components/site/nav-links.ts を作成（型付きデータ + Zod スキーマ）
- web-next/components/site/SiteHeader.tsx を作成（Server Component、nav 構造描画）
- web-next/components/site/SiteHeaderClient.tsx を作成（Client、ハンバーガー + dropdown ハンドラ）
- web-next/components/site/DisclaimerBanner.tsx を作成（Client、ResizeObserver + CSS 変数同期）
- web-next/app/layout.tsx に <SiteHeader /> と <DisclaimerBanner /> を挿入（{children} の前、body に class="has-common-header"）
- Phase A Red テスト 5 本（~38 ケース）が全 green 化することを確認

参考: legacy/shared/common-header.js（L50-91 で defaultLinks、L298-309 で disclaimer テキスト、L331 で has-common-header クラス）を clean URL に書き換えつつ移植。
