...
- 最新 HEAD: 1e99e3f (Phase 13 完了、deploy preview 検証待ち)
- 移行計画: docs/NEXTJS_MIGRATION_PLAN.md
- 進捗トラッカー: MIGRATION_PROGRESS.md

上記を読んで現在地を把握した上で、Phase 14 (Cutover) に進んでください。

Phase 14 の目標:
- Deploy Preview の検証結果を確認し、問題なければ main へ PR merge
- ルート /pricing.json URL を維持 (最小案: web-next/public/pricing.json 配置)
- web/ → legacy/web/ 移動
- update.sh を web-next 主系列に更新
- docs/NEXTJS_MIGRATION_PLAN.md の設計判断 8 を「棚上げ」として注記
