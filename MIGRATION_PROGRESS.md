...
- **最新 HEAD**: `ea7cb68`（**feat(web-next): D-1 Green s09 (YAMLフロントマター全フィールド解説) 移植完了**）
- **未コミット作業**: なし（working tree クリーン）
- **次の作業**: **D-1 Green s10 (`args`) 移植**（`legacy/claude/skill-guide-for-claude.html` 行 1531–1583）
...
| D | Long-form guides × 9（全ページ HTML → JSX 直接移植。MDX 不採用） | **着手中** — D-1 Green s09 完了 | Red: `af1e9e7` / s01–s09: 完了 |
...
次の作業: **D-1 Green s10 着手** — s09 完了（ea7cb68）。
...
D-1 `/claude/skill-guide` は s09 完了。次は Green s10（`args` セクション行 1531–1583）を実装する。
