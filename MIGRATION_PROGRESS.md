...
- **コミット済み（C-2 関連、新しい順）**:
  ...
  - `a1a6e47` feat(web-next): faithful migration of /gemini/agent s17 (full file role summary table)
  - `87b0cb9` feat(web-next): faithful migration of /gemini/agent s16 (multi-agent anti-patterns)
  - `6f04352` feat(web-next): faithful migration of /gemini/agent s15 (use cases ucGrid)
  - `5932c9b` feat(web-next): faithful migration of /gemini/agent s14 (AgentEngine deploy)
  - `c8ef2b8` feat(web-next): faithful migration of /gemini/agent s13 (Orchestrator + RemoteA2aAgent) (+245/-30)
  - `7ede8d7` feat(web-next): faithful migration of /gemini/agent s12 (multi-agent GEMINI.md)
  - `8e49dd4` feat(web-next): faithful migration of /gemini/agent s11 (agent.json Agent Card)
  - `fb235dc` feat(web-next): faithful migration of /gemini/agent s10 (4-layer ADK×A2A×MCP×AP2/A2UI)
  - `c42542d` feat(web-next): faithful migration of /gemini/agent s09 (summary table)
  - `7e4aa2f` feat(web-next): faithful migration of /gemini/agent s08 (anti-patterns)
  - `3a03261` feat(web-next): faithful migration of /gemini/agent s07 (cost-optimal model selection)
  - `1cd285f` feat(web-next): faithful migration of /gemini/agent s06 (routing decision tree)
  - `60cf089` feat(web-next): faithful migration of /gemini/agent s05 (.geminiignore / settings.json)
  - `9d1a6bd` feat(web-next): faithful migration of /gemini/agent s04 (ADK agent.py)
  - `8b582a3` feat(web-next): faithful migration of /gemini/agent s03 (AGENTS.md interop) (+113/-16)
...
| C | Provider agent.html × 4 | **C-1 完了 / C-2 完了（faithful 全件）/ C-3・C-4 未着手** | C-1: Red `2b7a0fa`, `2e6f993` / Green `5394d9d` ／ C-2: Red `9dfa184` / 準備 `cb62441` / Green `aa9c2ee` / s01 `e9e9547` / s02 `90b163e` / s03 `8b582a3` / s04 `9d1a6bd` / s05 `60cf089` / s06 `1cd285f` / s07 `3a03261` / s08 `7e4aa2f` / s09 `c42542d` / s10 `fb235dc` / s11 `8e49dd4` / s12 `7ede8d7` / s13 `c8ef2b8` / s14 `5932c9b` / s15 `6f04352` / s16 `87b0cb9` / s17 `a1a6e47` / sources `448368a` |
...
bun run test        # Vitest (current test count, may vary)
...
| `web-next/app/gemini/agent/page.tsx` | **faithful 移植完了**（`aa9c2ee` → hero/s01〜s17/sources 全件 commit 済み、最新 `448368a`） | legacy 全 3,722 行を省略なし JSX 化完了。SOURCES_EXISTING 12 件 + SOURCES_NEW 13 件 + srcCardNew 分割済み|
...
- [x] s03: AGENTS.md interop（legacy 1792-1888、4 列 6 行 table + AGENTS.md コードブロック）— `8b582a3`
- [x] s04: ADK agent.py パターン（legacy 1889-2035、card 2 + alertWarn + 2 コードブロック + agentGrid 3 + alertDanger）— `9d1a6bd`
- [x] s05: .geminiignore / settings.json（legacy 2036-2121）— `60cf089`
- [x] s06: ルーティング決定ツリー（legacy 2122-2189、`flowWrap` クラス使用）— `1cd285f`
- [x] s07: コスト最適モデル選択（legacy 2190-2263、`tblWrap`）— `3a03261`
- [x] s08: Anti-Patterns（legacy 2264-2298）— `7e4aa2f`
- [x] s09: まとめ（legacy 2299-2390、`tblWrap`）— `c42542d`
- [x] s10: ADK×A2A×MCP 4層構造（legacy 2391-2643、`maBanner` + `layerStack`）— `fb235dc`
- [x] s11: agent.json (Agent Card)（legacy 2644-2777、code-wrap 含む）— `8e49dd4`
- [x] s12: マルチエージェント GEMINI.md（legacy 2778-2873）— `7ede8d7`
- [x] s13: Orchestrator + RemoteA2aAgent（legacy 2874-3033、code-wrap 含む）— `c8ef2b8`
- [x] s14: AgentEngine（legacy 3034-3123）— `5932c9b`
- [x] s15: ユースケース（legacy 3124-3196、`ucGrid`）— `6f04352`
- [x] s16: マルチエージェント Anti-Patterns（legacy 3197-3231）— `87b0cb9`
- [x] s17: 全ファイル役割まとめ（legacy 3232-3333、`tblWrap`）— `a1a6e47`
- [x] sources: 25 件 src-card（legacy 3334-3722、SOURCES_EXISTING 12 件 + SOURCES_NEW 13 件 + srcCardNew クラス分離）— `448368a`

### 次セッションのワークフロー（厳守）

**C-2 は完了。次は C-3 `/codex/agent`**（`docs/NEXTJS_PHASE_C_DETAILED_DESIGN.md` §5.3 参照）。

C-2 で確立したサイクルを C-3・C-4 でも繰り返す:

1. `Read legacy/<provider>/agent.html` を行範囲指定で実行
2. `web-next/app/<provider>/agent/page.tsx` を `Edit` で faithful 版に置き換え
...
5. `cd web-next && bunx biome check app/<provider>/agent/page.tsx ...`（違反 0）
6. `cd web-next && bun run test`（全件 pass）
7. `git add ... && git commit -m "feat(web-next): faithful migration of /<provider>/agent <sNN> (...)"`
