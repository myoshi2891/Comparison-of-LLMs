...
- **最新 HEAD**: `d335f4d`（**feat(web-next): D-1 Green s12 (ベストプラクティス) 移植完了**）
- **未コミット作業**: なし（working tree クリーン）
- **次の作業**: **D-1 Green s13 (`debug`) 移植**（`legacy/claude/skill-guide-for-claude.html` 行 1691–1750）
...
| D | Long-form guides × 9（全ページ HTML → JSX 直接移植。MDX 不採用） | **着手中** — D-1 Green s12 完了 | Red: `af1e9e7` / s01–s12: 完了 |
...
次の作業: **D-1 Green s13 着手** — s12 完了（d335f4d）。
...
D-1 `/claude/skill-guide` は s12 完了。次は Green s13（`debug` セクション行 1691–1750）を実装する。
