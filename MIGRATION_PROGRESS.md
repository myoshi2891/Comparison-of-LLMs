...
- 最新 HEAD: 7f185b2（feat(web-next): faithful migration of /copilot/github-copilot s05/features）
- 未コミット作業: なし（working tree クリーン）
- 次の作業: **D-9 — `/copilot/github-copilot` s06/tips faithful 移植**
- テスト数: `bun run test` **534 passed / 534 total（全 Green ✅）**  — マージ前必須条件: `bun run build` / `bun run typecheck` / `bun run test`（全件 pass）/ `bun run lint`（新規違反ゼロ）/ `cd scraper && uv run pytest`（5/5）すべて成功していること
- ビルド: `bun run build` ✅（ee18287 時点）・`bun run typecheck` ✅・`uv run pytest` 5/5 passed
- **lint 既知違反**: 7 件
...
**テスト数**: **519 passed / 519 total（全 Green）** — マージ前必須条件: `bun run build` / `bun run typecheck` / `bun run test`（全件 pass）/ `bun run lint`（新規違反ゼロ）/ `cd scraper && uv run pytest`（5/5）すべて成功していること
...
- 最新 HEAD: 5779e97（feat(web-next): faithful migration of /copilot/github-copilot s02/plans）
...
次の作業: **D-9 — `/copilot/github-copilot` s06/tips faithful 移植**。scaffold + s01〜s05 完了（7f185b2 時点）。
...
- Copilot: markdown-file-guide（**次の作業**）/ github-copilot
- lint 既知違反: 7 件
...
検証コマンド早見表:
  (cd web-next && bun run test)          # 全件 pass が期待値（現在 514 件 全 pass）
