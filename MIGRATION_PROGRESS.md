...
- **最新 HEAD**: `4c61251`（**feat(web-next): D-1 Green s07 (CLAUDE.md との違い) 移植完了**）
- **未コミット作業**: なし（working tree クリーン）
- **次の作業**: **D-1 Green s08 (`build`) 移植**（`legacy/claude/skill-guide-for-claude.html` 行 1286–1423）
...
| D | Long-form guides × 9（全ページ HTML → JSX 直接移植。MDX 不採用） | **着手中** — D-1 Green s07 完了 | Red: `af1e9e7` / s01–s07: 完了 |
...
次の作業: **D-1 Green s08 着手** — s07 完了（4c61251）。
...
D-1 `/claude/skill-guide` は s07 完了。次は Green s08（`build` セクション行 1286–1423）を実装する。
