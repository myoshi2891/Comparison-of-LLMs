...
- **最新 HEAD**: `af1e9e7`（**feat(web-next): D-1 Red scaffold — /claude/skill-guide + MermaidDiagram component**）
- **未コミット作業**: なし（working tree クリーン）
- **次の作業**: **D-1 Green s01 (`what`) 移植**（`legacy/claude/skill-guide-for-claude.html` 行 760–827）。`MermaidDiagram` を `next/dynamic({ ssr: false })` で import して Mermaid ダイアグラムを実装
- **テスト数**: `bun run test` **469 tests（全件 pass）** + D-1 scaffold 5 テスト追加（うち 1 失敗が Red 状態）— マージ前必須条件: `bun run build` / `bun run typecheck` / `bun run test`（全件 pass）/ `bun run lint`（新規違反ゼロ）/ `cd scraper && uv run pytest`（5/5）すべて成功していること
...
| D | Long-form guides × 9（全ページ HTML → JSX 直接移植。MDX 不採用） | **着手中** — D-1 Red scaffold 完了 | Red: `af1e9e7` |
...
- 最新 HEAD: af1e9e7（feat(web-next): D-1 Red scaffold 完了 — /claude/skill-guide + MermaidDiagram）
...
次の作業: **D-1 Green s01 着手** — D-1 Red scaffold 完了（af1e9e7）。
...
D-1 `/claude/skill-guide` は Red scaffold 完了（af1e9e7）。次は Green s01（`what` セクション行 760–827）を実装する。
- `MermaidDiagram` は `components/docs/MermaidDiagram.tsx` に実装済み（mermaid@10.9.5、next/dynamic ssr:false で使用）
- セクション構造: hero(745-758) / what(760-827) / arch(828-919) / token(920-998) / structure(999-1058) / directory(1059-1141) / invoke(1142-1212) / claudemd(1213-1285) / build(1286-1423) / yaml(1424-1530) / args(1531-1583) / bundle(1584-1615) / best(1616-1690) / debug(1691-1750) / mistakes(1751-1827) / enterprise(1828-2008)
...
- 1 セクション完了ごとに以下を順に実行 → 全部 OK なら 1 コミット → 次のセクションへ:
  ...
  6. **MIGRATION_PROGRESS.md を更新 → git commit**（HEAD・次の作業・テスト数を同期）← **次セクション HTML を Read する前の必須ゲート**
...
検証コマンド早見表:
  (cd web-next && bun run test)          # 全件 pass が期待値（現在 469 件）
