...
- **最新 HEAD**: `78de4b3`（**feat(web-next): D-1 Green s08 (ステップバイステップ) 移植完了**）
- **未コミット作業**: なし（working tree クリーン）
- **次の作業**: **D-1 Green s09 (`yaml`) 移植**（`legacy/claude/skill-guide-for-claude.html` 行 1424–1530）
...
| D | Long-form guides × 9（全ページ HTML → JSX 直接移植。MDX 不採用） | **着手中** — D-1 Green s08 完了 | Red: `af1e9e7` / s01–s08: 完了 |
...
次の作業: **D-1 Green s09 着手** — s08 完了（78de4b3）。
...
D-1 `/claude/skill-guide` は s08 完了。次は Green s09（`yaml` セクション行 1424–1530）を実装する。
