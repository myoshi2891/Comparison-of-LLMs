...
- **最新 HEAD**: `4d5eaa5`（**feat(web-next): D-1 Green s06 (スキルが呼び出される仕組み) 移植完了**）
- **未コミット作業**: なし（working tree クリーン）
- **次の作業**: **D-1 Green s07 (`claudemd`) 移植**（`legacy/claude/skill-guide-for-claude.html` 行 1213–1285）
...
| D | Long-form guides × 9（全ページ HTML → JSX 直接移植。MDX 不採用） | **着手中** — D-1 Green s06 完了 | Red: `af1e9e7` / s01–s06: 完了 |
...
次の作業: **D-1 Green s07 着手** — s06 完了（4d5eaa5）。
...
D-1 `/claude/skill-guide` は s06 完了。次は Green s07（`claudemd` セクション行 1213–1285）を実装する。
