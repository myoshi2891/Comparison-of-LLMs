...
- ビルド: `bun run build` ✅（a2cb93a 時点）・`bun run typecheck` ✅・`uv run pytest` 5/5 passed
...
- 最新 HEAD: a2cb93a（fix(web-next): use direct MermaidDiagram import instead of dynamic() in git-worktree page）
- 移行計画: docs/NEXTJS_PHASE_A_F_PLAN.md（Phase A–F）§Phase F を参照
...
次の作業: **Phase F — netlify.toml [[redirects]] 一括設定 + sitemap 更新**
