...
- **最新 HEAD**: `62262d9`（**feat(web-next): D-2 Green s08 — /claude/skill-guide-intermediate enterprise (provisioning + network control)**）
- **未コミット作業**: なし（working tree クリーン）
- **次の作業**: **D-2 Green s09 移植 — Section 09 (self: 2318-2761 行) faithful 移植**
...
| D | Long-form guides × 9（全ページ HTML → JSX 直接移植。MDX 不採用） | **D-1 完了・D-2 Red 済**（D-1: 全 15 セクション faithful 移植完了・D-2 Red scaffold: 5/5 contract tests pass）| Red: `af1e9e7` / D-1 全セクション完了: `be2aef0` / D-2 Red: `d491da8` |
...
- 最新 HEAD: 62262d9（feat(web-next): D-2 Green s08 — /claude/skill-guide-intermediate enterprise）
...
次の作業: **D-2 (`/claude/skill-guide-intermediate`) Green 移植 — Section 09 (self: 2318-2761 行) から再開**。s01〜s08 完了済み（62262d9）。
...
D-2 のファイル: `legacy/claude/skill-guide-of-claude-for-intermediate.html`（2762 行）
出力先: `web-next/app/claude/skill-guide-intermediate/page.tsx`（scaffold 済）
