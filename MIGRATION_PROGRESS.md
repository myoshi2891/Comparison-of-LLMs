...
- 最新 HEAD: 37fbc3e（feat(web-next): faithful migration of /copilot/github-copilot s10/sources）
- 未コミット作業: なし（working tree クリーン）
- 次の作業: **D-10 — `/copilot/markdown-file-guide` scaffold → faithful 移植**
...
- ビルド: `bun run build` ✅（37fbc3e 時点）・`bun run typecheck` ✅・`uv run pytest` 5/5 passed
- **lint 既知違反**: 7 件
...
**テスト数**: **536 passed / 536 total（全 Green）** — マージ前必須条件: `bun run build` / `bun run typecheck` / `bun run test`（全件 pass）/ `bun run lint`（新規違反ゼロ）/ `cd scraper && uv run pytest`（5/5）すべて成功していること
...
次の作業: **D-10 — `/copilot/markdown-file-guide` scaffold → faithful 移植**（D-9 `github-copilot` 全セクション完了 37fbc3e）。
...
- Copilot: github-copilot ✅ / markdown-file-guide（**次の作業**）
- lint 既知違反: 7 件
...
検証コマンド早見表:
  (cd web-next && bun run test)          # 全件 pass が期待値（現在 536 件 全 pass）
