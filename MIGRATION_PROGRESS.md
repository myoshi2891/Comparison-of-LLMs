...
- **最新 HEAD**: `a2bcfdc`（**feat(web-next): D-5 s02 — faithful migration of /gemini/skill-guide-intermediate #sec-tools**）
- **未コミット作業**: なし（working tree クリーン）
- **次の作業**: **D-5 s03 — `/gemini/skill-guide-intermediate` #sec-progressive の faithful 移植**（legacy 行範囲: 1250-1353）
- **テスト数**: `bun run test` **501 passed / 502 total（h2 count 2/12 — 残 10 セクション移植で全 Green）**  — マージ前必須条件: `bun run build` / `bun run typecheck` / `bun run test`（全件 pass）/ `bun run lint`（新規違反ゼロ）/ `cd scraper && uv run pytest`（5/5）すべて成功していること
- **ビルド**: `bun run build` 未確認（a2bcfdc 時点）・`bun run typecheck` ✅・`uv run pytest` 5/5 passed
- **lint 既知違反**: 7 件
...
| D | Long-form guides × 9（全ページ HTML → JSX 直接移植。MDX 不採用） | **D-1・D-2・D-3・D-4 完了、D-5 進行中（s01-s02 完了）**（D-1: 全 15 セクション完了・D-2: 全 9 セクション完了・D-3: 全 11 セクション完了・D-4: 全 11 セクション完了）| Red: `af1e9e7` / D-1 全: `be2aef0` / D-2 Red: `d491da8` / D-2 全: `a62ceec` / D-3 Red: `11d9e79` / D-3 s01: `4f29f56` / s02: `10dca55` / s03: `7a2d136` / s04: `8bbfacb` / s05: `f496ac2` / s06: `998cb6d` / s07: `9a1f485` / s08: `6b8fb8c` / s09: `405c899` / s10: `8a919aa` / s11: `b32b162` / D-4 Red: `5b3ab9c` / D-4 s02: `5d29999` / D-4 s03: `e113515` / D-4 s04: `f32c2e8` / D-4 s05: `6e1f145` / D-4 s06: `b368263` / D-4 s07: `b6e8cc6` / D-4 s08: `8a3cea6` / D-4 s09: `7e22c66` / D-4 s10: `7a3ed65` / D-4 s11: `b44c895` / D-5 Red: `58f9048` / D-5 s01: `9fdcafc` / D-5 s02: `a2bcfdc` |
...
次の作業: **D-5 s03 — `/gemini/skill-guide-intermediate` #sec-progressive の faithful 移植**（legacy 行範囲: 1250-1353）。D-5 s01・s02 完了済み（a2bcfdc 時点）。
...
- Gemini: skill-guide-for-gemini ✅ / skill-guide-of-gemini-for-intermediate（進行中: s01-s02 完了 / s03-s12+footer 残） / antigravity-guide
- Codex: openai-codex-guide
- Copilot: markdown-file-guide / github-copilot
D-5 進行中: `legacy/gemini/skill-guide-of-gemini-for-intermediate.html`（2625 行 / 12 sections） → `web-next/app/gemini/skill-guide-intermediate/page.tsx`
- lint 既知違反: 7 件
