...
- **最新 HEAD**: `e113515`（**feat(web-next): D-4 s03 — faithful migration of /gemini/skill-guide #why**）
- **未コミット作業**: なし（working tree クリーン）
- **次の作業**: **D-4 s04 — `/gemini/skill-guide` #structure (lines 592–721) faithful 移植**
...
- **ビルド**: `bun run build` 成功（15 ルート全 Static）・`bun run typecheck` 成功・`uv run pytest` 5/5 passed（ビルド未再確認: D-4 scaffold+s02+s03 追加後は要確認）
- **lint 既知違反**: 7 件
...
| D | Long-form guides × 9（全ページ HTML → JSX 直接移植。MDX 不採用） | **D-1・D-2・D-3 完了、D-4 移植中（s02・s03 完了）**（D-1: 全 15 セクション完了・D-2: 全 9 セクション完了・D-3: 全 11 セクション完了）| Red: `af1e9e7` / D-1 全: `be2aef0` / D-2 Red: `d491da8` / D-2 全: `a62ceec` / D-3 Red: `11d9e79` / D-3 s01: `4f29f56` / s02: `10dca55` / s03: `7a2d136` / s04: `8bbfacb` / s05: `f496ac2` / s06: `998cb6d` / s07: `9a1f485` / s08: `6b8fb8c` / s09: `405c899` / s10: `8a919aa` / s11: `b32b162` / D-4 Red: `5b3ab9c` / D-4 s02: `5d29999` / D-4 s03: `e113515` |
...
**テスト数**: **490 passed / 490 total（全 Green）** — マージ前必須条件: `bun run build` / `bun run typecheck` / `bun run test`（全件 pass）/ `bun run lint`（新規違反ゼロ）/ `cd scraper && uv run pytest`（5/5）すべて成功していること
...
次の作業: **D-4 s04 — `/gemini/skill-guide` #structure (lines 592–721) faithful 移植**。D-4 scaffold+s02+s03 完了済み（e113515 時点）。
...
完了: scaffold(Red) / s02(#overview 252-511) / s03(#why 514-589)
次: s04(#structure 592-721) → s05(#howto 724-798) → s06(#steps 801-808+React) → s07(#install 811-932) → s08(#examples 935-944+React) → s09(#checklist 947-1064+React) → s10(#whatsnew 1067-1305) → s11(footer 1308-1319)
- lint 既知違反: 7 件
...
検証コマンド早見表:
  (cd web-next && bun run test)          # 全件 pass が期待値（現在 490 件 全 pass）
