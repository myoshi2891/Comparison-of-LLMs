...
- **ブランチ**: `feat/nextjs-migration`
- **ブランチ**: `feat/nextjs-migration`
- 最新 HEAD: c11185f（feat(web-next): faithful migration of /git-worktree s01）
- 未コミット作業: なし（working tree クリーン）
- 次の作業: **Phase E s02 — `git_worktree.html` 1381-1742 行 faithful 移植**（s01 完了）
...
- ビルド: `bun run build` ✅（e598670 時点）・`bun run typecheck` ✅・`uv run pytest` 5/5 passed
- **lint 既知違反**: 7 件
...
| E | git_worktree.html（Mermaid + SVG） | **進行中**（s01 完了、s02〜ref 未着手） | scaffold: `8fffdb9` / s00: `e598670` / s01: `c11185f` |
...
- 最新 HEAD: e598670（feat(web-next): faithful migration of /git-worktree s00）
- 移行計画: docs/NEXTJS_PHASE_A_F_PLAN.md（Phase A–F）§Phase E を参照
...
次の作業: **Phase E s00 — `legacy/git_worktree.html` 行 723-1287 faithful 移植**（scaffold 完了 `8fffdb9`）

セクション行範囲（各 Read 時に参照）:
| Section | 行範囲 | 内容 |
|---|---|---|
| s00 | 723-1287 | git worktree とは — SVG 構造比較図・比較表・コードブロック |
| s01 | 1288-1380 | リポジトリ初期設定 — Mermaid gitGraph |
| s02 | 1381-1742 | worktree 追加 — Mermaid + SVG フロー図 + コードブロック |
| s03 | 1743-1862 | 日常ワークフロー — Mermaid sequence |
| s04 | 1863-2009 | メリット・デメリット — mdg グリッド |
| s05 | 2010-2186 | 注意点 7 ルール — Mermaid + rules リスト |
| s06 | 2187-2282 | GitHub Actions 統合 — Mermaid |
| ref | 2283-2979 | 参考リンク集 |

Phase E の特記事項:
- **MermaidDiagram コンポーネント**: `web-next/components/docs/MermaidDiagram.tsx` が既存（`use client` + useEffect で動的 import）。s01 以降で `dynamic()` ラップして使用
- **Mermaid テストモック**: `vi.mock("next/dynamic", () => ({ default: () => () => null }))` をテストに追加（s01 実装時）
- **Mermaid 記述は左端揃え必須**（HTML インデントが混じると Mermaid v10 が構文エラーを出す）
- **SVG**: `role="img"` + `aria-label` + `<title>` 付与（Biome `noSvgWithoutTitle`）、属性 camelCase 変換
- lint 既知違反: 7 件
