...
- **最新 HEAD**: `8dcd79c`（**feat(web-next): D-1 Green s14 (よくあるミスと対処法) 移植完了**）
- **未コミット作業**: なし（working tree クリーン）
- **次の作業**: **D-1 Green s15 (`enterprise`) 移植**（`legacy/claude/skill-guide-for-claude.html` 行 1828–2008）
...
| D | Long-form guides × 9（全ページ HTML → JSX 直接移植。MDX 不採用） | **着手中** — D-1 Green s14 完了（全 474 tests pass）| Red: `af1e9e7` / s01–s14: 完了 |
...
次の作業: **D-1 Green s15 着手** — s14 完了（8dcd79c）。
...
D-1 `/claude/skill-guide` は s14 完了（全テスト Green）。次は Green s15（`enterprise` セクション行 1828–2008）を実装する。
