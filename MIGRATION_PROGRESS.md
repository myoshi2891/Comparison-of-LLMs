...
- **最新 HEAD**: `6b70f35` (Phase 11 コード検証済み + その後の i18n / a11y 修正を反映)
- **次の作業**: **Phase 12 統合テスト完了（未コミット）** → Phase 13 (Deployment) 着手準備
- **検証状態**: Phase 12 新規ファイル 2 本は lint / typecheck / test すべて green。全体では 19 新規統合テスト含む **361 件中 360 passed**（失敗 1 件は Phase 12 以前から持ち越しの `lib/i18n.test.ts` key count ハードコード — Phase 12 スコープ外）
...
```
docs/NEXTJS_INTEGRATION_TEST_DESIGN.md         # 設計書（新規）
MIGRATION_PROGRESS.md                           # 本ファイル（更新）
web-next/components/HomePage.integration.test.tsx  # 統合テスト（新規）
web-next/tests/fixtures/pricing.ts              # フィクスチャ（新規）
```
...
- 最新 HEAD: (Phase 12 統合テスト完了・コミット済み想定)
- 移行計画: docs/NEXTJS_MIGRATION_PLAN.md
- 進捗トラッカー: MIGRATION_PROGRESS.md
- Phase 12 設計書: docs/NEXTJS_INTEGRATION_TEST_DESIGN.md

上記を読んで現在地を把握した上で、Phase 13 (Deployment) に進んでください。

Phase 13 の目標:
- netlify.toml の更新（web-next/ をビルド対象に切替）
- @netlify/plugin-nextjs の動作確認
- ステージング URL（Netlify deploy preview）での実機確認
- 現行 web/ との並行運用（カットオーバーは Phase 14）

既知の Phase 12 外ブレ（別 Issue で対応）:
- lib/i18n.test.ts:18 の key count ハードコード (47 vs 45)
- 一部既存ファイルの Biome printWidth 違反
```
