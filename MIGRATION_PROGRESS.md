...
- **最新 HEAD**: `a62ceec`（**feat(web-next): faithful migration of /claude/skill-guide-intermediate s09 (self) — D-2 全セクション完了**）
- **未コミット作業**: なし（working tree クリーン）
- **次の作業**: **D-3 着手 — `/claude/claude-cowork-guide` scaffold (Red) → faithful 移植 (Green)**
...
- **ビルド**: `bun run build` 成功（14 ルート全 Static）・`bun run typecheck` 成功・`uv run pytest` 5/5 passed
...
| D | Long-form guides × 9（全ページ HTML → JSX 直接移植。MDX 不採用） | **D-1・D-2 完了**（D-1: 全 15 セクション faithful 移植完了・D-2: 全 9 セクション faithful 移植完了）| Red: `af1e9e7` / D-1 全セクション完了: `be2aef0` / D-2 Red: `d491da8` / D-2 全セクション完了: `a62ceec` |
...
- 最新 HEAD: a62ceec（feat(web-next): faithful migration of /claude/skill-guide-intermediate s09 — D-2 全セクション完了）
...
次の作業: **D-3 (`/claude/claude-cowork-guide`) scaffold (Red) → faithful 移植 (Green)**。D-1・D-2 完了済み（a62ceec）。
Phase D の対象は長文ガイド 9 ページ（すべて `.html` → JSX 直接移植。**MDX・`.md` SSoT 方式は不採用**）:
- Claude: skill-guide-for-claude ✅ / skill-guide-of-claude-for-intermediate ✅ / claude-cowork-guide ← **次**
...
D-3 のファイル: `legacy/claude/claude-cowork-guide.html`（行数は wc -l で確認してから着手）
出力先: `web-next/app/claude/claude-cowork-guide/page.tsx`（scaffold 未作成）
- `MermaidDiagram` は `components/docs/MermaidDiagram.tsx` に実装済み（mermaid@10.9.5、next/dynamic ssr:false で使用）
- CSS Module パターン: `web-next/app/claude/skill-guide-intermediate/page.module.css` を参考に新規作成
- 外部リンクは `target="_blank" rel="noopener noreferrer"` 付与必須
