...
- **最新 HEAD**: `405c899`（**feat(web-next): faithful migration of /claude/claude-cowork-guide s09 (best-practices)**）
- **未コミット作業**: なし（working tree クリーン）
- **次の作業**: **D-3 継続 — `/claude/claude-cowork-guide` s10 (advanced) faithful 移植**（s01〜s09 完了・s10〜s11 残り）
- **テスト数**: `bun run test` **489 passed / 490 total（h2 count=11 の 1 テスト fail — s10〜s11 実装で Green 予定）** — マージ前必須条件: `bun run build` / `bun run typecheck` / `bun run test`（全件 pass）/ `bun run lint`（新規違反ゼロ）/ `cd scraper && uv run pytest`（5/5）すべて成功していること
- **ビルド**: `bun run build` 成功（15 ルート全 Static）・`bun run typecheck` 成功・`uv run pytest` 5/5 passed
- **lint 既知違反**: 7 件
...
| D | Long-form guides × 9（全ページ HTML → JSX 直接移植。MDX 不採用） | **D-1・D-2 完了、D-3 進行中**（D-1: 全 15 セクション完了・D-2: 全 9 セクション完了・D-3: s01〜s09 完了 / s10〜s11 残り）| Red: `af1e9e7` / D-1 全: `be2aef0` / D-2 Red: `d491da8` / D-2 全: `a62ceec` / D-3 Red: `11d9e79` / D-3 s01: `4f29f56` / s02: `10dca55` / s03: `7a2d136` / s04: `8bbfacb` / s05: `f496ac2` / s06: `998cb6d` / s07: `9a1f485` / s08: `6b8fb8c` / s09: `405c899` |
...
- 最新 HEAD: 405c899（feat(web-next): faithful migration of /claude/claude-cowork-guide s09 — D-3 s09 完了）
...
次の作業: **D-3 継続 — `/claude/claude-cowork-guide` s10 (advanced) faithful 移植**。D-1・D-2 完了済み、D-3 s01〜s09 完了済み（405c899）。
Phase D の対象は長文ガイド 9 ページ（すべて `.html` → JSX 直接移植。**MDX・`.md` SSoT 方式は不採用**）:
- Claude: skill-guide-for-claude ✅ / skill-guide-of-claude-for-intermediate ✅ / claude-cowork-guide（進行中: s10〜s11 残り）
...
D-3 のファイル: `legacy/claude/claude-cowork-guide.html`（2307 行）
出力先: `web-next/app/claude/claude-cowork-guide/page.tsx`（scaffold + s01〜s07 実装済み）
- Mermaid 定数: DIAG_0 (graph LR) / DIAG_1 (sequenceDiagram) / DIAG_4 (graph TD) — s10 (advanced) で使用予定
- D-3 残りセクション行範囲:
  - s08 (skill-writing): lines 1676–1809（code-block 複数・compare）
  - s09 (best-practices): lines 1810–1906（li 10件）
  - s10 (advanced): lines 1907–2003（Mermaid diag-4）
  - s11 (sources): lines 2004–2159（source-grid・外部リンク多数）
- lint 既知違反: 7 件
