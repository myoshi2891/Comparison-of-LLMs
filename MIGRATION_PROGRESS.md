...
- **最新 HEAD**: `8d0e96b`（**feat(web-next): D-1 Green s11 (リソースファイルのバンドル) 移植完了**）
- **未コミット作業**: なし（working tree クリーン）
- **次の作業**: **D-1 Green s12 (`best`) 移植**（`legacy/claude/skill-guide-for-claude.html` 行 1616–1690）
...
| D | Long-form guides × 9（全ページ HTML → JSX 直接移植。MDX 不採用） | **着手中** — D-1 Green s11 完了 | Red: `af1e9e7` / s01–s11: 完了 |
...
次の作業: **D-1 Green s12 着手** — s11 完了（8d0e96b）。
...
D-1 `/claude/skill-guide` は s11 完了。次は Green s12（`best` セクション行 1616–1690）を実装する。
