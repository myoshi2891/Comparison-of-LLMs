...
- **最新 HEAD**: `198c269`（**chore(docs): Phase C 完了・Phase D 着手前同期**）
...
- 最新 HEAD: 198c269（chore(docs): Phase C 完了・Phase D 着手前同期。git status は clean）
...
次の作業: **Phase D D-1 着手（即着手可能）** — Phase C（C-1〜C-4）全 4 ページ完了済み（198c269）。
...
D-1 は `/claude/skill-guide`（`legacy/claude/skill-guide-for-claude.html` → `web-next/app/claude/skill-guide/page.tsx`）から着手する。
`.md` ファイルは移行対象外。対応する `.html` を faithful 移植するのみ。
