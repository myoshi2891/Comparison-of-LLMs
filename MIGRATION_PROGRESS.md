...
- **最新 HEAD**: `35392f2`（**feat(web-next): D-1 Green s04 (SKILL.md の基本構造) 移植完了**）
- **未コミット作業**: なし（working tree クリーン）
- **次の作業**: **D-1 Green s05 (`directory`) 移植**（`legacy/claude/skill-guide-for-claude.html` 行 1059–1141）
...
| D | Long-form guides × 9（全ページ HTML → JSX 直接移植。MDX 不採用） | **着手中** — D-1 Green s04 完了 | Red: `af1e9e7` / s01: `e16174a` / s02: `3de490e` / s03: `3f40a7d` / s04: `35392f2` |
...
次の作業: **D-1 Green s05 着手** — s04 完了（35392f2）。
...
D-1 `/claude/skill-guide` は s04 完了。次は Green s05（`directory` セクション行 1059–1141）を実装する。
