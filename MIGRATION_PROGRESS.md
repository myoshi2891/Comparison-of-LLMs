...
- **最新 HEAD**: `0b00841`（**feat(web-next): D-1 Green s10 (引数と動的コンテキストの注入) 移植完了**）
- **未コミット作業**: なし（working tree クリーン）
- **次の作業**: **D-1 Green s11 (`bundle`) 移植**（`legacy/claude/skill-guide-for-claude.html` 行 1584–1615）
...
| D | Long-form guides × 9（全ページ HTML → JSX 直接移植。MDX 不採用） | **着手中** — D-1 Green s10 完了 | Red: `af1e9e7` / s01–s10: 完了 |
...
次の作業: **D-1 Green s11 着手** — s10 完了（0b00841）。
...
D-1 `/claude/skill-guide` は s10 完了。次は Green s11（`bundle` セクション行 1584–1615）を実装する。
