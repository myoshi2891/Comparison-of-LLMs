...
- **最新 HEAD**: `b2b5b8e`（**feat(web-next): D-1 Green s05 (ディレクトリ構造と優先順位) 移植完了**）
- **未コミット作業**: なし（working tree クリーン）
- **次の作業**: **D-1 Green s06 (`invoke`) 移植**（`legacy/claude/skill-guide-for-claude.html` 行 1142–1212）
- **テスト数**: `bun run test` **473 passed / 474 total（1 failed = Red scaffold 継続）** — マージ前必須条件: `bun run build` / `bun run typecheck` / `bun run test`（全件 pass）/ `bun run lint`（新規違反ゼロ）/ `cd scraper && uv run pytest`（5/5）すべて成功していること
...
| D | Long-form guides × 9（全ページ HTML → JSX 直接移植。MDX 不採用） | **着手中** — D-1 Green s05 完了 | Red: `af1e9e7` / s01: `e16174a` / s02: `3de490e` / s03: `3f40a7d` / s04: `35392f2` / s05: `b2b5b8e` |
...
次の作業: **D-1 Green s06 着手** — s05 完了（b2b5b8e）。
...
D-1 `/claude/skill-guide` は s05 完了。次は Green s06（`invoke` セクション行 1142–1212）を実装する。
...
検証コマンド早見表:
  (cd web-next && bun run test)          # 全件 pass が期待値（現在 473 件 + 1 failed = Red scaffold 継続）
