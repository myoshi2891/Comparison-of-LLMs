...
- **最新 HEAD**: `56611e5`（**feat(web-next): D-1 Green s13 (デバッグと最適化) 移植完了**）
- **未コミット作業**: なし（working tree クリーン）
- **次の作業**: **D-1 Green s14 (`mistakes`) 移植**（`legacy/claude/skill-guide-for-claude.html` 行 1751–1827）
...
| D | Long-form guides × 9（全ページ HTML → JSX 直接移植。MDX 不採用） | **着手中** — D-1 Green s13 完了 | Red: `af1e9e7` / s01–s13: 完了 |
...
次の作業: **D-1 Green s14 着手** — s13 完了（56611e5）。
...
D-1 `/claude/skill-guide` は s13 完了。次は Green s14（`mistakes` セクション行 1751–1827）を実装する。
