...
MCPはAIエージェントに強力な実行権限を与える性質上、独自のセキュリティリスクを抱えています。2025年3月にセキュリティ企業Equixlyが実施した調査では、公開されているMCPサーバーの43%にコマンドインジェクションの欠陥があり、22%がパストラバーサルや任意ファイル読み取りを許し、30%がSSRF（サーバーサイドリクエストフォージェリ）で悪用可能で、しかもデフォルトで認証が存在しなかったと報告されています。2026年の別調査でも、MCPサーバーの40%がいまだ認証を必須としておらず、79%が認証情報を平文で扱っていたとされ、依然として本番運用に耐える設計ができていないサーバーが多いのが実情です。
...
- **OAuth 2.1とPKCEを必須にする**: 2025年6月18日の仕様改定でOAuth 2.1ベースの認可が正式に採用され、MCPサーバーは「OAuthリソースサーバー」として位置づけられ、認可機能は専用の認可サーバーが担う設計に整理されました。
...
- **SSRF対策としてURLの許可リストを設ける**: LLMが生成したパラメータをもとにURLを取得するツールでは、厳格な許可リストによる検証なしに任意のURLへアクセスさせないようにします。
- **シークレットをコードや設定に埋め込まない**: APIキーやOAuthトークンはコード・設定ファイル・環境変数に直接書かず、専用のシークレット管理サービス（例: HashiCorp Vault等）を利用します。
