...
「動いているように見える」ことと「実際に正しい」ことは別物です。Databricksや主要なLLMOpsプラットフォームが公開したAI利用動向レポートなどの調査によれば、多くの企業において新規RAGデプロイ時にサービス開始の初日から体系的な自動評価（RAGASやTruLensによる品質ゲート）をパイプラインに組み込んで運用する傾向が顕著になっています。これは、業界全体がRAGを「作って終わり」ではなく「継続的に測定するもの」として扱うようになってきたことを示しています。
