...
| 配置パス | スコープ | 主な用途 |
|---|---|---|
| `<workspace-root>/.agents/skills/<skill-folder>/` | ワークスペース(プロジェクト)固有 | チームのデプロイ手順、テスト規約など、そのリポジトリだけで使う知識 |
| `~/.gemini/antigravity/skills/<skill-folder>/` | グローバル(全プロジェクト共通) | 個人的によく使うユーティリティや汎用ツール |
...
**実務上のポイント**: Antigravity 単体では `name` は省略可能ですが、複数のツール間でスキルを使い回す(移植性を確保する)ことを前提にするなら、より厳格な Anthropic 側のルール(64文字以内・小文字とハイフンのみ・予約語禁止)に合わせて `name` を明示的に書いておくのが安全です。これにより、どちらの環境でも問題なく動作します。
...
まず、そのスキルが「特定のプロジェクトだけのもの」か「あらゆるプロジェクトで使う汎用的なもの」かを判断します。前者なら `.agents/skills/`、後者なら `~/.gemini/antigravity/skills/` に置きます。判断に迷ったら、最初はワークスペース側に置き、複数プロジェクトで使い回したくなった時点でグローバル側へ移動するのが安全です。
...
|---|---|---|
| `name` の必須性 | 必須(64文字以内、小文字+ハイフン、予約語禁止) | 任意(省略時はフォルダ名を使用) |
| `description` の必須性 | 必須(1,024文字以内) | 必須 |
| 主な配置場所 | `~/.claude/skills/`(個人用)、`.claude/skills/`(プロジェクト用) | `~/.gemini/antigravity/skills/`(グローバル)、`.agents/skills/`(ワークスペース) |
