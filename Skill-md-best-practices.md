...
`SKILL.md` の必須要件は `name` と `description` の2フィールドです。バリデーションルールは以下の通りです。

| フィールド | 要件 |
|---|---|
| `name` | 最大64文字、小文字英数字とハイフンのみ、XMLタグ不可、予約語（`anthropic`, `claude`）不可 |
| `description` | 空文字不可、最大1,024文字（Claude Code上の一覧表示では `when_to_use` と合算で1,536文字が上限）、XMLタグ不可 |

命名規則は「動名詞（-ing形）」が推奨されています。
