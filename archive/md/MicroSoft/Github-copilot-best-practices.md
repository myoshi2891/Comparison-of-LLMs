...
| 分析の深さ | レビュー単位で選択されたeffort level（Low / Medium）と、組織またはリポジトリの既定値に応じて分析深度が変わる。PRの複雑さだけで自動昇格するわけではない |
...
- **CVE-2025-53773**:リポジトリ内のソースコードに埋め込まれたインジェクションペイロードが、エージェントに任意のターミナルコマンドを実行させた脆弱性(CVSS v3.1 CNA評価 7.8、High)。特別な権限昇格を必要とせず、エージェントの通常の「コードを読む」挙動だけで発火した点が特徴。
