...
Microsoft Agent Framework のドキュメントでは `load_skill` ツールや `read_skill_resource` ツールという形で、この2段階目・3段階目の読み込みを明示的なツール呼び出しとして実装している例もある。呼称は違えど、**「まず名前と説明だけを全件読み込む」→「関連しそうなものだけ本文を読む」→「本当に必要になったファイルだけをさらに読む」**という3層構造は共通である。
