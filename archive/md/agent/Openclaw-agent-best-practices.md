...
- 向いている: コマンドラインに抵抗がなく、APIキーやトークン管理を自分でできる個人・小規模チームが、メール/カレンダー確認、リサーチ、コード作業の下請け、日次ブリーフィングなどを自動化するケース。
...
OpenClawのエージェントは、Markdownファイル群（ワークスペース）によって人格・振る舞い・知識が定義される「file-based agent runtime」である。ただし、すべてのファイルが毎回固定順序で読み込まれるわけではなく、セッション種別と設定に応じて注入対象が変わる。

```mermaid
flowchart TB
    MODE{"セッション種別と設定"}
    MODE -->|"初期セットアップ時のみ"| BOOT["BOOTSTRAP.md"]
    MODE -->|"通常セッション"| MAIN["SOUL.md / IDENTITY.md / USER.md / AGENTS.md"]
    MAIN -.->|"存在する場合のみ"| MEM["MEMORY.md（任意）"]
    MODE -->|"bootstrapMode"| BOOTMODE["設定されたブートストラップ対象"]
    MODE -->|"sub-agent"| SUB["サブエージェント向けの限定コンテキスト"]
...
`BOOTSTRAP.md`は初期セットアップ時だけ使い、`MEMORY.md`は存在する場合のみ注入する任意ファイルとして扱う。`TOOLS.md`は注入対象ではないため、ツール利用方針は`AGENTS.md`の`## Tools`配下に置く。さらに、`bootstrapMode`では設定されたブートストラップ対象が使われ、サブエージェントには通常セッションと同一の全ファイルではなく、その役割に必要な限定コンテキストが注入される。
...
9. **`openclaw security audit --deep`を定期実行するcronを組む**: 監査自体を自動化・定期化する。
