...
| `AGENTS.md` | 「何を・どう行うか」の手続き的ルール、ツールの利用方法に関する指針 | 中 | 複雑なワークフローを持つエージェントほど重要度が増す最大のファイルになりやすい。ツールの可否そのものはツールポリシーで制御する |
| `TOOLS.md` | 環境固有のツール注意事項と利用上の指針 | 中 | ツールの可否を制御するファイルではない。各スキルのSKILL.mdに書くべき汎用手順とも混同しない |
