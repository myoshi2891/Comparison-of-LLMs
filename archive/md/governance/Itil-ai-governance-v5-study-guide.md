...
3. **ソース** — 主張を裏付ける情報源の URL（巻末の参考資料一覧に一次情報源・二次資料をまとめて掲載）
...
flowchart LR
    ...
    N -.->|"一例: 自律性・複雑性が増す方向<br/>（必須の発展順序ではない）"| G -.-> A
```

上図の矢印は3タイプの関係性を示す一例であり、Narrow AI → Generative AI → Agentic AIという順序で必ず発展・導入しなければならないという意味ではありません。実際には目的に応じてどのタイプを組み合わせて使うかが決まります。
