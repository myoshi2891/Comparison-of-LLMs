...
3. **厳格な TDD（Red → Green → Refactor → Docs）とコミット分割**:
   ...
   - コミット前には必ず `git diff --cached` でローカル環境の絶対パス（`/Users/johndoe/...` 等）や PII が含まれていないことを検証してください（`.claude/rules/no-absolute-paths.md`）。**新規ファイルは検証前に必ず `git add` でステージングする**か、`git ls-files --others --exclude-standard` で未追跡ファイルの一覧を取得し、同様に検証してください（`git diff --cached` は未ステージングの新規ファイルの内容を含まないため、ステージングし忘れた新規ファイルにこのチェックが効きません）。
