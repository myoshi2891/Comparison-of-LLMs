...
### Phase C — Provider `agent.html` × 4

**目的**: Phase B で確立した `DocLayout` を再利用して 4 枚を高速移植。

**順序**:
1. `/claude/agent`
2. `/gemini/agent`（Mermaid 含有の可能性あり → `MermaidDiagram` を先行導入）
3. `/codex/agent`
4. `/copilot/agent`
...
**MDX 採用判断**:
- `.md` と `.html` の両方が存在するページ: `legacy/claude/skill-guide-for-claude.md`, `legacy/gemini/skill-guide-for-gemini.md`, `legacy/claude/skill-guide-deep-dive.md`
- これらは **`.md` を SSoT 化**し、`md-to-nextjs-migration` スキルで `page.tsx` を生成
- `.md` が無い長文 HTML は Phase B テンプレで素直に JSX 化
...
4. **`.md` / `.html` 二重管理の解消**: Phase D で `.md` が SSoT になるページは `.html` 版を即削除してドリフトを防ぐ
...
| Phase | 工数 | 累積 | 備考 |
|---|---|---|---|
| A | 1 日 | 1 日 | Red は本 PR で着手 |
| B | 2 日 | 3 日 | テンプレ確立 |
| C | 1.5 日 | 4.5 日 | |
| D | 3 日 | 7.5 日 | MDX 移行含む |
