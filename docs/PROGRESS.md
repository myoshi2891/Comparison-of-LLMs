...
## 最近の追加内容
- **仕様駆動開発（SDD）実践ガイド ― 中級・上級エンジニア向けベストプラクティス（/sdd/spec-driven-development-guide）の Next.js 移行**: `Spec-driven-development-guide.html` を `web-next/app/sdd/spec-driven-development-guide/page.tsx` に Pure JSX として完全忠実移植 🚀。要約・省略なしで全16セクション・全表・全コードブロック・10 Mermaid図・TOCスクロール追従（`TocObserver.tsx`）・外部リンク安全属性・`page-registry.ts`（`開発プロセス` グループ）登録を完了。原本 `Spec-driven-development-guide.html` は `archive/html/SDD/Spec-driven-development-guide.html` へ `git mv` 退避保存。契約テスト5件を追加。
