...
- **テストの実行状況**:
  - **フロントエンド (`web-next/`)**: Vitest 実行で **553 件すべて合格** (全 Green ✅)
...
### テスト分野別のカバレッジ概要 (2026-05-20 時点)
- **Unit**:
  - `app/` (全 18 ガイドページルート): ✅ 100% 契約テスト（タイトル、セクション数、rel、metadata）
...
- 現在のステータス: docs/PROGRESS.md を参照。テストは Vitest (553/553 passed) / pytest (5/5 passed) で全 Green
