...
- **テストの実行状況**:
  - **フロントエンド (`web-next/`)**: Vitest 実行で **864 件すべて合格** (全 Green ✅)
...
- リポジトリ: LLM-Studies (Next.js 移行プロジェクトは dev/main へ完全マージ済み)
  - 現在のステータス: docs/PROGRESS.md を参照。テストは Vitest (864/864 passed) / pytest (38/38 passed) で全 Green
