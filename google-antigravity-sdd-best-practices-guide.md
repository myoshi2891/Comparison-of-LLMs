...
> **注記**: リポジトリ内に `GEMINI.md` と `Gemini.md` が混在する場合、大文字小文字を区別する環境で競合します。両者は同じリソースを指しますが、本ガイドでは大文字の `GEMINI.md` を正規のファイル名として推奨・統一しています。
...
> **注記**: 前述の通り、ファイル名は `GEMINI.md` を標準とします。
