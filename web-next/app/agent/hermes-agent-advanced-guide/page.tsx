    ...
    <div className={styles.layout}>
      ...
      <nav className={styles.sidebar} id="sidebar">
        ...
        <div
          ...
          style={{ "--group-color": "var(--c-overview)" } as React.CSSProperties}
        >
        ...
        <div
          className={styles.navGroup}
          style={{ "--group-color": "var(--c-data)" } as React.CSSProperties}
        >
          <div className={styles.navGroupLabel}>環境 &amp; メモリ設計</div>
          <a
            className={styles.navLink}
            href="#3-セットアップとプロファイル運用のベストプラクティス"
          >
            3. セットアップとプロファイル運用
          </a>
        ...
        <div
          className={styles.navGroup}
          style={{ "--group-color": "var(--c-skills)" } as React.CSSProperties}
        >
        ...
        <div
          className={styles.navGroup}
          style={{ "--group-color": "var(--c-exec)" } as React.CSSProperties}
        >
        ...
        <div
          className={styles.navGroup}
          style={{ "--group-color": "var(--c-ops)" } as React.CSSProperties}
        >
      ...
      <div className={styles.main}>
        <div className={styles.hero}>
          ...
          <h1>
            Hermes Agent ベストプラクティスガイド <br />― 中級者から上級者向け
          </h1>
          <p className={styles.subtitle}>
            Nous Research製オープンソース自己改善型AIエージェント「Hermes
            Agent」を、実運用レベルで使いこなすためのステップバイステップ解説。対象読者は、CLI／エージェント系ツールの利用経験があり、メモリ設計・スキル運用・マルチエージェント委任・本番セキュリティまで踏み込みたい中級〜上級エンジニア。情報基準日:
            2026年8月2日(Hermes Agent公式ドキュメント/GitHubリポジトリの内容に基づく)。
          </p>
        </div>
        ...
        <div className={styles.content}>
          ...
          <p>
            Hermes Agentは、AI基盤研究企業Nous
            Research（Hermesモデルシリーズ、分散学習フレームワークDisTrO、分散学習ネットワークPsycheなどで知られる）が開発したMITライセンスのオープンソース自律AIエージェントである。GitHubリポジトリ
            <code>NousResearch/hermes-agent</code>
            は2026年8月時点で20万スターを超える規模に成長しており、公式には「The agent that grows
            with you（あなたと共に成長するエージェント）」と位置づけられている。
          </p>
          <p>
            Hermes
            Agentが他のコーディング特化エージェント（IDE常駐型のコパイロット）や単発のチャットボットラッパーと一線を画す最大の特徴は、
            <strong>組み込みの学習ループ</strong>を持つ点にある。
          </p>
          <ul>
            <li>
          ...
          <p>
            ローカルのlocalバックエンドはもちろん、Docker・SSH・Singularity・Daytona・Modal・Vercel
            Sandboxなど6種類のターミナルバックエンドで動作し、$5程度のVPSからGPUクラスタ、アイドル時にほぼ課金されないサーバーレス基盤まで、実行環境を選ばない。Telegram・Discord・Slack・WhatsApp・Signal・Matrix・Mattermost・Email・SMSなど20以上のメッセージングプラットフォームに単一のゲートウェイプロセスから同時接続できる。
          </p>

          <h3 id="11-業界での位置づけ">1.1 業界での位置づけ</h3>
          <p>
            2026年前半、Peter
            Steinberger氏が公開したエージェント「OpenClaw」が爆発的に普及したことを受け、その数週間後にNous
            ResearchがHermesを競合として投入した経緯がある。両者は設計思想が異なり、OpenClawが多チャンネル対応の中央ゲートウェイ型アーキテクチャを志向するのに対し、Hermesは自己改善ループ（スキル自動生成・メモリ蓄積）に重きを置く。2026年5月時点でOpenRouter上の1日あたりトークン生成量でHermesがOpenClawを上回り首位に立ったと報じられている。同年7月にはNous
            ResearchがRobot
            Ventures主導・USV参加のラウンドで評価額15億ドル規模の資金調達を進めていると報じられた。
          </p>
          <p>
            OpenClawからの移行者向けに<code>hermes claw migrate</code>
            コマンドが用意されており、SOUL.md相当のペルソナファイル、メモリ、スキル、コマンド許可リスト、メッセージング設定、APIキーなどを自動的に取り込める。
          </p>
          ...
          <p>
            Hermes
            Agentのソースツリーは大きく分けて次の要素で構成される（実装を読む・パッチを当てる際の道しるべとして把握しておくとよい）。
          </p>
          <ul>
            <li>
              <code>run_agent.py</code> ― <code>AIAgent</code>
              本体。プロバイダ選択・プロンプト構築・ツール実行・リトライ・フォールバック・圧縮・永続化を司る会話ループ
            </li>
            <li>
              <code>cli.py</code> ― <code>HermesCLI</code>、対話型ターミナルUI
            </li>
            <li>
              <code>agent/</code> ― プロンプト組み立て（<code>prompt_builder.py</code>
              ）、コンテキスト圧縮（<code>context_compressor.py</code>
              ）、Anthropicプロンプトキャッシュ（<code>prompt_caching.py</code>）など
            </li>
            <li>
              <code>hermes_cli/</code> ― サブコマンド群、認証・プロバイダ解決、プラグインマネージャ
            </li>
            <li>
              <code>tools/</code> ― ツール実装。1ファイル1ツールが原則で、インポート時に
              <code>registry.register()</code>を呼んで自己登録する
            </li>
            <li>
              <code>gateway/</code> ―
              メッセージングゲートウェイ本体。20種のプラットフォームアダプタを収録
            </li>
            <li>
              <code>cron/</code> ― スケジューラ
            </li>
            <li>
              <code>skills/</code> と <code>optional-skills/</code> ―
              バンドル済みスキルと任意インストール可能な公式スキル
            </li>
            <li>
            ...
            <table>
              ...
              <tbody>
                <tr>
                  <td>プロンプトの安定性</td>
                  <td>
                    システムプロンプトはセッション中に変化しない。<code>/model</code>
                    など明示的な操作を除き、キャッシュを破壊する変更は起きない
                  </td>
                </tr>
                <tr>
                  <td>実行の可観測性</td>
                ...
                <tr>
                  ...
                  <td>
                    <code>AIAgent</code>
                    は1クラスでCLI・ゲートウェイ・ACP・バッチ・APIサーバーを兼ねる。差異はエントリーポイント側に閉じ込める
                  </td>
                </tr>
                <tr>
                  <td>疎結合</td>
                  <td>
                    MCP・プラグイン・メモリプロバイダ・RL環境などのオプション機構はレジストリパターンと
                    <code>check_fn</code>ゲーティングで実現し、ハード依存にしない
                  </td>
                </tr>
                <tr>
                  <td>プロファイル分離</td>
                  <td>
                    <code>hermes -p &lt;name&gt;</code>
                    でプロファイルごとにHERMES_HOME・設定・メモリ・セッション・ゲートウェイPIDが独立し、複数プロファイルを同時実行できる
                  </td>
                </tr>
              </tbody>
            </table>
          ...
          <p>
            ...
            個人用と業務用、あるいは検証用と本番用のエージェントを分けたい場合は、複数の
            <code>.hermes</code>ディレクトリを作るのではなく
            <code>hermes profile create &lt;name&gt;</code>
            でプロファイルを切ることを推奨する。プロファイルごとにスキル・メモリ・cronジョブが完全に独立するため、実験的な設定変更が本番プロファイルに波及しない。
          </p>
          ...
          <div className={styles.codeBlock}>
            <div className={styles.codeLine}>
              ...
              <span className={styles.ck}>bash</span>
            </div>
          </div>
          <p>
          ...
          <div className={styles.codeBlock}>
            <div className={styles.codeLine}>
              <span className={styles.ck}>iex</span> (irm
              https://hermes-agent.nousresearch.com/install.ps1)
            </div>
          </div>
          <p>
            インストーラはuv・Python 3.11・Node.js・ripgrep・ffmpeg・ポータブルGit
            Bash（MinGit）まで自動導入する。既存のGitがあればそれを優先利用し、システムのGit環境には干渉しない。
          </p>

          <h3 id="32-モデルプロバイダの選定">3.2 モデルプロバイダの選定</h3>
          <p>
            Nous
            Portal・OpenRouter・OpenAI・Anthropic・Google・その他OpenAI互換エンドポイントまで対応する。プロバイダごとに個別のAPIキーを管理したくない場合は、次の1コマンドでNous
            Portal経由のOAuthとTool
            Gateway（Web検索・画像生成・TTS・クラウドブラウザ）を一括有効化できる。
          </p>
          <div className={styles.codeBlock}>
            <div className={styles.codeLine}>
          ...
          <p>
            モデルの切り替えはセッション中いつでも<code>/model</code>
            で可能だが、後述するプロンプトキャッシュの観点から、切り替え頻度は最小限にとどめるのがベストプラクティスである。
          </p>

          <h3 id="33-プロファイル分離とバックアップ">3.3 プロファイル分離とバックアップ</h3>
          <ul>
            <li>
              検証用スキルをバンドルなしのクリーンな状態で試したい場合は
              <code>hermes profile create research --no-skills</code>で空のプロファイルを作る
            </li>
            <li>
              既存プロファイルをバンドルスキルなしに切り替えたい場合は
              <code>hermes skills opt-out</code>（<code>--remove</code>
              で未編集のバンドルスキルも削除）
            </li>
            <li>
              定期的に<code>hermes update</code>でセキュリティパッチを取り込む
            </li>
          </ul>

          <hr />

          <h2 id="4-メモリシステム設計のベストプラクティス">
            4. メモリシステム設計のベストプラクティス
          </h2>
          <h3 id="41-2つのメモリストア">4.1 2つのメモリストア</h3>
          <p>
            Hermesのメモリは意図的に<strong>厳格な文字数上限</strong>を持つ2ファイル構成である。
          </p>
          <div className={styles.tableScroll}>
            <table>
              <thead>
          ...
          <p>
            両ファイルは<code>~/.hermes/memories/</code>に保存され、セッション開始時に
            <strong>フローズンスナップショット</strong>
            としてシステムプロンプトへ注入される。この設計はプロンプトキャッシュを維持するための意図的なトレードオフであり、セッション中にエージェントがメモリを追加・削除しても、その変更はディスクには即時反映される一方、システムプロンプト上の表示は次回セッション開始まで更新されない。
          </p>

          <h3 id="42-何を保存し何を保存しないか">4.2 何を保存し、何を保存しないか</h3>
          <p>
            <strong>保存すべきもの（エージェントが自動的に行う）</strong>
          </p>
          <ul>
            <li>
              ユーザーの技術的な好み（「TypeScriptを使う」など）→ <code>user</code>
            </li>
            <li>
              環境事実（OS・ミドルウェアのバージョンなど）→ <code>memory</code>
            </li>
            <li>
              訂正（「Dockerコマンドにsudoは不要、ユーザーはdockerグループに所属」など）→{" "}
              <code>memory</code>
            </li>
            <li>
              プロジェクト規約（インデント幅・命名規則など）→ <code>memory</code>
            </li>
            <li>
              完了した作業の記録 → <code>memory</code>
            </li>
          </ul>
          <p>
            <strong>保存すべきでないもの</strong>
          </p>
          <ul>
          ...
          <p>
            メモリは自動圧縮されない。上限を超える書き込みはエラーとして拒否され、エージェントは既存エントリを
            <code>replace</code>で統合するか<code>remove</code>で削除してから再試行する必要がある。
            <strong>容量が80%を超えた時点で統合する</strong>
            運用がベストプラクティスとして明示されている。良いエントリの例は次のように情報密度が高く簡潔である。
          </p>
          <div className={styles.codeBlock}>
            <div className={styles.codeLine}>
          ...
          <p>
            有効化すると、対話的CLIではその場でインラインに承認を求められ、メッセージングプラットフォームやバックグラウンドの自己改善レビューからの書き込みは
            <code>/memory pending</code>・<code>/skills pending</code>でステージングされ、
            <code>approve</code>/<code>reject</code>
            で個別に判断できる。「エージェントが自分について誤った前提を保存してしまった」という問題への直接的な解決策である。
          </p>
            ...
            <table>
              <thead>
                <tr>
                  ...
                  <th>永続メモリ</th>
                  <th>
                    セッション検索（<code>session_search</code>）
                  </th>
                </tr>
              </thead>
              <tbody>
          ...
          <p>
            ...
            「常にコンテキストにあるべき事実」はメモリへ、「特定の過去の会話を思い出したい」場合はセッション検索へ、と役割を明確に分離する。
          </p>

          <h3 id="46-外部メモリプロバイダ">4.6 外部メモリプロバイダ</h3>
          <p>
            MEMORY.md/USER.mdを超える深いメモリが必要な場合、Honcho・OpenViking・Mem0・Hindsight・Holographic・RetainDB・ByteRover・Supermemoryの8種の外部メモリプロバイダプラグインが用意されている。これらは組み込みメモリを置き換えるのではなく
            <strong>並行して動作</strong>
            し、知識グラフ・意味検索・自動事実抽出・セッション横断のユーザーモデリングを追加する。
            <code>hermes memory setup</code>対話的に選択できる。
          </p>
          ...
          <p>
            スキルは<Ext href="https://agentskills.io">agentskills.io</Ext>
            のオープン標準に準拠した、オンデマンドで読み込まれる知識ドキュメントである。トークン効率を保つため次の3段階で開示される。
          </p>
          <div className={styles.mermaidWrap}>
            <MermaidDiagram chart={DIAGRAM_5_1} />
          </div>
          <p>
            エージェントは実際に必要になった時だけ完全な内容をロードする。すべてのスキルは
            <code>~/.hermes/skills/</code>を単一の情報源として保持され、<code>external_dirs</code>
            設定で共有ディレクトリ（複数ツール共通の<code>~/.agents/skills/</code>
            など）を追加でスキャンさせることもできる。
          </p>
          ...
          <div className={styles.codeBlock}>
            ...
            <div className={styles.codeLine}>
              <span className={styles.ck}>name:</span> <span className={styles.cs}>my-skill</span>
            </div>
            <div className={styles.codeLine}>
              <span className={styles.ck}>description:</span>{" "}
              <span className={styles.cs}>このスキルが何をするかの簡潔な説明</span>
            </div>
            <div className={styles.codeLine}>
              <span className={styles.ck}>version:</span> <span className={styles.cs}>1.0.0</span>
            </div>
            <div className={styles.codeLine}>
              <span className={styles.ck}>platforms:</span> [
              <span className={styles.cs}>macos</span>, <span className={styles.cs}>linux</span>]
            </div>
            <div className={styles.codeLine}>
          ...
          <p>
            <code>fallback_for_toolsets</code>は特に有用なパターンで、有料ツール（例:
            Web検索）が利用不可の時にだけ無料の代替スキル（DuckDuckGo検索など）を表示させる、というフォールバック設計を宣言的に書ける。
          </p>

          <h3 id="53-learnでスキルを素早く作る">
            5.3 <code>/learn</code>でスキルを素早く作る
          </h3>
          <p>
            手作業でSKILL.mdを書く代わりに、<code>/learn</code>
            はエージェントに既存の知識・資料からスキルを自動生成させるコマンドである。
          </p>
          <div className={styles.codeBlock}>
            <div className={styles.codeLine}>
          ...
          <p>
            以後<code>/backend-dev 認証ミドルウェアをリファクタリングして</code>
            と打つだけで、3つのスキルが同時にロードされる。バンドルはスキル名の衝突時に個別スキルより優先される点に注意する。
          </p>
          ...
          <p>
            エージェントは複雑なタスク（5回以上のツール呼び出し）を完了した後、エラーから抜け出す方法を発見した後、あるいはユーザーに訂正された後に、自発的にスキルを作成する。
            <code>create</code> / <code>patch</code>（推奨・差分のみで効率的）/ <code>edit</code> /{" "}
            <code>delete</code> / <code>write_file</code> / <code>remove_file</code>
            の6アクションがある。
          </p>

          <h3 id="56-skills-hubとトラストレベル">5.6 Skills Hubとトラスト・レベル</h3>
          <p>
            外部のスキルエコシステムとして、公式オプションスキル・skills.sh（Vercel運営）・well-known
            エンドポイント（<code>/.well-known/skills/index.json</code>
            ）・GitHubタップ（openai/skills、anthropics/skills、huggingface/skills、NVIDIA/skillsなど）・ClawHub・LobeHub・browse.sh（Browserbase運営の200以上のサイト別ブラウザ自動化スキル集）が統合されている。
          </p>
          <div className={styles.tableScroll}>
            <table>
              <thead>
              ...
              <tbody>
                ...
                <tr>
                  ...
                  <td>
                    リポジトリの<code>optional-skills/</code>
                  </td>
                  <td>組み込み信頼、警告表示なし</td>
                </tr>
                <tr>
                ...
                <tr>
                  ...
                  <td>
                    危険でない指摘は<code>--force</code>で上書き可能。<code>dangerous</code>
                    判定は上書き不可
                  </td>
                </tr>
              </tbody>
          ...
          <p>
            すべてのハブ経由インストールは、データ漏洩・プロンプトインジェクション・破壊的コマンド・サプライチェーン兆候を検査するセキュリティスキャナを通過する。
            <strong>
              未検証のコミュニティスキルを本番プロファイルにインストールする前には、必ず
              <code>hermes skills inspect</code>でプレビューする
            </strong>
            のがベストプラクティスである。
          </p>
          ...
          <p>
            Curatorが管理するのは「<strong>エージェント作成</strong>
            」と明示的にマークされたスキルのみである。具体的には、バックグラウンドの自己改善レビュー（約10ターンごとに実行）が生成したアンブレラスキルがこれに該当し、ユーザーが手動作成したスキルや、フォアグラウンド会話中にエージェントへ依頼して作らせたスキルは対象外となる。バンドル済み・ハブインストール済みスキルは原則対象外だが、
            <code>curator.prune_builtins: true</code>
            （デフォルト）の場合のみ、未使用のバンドルスキルが90日後にアーカイブされる（削除ではなく退避）。
          </p>

          <h3 id="62-実行タイミングと安全策">6.2 実行タイミングと安全策</h3>
          <p>
            Curatorはcronデーモンではなく、<strong>非アクティブ検知</strong>
            によって起動する。CLIセッション開始時、およびゲートウェイのcronティッカー内で、前回実行から
            <code>interval_hours</code>（デフォルト7日）が経過し、かつ<code>min_idle_hours</code>
            （デフォルト2時間）以上アイドルであった場合にのみバックグラウンドフォークとして走る。新規インストール直後は最初の実行が1インターバル分先送りされ、ユーザーがスキルライブラリを確認したりピン留めしたりする猶予が確保される。
          </p>
          <p>実行は2フェーズに分かれる。</p>
          <ol>
            <li>
              <strong>自動遷移</strong>（決定的・LLM不使用）: 30日/90日ルールによる
              <code>active → stale → archived</code>の遷移
            </li>
            <li>
              <strong>LLM統合レビュー</strong>（デフォルトOFF、
              <code>curator.consolidate: true</code>で有効化）:
              補助モデルが重複スキルの統合・パッチ・アーカイブを提案する
            </li>
          </ol>

          <h3 id="63-バックアップとロールバック">6.3 バックアップとロールバック</h3>
          <p>
            実際のCurator実行の前には必ず<code>~/.hermes/skills/.curator_backups/</code>
            にtar.gzスナップショットが作成される。想定外の変更があった場合は次の1コマンドで即座に復元できる。
          </p>
          <div className={styles.codeBlock}>
            <div className={styles.codeLine}>
              <span className={styles.ck}>hermes</span> curator rollback{" "}
              <span className={styles.cc}># 最新スナップショットへ復元（確認あり）</span>
            </div>
            <div className={styles.codeLine}>
              <span className={styles.ck}>hermes</span> curator rollback --list{" "}
              <span className={styles.cc}># スナップショット一覧</span>
            </div>
          </div>
          <p>
            ロールバック自体も「ロールバック前」のスナップショットを取ってから実行されるため、誤ロールバックすら取り消せる。
            <strong>
              本番運用では<code>curator.backup.enabled</code>を明示的にtrueのままにしておくこと
            </strong>
            、そして<code>/plan</code>
            のようなスラッシュコマンドが依存する保護対象ビルトインスキルは、
            <code>prune_builtins</code>
            設定に関わらずCuratorの候補リストから常に除外される点も押さえておきたい。
          </p>
          ...
          <p>
            プロジェクトコンテキストファイルは<strong>1セッションにつき1種類のみ</strong>
            ロードされる。優先順位は次の通りで、最初に見つかったものが採用される。
          </p>
          <p>
            <code>.hermes.md</code>（<code>HERMES.md</code>も可）→ <code>AGENTS.md</code> →{" "}
            <code>CLAUDE.md</code> → <code>.cursorrules</code>
          </p>
          <p>
            一方、<code>SOUL.md</code>
            は常に独立してロードされる（Hermesインスタンス全体のペルソナ層）。
            <code>HERMES_HOME</code>直下からのみ読み込まれ、作業ディレクトリは探索しない。
          </p>

          <h3 id="72-段階的サブディレクトリ発見">7.2 段階的サブディレクトリ発見</h3>
          <p>
            モノレポで各サブディレクトリに個別の規約がある場合、起動時にすべてを読み込む必要はない。エージェントが
            <code>read_file</code>や<code>terminal</code>でそのディレクトリに触れた瞬間に、該当する
            <code>AGENTS.md</code>が遅延発見されてツール結果に注入される仕組みになっている。
          </p>
          <div className={styles.codeBlock}>
          ...
          <ol>
            <li>
              <strong>簡潔に保つ</strong> — 設定した<code>context_file_max_chars</code>
              （デフォルト2万文字）以内に収める。毎ターン読み込まれるコストを意識する
            </li>
            <li>
              <strong>見出しで構造化する</strong> — アーキテクチャ・規約・注意事項を<code>##</code>
              セクションで分ける
            </li>
            <li>
              <strong>具体例を含める</strong> — 望ましいコードパターン・API形状・命名規則を示す
            </li>
            <li>
              <strong>やってはいけないことを明記する</strong> —
              「マイグレーションファイルを直接編集しない」など
            </li>
            <li>
              <strong>重要なパス・ポート番号を列挙する</strong> —
              エージェントがターミナルコマンドで使う
            </li>
          </ol>
          ...
          <p>
            すべてのコンテキストファイルは読み込み前にスキャンされ、「以前の指示を無視しろ」といった指示上書き試行、非表示のHTMLコメント、認証情報の窃取パターン（
            <code>curl ... $API_KEY</code>
            など）、不可視Unicode文字（ゼロ幅スペース、双方向オーバーライド）が検出されるとブロックされる。
            <strong>
              共有リポジトリ内のAGENTS.mdは、この自動スキャンだけに頼らず、自分でも内容を確認する
            </strong>
            ことが推奨されている。
          </p>
          ...
          <p>
            ...
            が生成する子エージェントは完全にフレッシュな会話から始まる。親の会話履歴・過去のツール呼び出し・議論内容について一切の知識を持たない。したがって、
            <code>goal</code>と<code>context</code>パラメータに<strong>子が必要とするすべて</strong>
            を明示的に渡す必要がある。
          </p>
          <div className={styles.codeBlock}>
            <div className={styles.codeLine}>
              <span className={styles.cc}># 悪い例：「そのエラー」が何か子には分からない</span>
            </div>
            <div className={styles.codeLine}>
              delegate_task(goal=<span className={styles.cs}>&quot;そのエラーを直して&quot;</span>)
            </div>
            <div className={styles.codeLine}></div>
            <div className={styles.codeLine}>
              <span className={styles.cc}># 良い例：子が必要とする情報を全て含む</span>
            </div>
          ...
          <div className={styles.codeBlock}>
            <div className={styles.codeLine}>
              <span className={styles.cc}># 単発</span>
            </div>
            <div className={styles.codeLine}>
              delegate_task(goal=
              <span className={styles.cs}>&quot;テスト失敗の原因を調査&quot;</span>, context=
              <span className={styles.cs}>&quot;...&quot;</span>)
            </div>
            <div className={styles.codeLine}></div>
            <div className={styles.codeLine}>
              <span className={styles.cc}># 並列バッチ（デフォルト最大3並列、設定変更可）</span>
            </div>
          ...
          <p>
            デフォルトでは委任は<strong>フラット</strong>
            （親→子の1階層のみ、子はさらに委任できない）。多段階ワークフロー（調査→統合など）が必要な場合のみ、
            <code>role="orchestrator"</code>の子を作る。
          </p>
          <div className={styles.mermaidWrap}>
            <MermaidDiagram chart={DIAGRAM_8_1} />
          </div>
          <p>
            <code>max_spawn_depth: 3</code>かつ<code>max_concurrent_children: 3</code>
            を組み合わせると理論上3×3×3=27の並列leafエージェントが生まれうる点はコスト上の注意点として明記されている。深さを上げる際は意図的に行うべきである。
          </p>
          ...
          <p>
            <strong>判断基準</strong>: サブタスクが推論・判断・多段階の問題解決を要するなら
            <code>delegate_task</code>、機械的なデータ処理やスクリプト化されたワークフローなら
            <code>execute_code</code>を選ぶ。
          </p>

          <h3 id="85-恒久実行が必要な場合">8.5 恒久実行が必要な場合</h3>
          <p>
            セッションが閉じられても・プロセスが再起動してもタスクを継続させたい場合、
            <code>delegate_task</code>は不向きである（プロセス再起動後の子は<code>unknown</code>
            扱いになり再開されない）。代わりに以下を使う。
          </p>
          <ul>
            <li>
              <code>cronjob(action="create")</code> ―
              独立したエージェント実行としてスケジュールし、親ターンの中断の影響を受けない
            </li>
            <li>
              <code>terminal(background=True, notify_on_complete=True)</code> ―
              長時間シェルコマンドをバックグラウンドで継続
            </li>
          </ul>
          ...
          <p>
            <code>execute_code</code>
            は、Hermesツールをプログラム的に呼び出すPythonスクリプトをエージェントに書かせ、多段階ワークフローを1回のLLMターンに圧縮する仕組みである。スクリプトは子プロセスとして動作し、Unixドメインソケット経由のRPCでツールを呼び出す。
            <strong>
              中間結果はコンテキストウィンドウに一切入らず、<code>print()</code>
              の出力のみがLLMに返る
            </strong>
            点が最大の利点である。
          </p>
          <div className={styles.codeBlock}>
            <div className={styles.codeLine}>
              <span className={styles.ck}>from</span> hermes_tools{" "}
              <span className={styles.ck}>import</span> web_search, web_extract
            </div>
            <div className={styles.codeLine}>
            ...
            <div className={styles.codeLine}>
              results = web_search(
              <span className={styles.cs}>&quot;Rust async runtime 比較 2025&quot;</span>, limit=5)
            </div>
            <div className={styles.codeLine}>summaries = []</div>
            <div className={styles.codeLine}>
              <span className={styles.ck}>for</span> r <span className={styles.ck}>in</span>{" "}
              results[<span className={styles.cs}>&quot;data&quot;</span>][
              <span className={styles.cs}>&quot;web&quot;</span>]:
            </div>
            <div className={styles.codeLine}>
          ...
          <p>
            <code>code_execution.mode</code>は<code>project</code>
            （デフォルト、アクティブな仮想環境のPythonを使用し作業ディレクトリもセッションと同じ）と
            <code>strict</code>
            （Hermes自身のPythonを使い、隔離された一時ディレクトリで実行）の2種類がある。再現性を最優先するCI的な用途では
            <code>strict</code>、通常の開発作業では<code>project</code>が適する。
          </p>
          <div className={styles.tableScroll}>
            <table>
              <thead>
          ...
          <p>
            セキュリティ面では、子プロセスは<code>KEY</code> / <code>TOKEN</code> /{" "}
            <code>SECRET</code> / <code>PASSWORD</code> / <code>CREDENTIAL</code> /{" "}
            <code>AUTH</code>を含む環境変数名をすべて除去した最小権限環境で動く。スキルが
            <code>required_environment_variables</code>
            を宣言していれば、そのスキルロード後は該当変数のみ自動的に通過する。この仕組みにより「任意コード実行だが秘密情報は漏れない」というバランスを取っている。
          </p>
          <p>
            <strong>プラットフォーム制限</strong>: Unixドメインソケットに依存するため
            <strong>Linux/macOSのみ</strong>
            対応。Windowsでは自動的に無効化され、通常の逐次ツール呼び出しにフォールバックする。
          </p>
          ...
          <p>
            <code>/goal</code>
            は、ターンをまたいで生き続ける目標をエージェントに与える機能である。毎ターン後、補助モデル（judge）がゴールが達成されたかを判定し、未達なら継続プロンプトを自動投入して作業を続けさせる。これはOpenAIのCodex
            CLI 0.128.0（Eric Traut氏実装）が普及させた「Ralph
            loop」パターンへの、Hermes独自実装によるオマージュである。「ゴールが達成されるまで止まらない」という中核アイデアはCodex由来であることが公式ドキュメントに明記されている。
          </p>

          <h3 id="102-使うべき場面">10.2 使うべき場面</h3>
          <p>
            「3回も『続けて』と言うことになりそうなタスク」がまさに<code>/goal</code>
            の適用対象である。
          </p>
          <ul>
            <li>
              <code>src/</code>内のすべてのlintエラーを修正し<code>ruff check</code>を通す
            </li>
          ...
          <p>
            judgeは意図的に保守的に設計されており、「応答が明示的にゴール完了を確認している」「成果物が明確に生成されている」「ゴールが達成不能・ブロックされている」場合のみ
            <code>done</code>と判定する。judge自体がエラーを返した場合は<code>continue</code>
            扱いとなり（フェイルオープン）、暴走を防ぐ最終防波堤はターン予算（デフォルト20、
            <code>goals.max_turns</code>で変更可）である。
          </p>

          <h3 id="104-実務上の注意点">10.4 実務上の注意点</h3>
          <ul>
            <li>
              ゴール実行中にユーザーが実際にメッセージを送ると、それは継続ループより
              <strong>常に優先</strong>される
            </li>
            <li>
              ゴール実行中に<strong>新しい</strong>ゴールを設定しようとすると拒否される。先に
              <code>/stop</code>してから再設定する
            </li>
            <li>
              継続プロンプトは通常のuser
              roleメッセージとして追記されるだけで、システムプロンプトを変更しないため、20ターンのゴール実行は20ターンの通常会話と
              <strong>同じキャッシュコスト</strong>で済む
            </li>
            <li>
              judgeの誤判定（早すぎる<code>done</code>、または<code>continue</code>
              し続ける）はどちらも起こりうる。前者は追加メッセージで継続させ、後者はターン予算が保険になる
            </li>
          </ul>
          ...
          <p>
            Cronジョブは<strong>完全にフレッシュなセッション</strong>
            で実行される。スキルが提供しない情報はすべてプロンプト自体に含める必要がある。
          </p>
          <div className={styles.codeBlock}>
          ...
          <h3 id="113-ジョブの連鎖context_from">
            11.3 ジョブの連鎖（<code>context_from</code>）
          </h3>
          <p>
            複数ジョブをパイプライン化したい場合、<code>context_from</code>
            で前段ジョブの最新出力を後段ジョブのプロンプトへ自動的に前置できる。
          </p>
          <div className={styles.mermaidWrap}>
          ...
          <div className={styles.codeBlock}>
            <div className={styles.codeLine}>
              cronjob(action=<span className={styles.cs}>&quot;create&quot;</span>, prompt=
              <span className={styles.cs}>&quot;...&quot;</span>, schedule=
              <span className={styles.cs}>&quot;0 7 * * *&quot;</span>, name=
              <span className={styles.cs}>&quot;AI News Collector&quot;</span>)
            </div>
            <div className={styles.codeLine}>
              cronjob(action=<span className={styles.cs}>&quot;create&quot;</span>, prompt=
              <span className={styles.cs}>&quot;...&quot;</span>, schedule=
          ...
          <p>
            各ジョブは<code>context_from</code>が指す先の<strong>直近の完了済み出力</strong>
            を読むだけで、他ジョブの完了を待つわけではない点に注意する。
          </p>
          ...
          <div className={styles.codeBlock}>
            <div className={styles.codeLine}>
              &#123;<span className={styles.cs}>&quot;wakeAgent&quot;</span>:{" "}
              <span className={styles.ck}>false</span>&#125;
            </div>
          </div>
          <p>
            ファイル変更ゲート・外部フラグゲート・SQL件数ゲートなど、この仕組みで大半の「変化があった時だけ動かす」ユースケースを$0で実現できる。さらに徹底したいなら、LLMを一切介さない
            <strong>no-agent モード</strong>（<code>no_agent=True</code>
            ）でスクリプトの標準出力をそのまま配信するワークドッグ運用も可能である。
          </p>
          ...
          <p>
            未指定（provider/modelを明示しない）で作成されたジョブは、作成時点のグローバルデフォルトを
            <strong>スナップショット</strong>
            する。後でグローバルデフォルトを変更しても、そのジョブは
            <strong>フェイルクローズ</strong>
            する（推論を呼ばず、実行をスキップしてアラートを送る）。これは「意図せず有料プロバイダへ切り替わって課金される」事故を防ぐ設計であり、変更を追随させたい場合は明示的にジョブを更新してピン留めし直す必要がある。
          </p>

          <h3 id="116-ワークディレクトリとagentsmd">11.6 ワークディレクトリとAGENTS.md</h3>
          <p>
            Cronジョブはデフォルトでどのリポジトリからも切り離されて動く（AGENTS.md等はロードされない）。
            <code>workdir</code>
            を指定するとそのディレクトリのAGENTS.md/CLAUDE.md/.cursorrulesが注入され、terminal・file系ツールもそのディレクトリを基準に動作する。ただし
            <code>workdir</code>付きジョブは並列プールではなく<strong>シーケンシャル</strong>
            に実行される点（プロセスグローバルなcwd状態の衝突を避けるため）は把握しておく必要がある。
          </p>
          ...
          <p>
            MCP（Model Context
            Protocol）は外部ツールサーバーに接続する標準規格である。Hermesはstdioサーバー（ローカルサブプロセス）とHTTPサーバー（リモートエンドポイント）の両方を同じ設定ファイルで扱える。
          </p>
          <div className={styles.codeBlock}>
            <div className={styles.codeLine}>
            ...
            <div className={styles.codeLine}>
              ...
              , <span className={styles.cs}>&quot;/home/user/projects&quot;</span>]
            </div>
            <div className={styles.codeLine}></div>
            <div className={styles.codeLine}>
              {"  "}
          ...
          <p>
            OAuth
            2.1が必要なホスト型MCPサーバー（Linear・Sentry・Atlassian・Asana・Figma・Stripeなど）は
            <code>auth: oauth</code>
            を指定するだけで、Hermesがディスカバリ・動的クライアント登録・PKCE・トークンリフレッシュまで面倒を見る。
          </p>
          ...
          <p>
            <code>include</code>（ホワイトリスト）と<code>exclude</code>
            （ブラックリスト）が両方指定された場合は<code>include</code>
            が優先される。すべてのツールがフィルタで除外され、かつリソース/プロンプトのユーティリティラッパーも無効化されている場合、Hermesはそのサーバーの空のツールセットを作らない（ツール一覧を汚さない）。
          </p>

          <h3 id="123-一意名前空間">12.3 一意名前空間</h3>
          <p>
            MCPツールは<code>mcp_&lt;server名&gt;_&lt;tool名&gt;</code>
            の形式で自動的にプレフィックスされ、組み込みツールと衝突しない。例えば
            <code>github</code>サーバーの<code>create-issue</code>ツールは
            <code>mcp_github_create_issue</code>として登録される。
          </p>

          <h3 id="124-並列実行と信頼できる用途のみ許可">12.4 並列実行と信頼できる用途のみ許可</h3>
          <p>
            読み取り専用の問い合わせなど、同時実行しても安全なツールに限り
            <code>supports_parallel_tool_calls: true</code>
            を設定すると並列実行が有効になる。書き込みを伴うツールや共有状態に触れるツールでは、競合状態のリスクを検証してから有効化すべきである。
          </p>

          <h3 id="125-資格情報の分離">12.5 資格情報の分離</h3>
          <p>
            stdioサーバーのサブプロセスには、ホストのシェル環境がそのまま渡ることはない。
            <code>PATH</code> / <code>HOME</code> / <code>USER</code> / <code>LANG</code>
            などの安全な変数と、<code>env:</code>
            で明示的に指定した変数のみが渡される。エラーメッセージ中のGitHub
            PAT・OpenAI形式キー・Bearerトークンなどは自動的に<code>[REDACTED]</code>へ置換される。
          </p>
          ...
          <p>
            <code>hermes mcp catalog</code> / <code>hermes mcp install &lt;name&gt;</code>
            で導入できる公式カタログエントリはNousのPRレビューを経ているが、それでも
            <strong>
              <code>source:</code>のリポジトリと<code>bootstrap</code>コマンドは自分の目で確認する
            </strong>
            ことが推奨されている。マニフェストは
            <code>optional-mcps/&lt;name&gt;/manifest.yaml</code>
            としてGitHub上で公開されており、インストール前にレビューできる。
          </p>
          ...
          <p>
            承認モードは<code>smart</code>
            （補助LLMがリスクを判定、低リスクは自動承認・高リスクは自動拒否・不明確なものは人間に確認）、
            <code>manual</code>（常に確認）、<code>off</code>（<code>--yolo</code>
            相当、全チェック無効）の3種類。
            <strong>
              本番のゲートウェイでは<code>off</code>を使わない
            </strong>
            のが原則である。
          </p>
          <p>
            さらに、<code>--yolo</code>や<code>approvals.mode: off</code>
            を設定していても絶対に実行されない<strong>ハードラインブロックリスト</strong>
            が存在する。<code>rm -rf /</code>、フォーク爆弾、マウント済みルートデバイスへの
            <code>mkfs</code>、<code>dd if=/dev/zero of=/dev/sd*</code>
            などが該当し、これは承認レイヤーより手前でコマンドを止める、上書き不可能な床（floor）として機能する。
          </p>
          <p>
            ユーザー定義の拒否ルール<code>approvals.deny</code>
            は、この床の一段上にある編集可能なガードレールで、<code>--yolo</code>
            より先に評価される。
          </p>
          <div className={styles.codeBlock}>
            <div className={styles.codeLine}>
          ...
          <p>
            本番のメッセージングゲートウェイでは、危険コマンド承認そのものを不要にする
            <strong>コンテナバックエンド（Docker/Modal/Daytona/Vercel Sandbox）の採用</strong>
            が推奨されている。コンテナはケイパビリティを<code>ALL</code>ドロップした上で必要最小限（
            <code>DAC_OVERRIDE</code>・<code>CHOWN</code>・<code>FOWNER</code>）のみ再付与し、
            <code>no-new-privileges</code>・プロセス数制限・サイズ制限付き<code>tmpfs</code>
            が既定で適用される。
          </p>
          ...
          <p>
            <strong>
              <code>GATEWAY_ALLOW_ALL_USERS=true</code>を本番で使わない
            </strong>
            ことが最重要事項である。プラットフォーム別許可リスト（
            <code>TELEGRAM_ALLOWED_USERS</code>
            など）かDMペアリング（ワンタイムコード＋所有者承認）を使う。認可チェックの優先順位は「プラットフォーム別all-allowフラグ
            → DMペアリング承認済みリスト → プラットフォーム別許可リスト → グローバル許可リスト →
            グローバルall-allow → デフォルト拒否」の順である。
          </p>

          <h3 id="134-ssrf対策とwebアクセス方針">13.4 SSRF対策とWebアクセス方針</h3>
          <p>
            Web検索・抽出・ブラウザ・vision URL取得はすべて、RFC
            1918プライベートアドレス・ループバック・リンクローカル（クラウドメタデータ
            <code>169.254.169.254</code>
            含む）・CGNAT空間へのアクセスをデフォルトでブロックする。DNS解決失敗もfail-closed（ブロック扱い）として扱われ、リダイレクトチェーンもホップごとに再検証される。社内サービスへのアクセスを意図的に許可する必要がある特殊な環境でのみ
            <code>security.allow_private_urls: true</code>を検討する。
          </p>

          <h3 id="135-tirithによる実行前スキャン">13.5 Tirithによる実行前スキャン</h3>
          <p>
            コマンド実行前に<Ext href="https://github.com/sheeki03/tirith">tirith</Ext>
            というコンテンツレベルのスキャナが追加で動作し、ホモグラフURLスプーフィング（国際化ドメイン攻撃）やパイプ・トゥ・インタープリタパターン（
            <code>curl | bash</code>など）をパターンマッチだけでは検出できない粒度で検知する。
          </p>

          <h3 id="136-サプライチェーンアドバイザリ">13.6 サプライチェーン・アドバイザリ</h3>
          <p>
            既知の侵害されたパッケージバージョン（例として2026年5月の<code>mistralai 2.4.6</code>
            汚染事案が挙げられている）に一致するPythonパッケージがアクティブなvenvにないか、起動時に検査される。
            <code>hermes doctor</code>で詳細な是正手順を確認できる。
          </p>
          ...
          <ol>
            ...
            <li>
              コンテナバックエンド（<code>terminal.backend: docker</code>等）を使う
            </li>
            <li>CPU・メモリ・ディスクのリソース制限を適切に設定する</li>
            <li>
              <code>.env</code>のパーミッションを<code>chmod 600</code>にする
            </li>
            <li>DMペアリングを有効化し、ユーザーIDのハードコードを避ける</li>
            <li>
              <code>command_allowlist</code>を定期的に監査する
            </li>
            <li>
              <code>terminal.cwd</code>を機密ディレクトリに設定しない
            </li>
            <li>ゲートウェイをrootで実行しない</li>
            <li>
              <code>~/.hermes/logs/</code>を監視する
            </li>
            <li>
              <code>hermes update</code>を定期的に実行する
            </li>
          </ol>
          ...
          <p>
            多くのLLMプロバイダは会話プレフィックス（システムプロンプト＋履歴）をキャッシュする。システムプロンプトが安定していれば（同じコンテキストファイル、同じメモリ）、以降のメッセージはキャッシュヒットにより大幅に安価になる。キャッシュはモデルとアカウントに紐づくため、
            <code>/model</code>
            での明示的切り替え、自動プロバイダフォールバック、資格情報プールのローテーションはいずれも
            <strong>次のターンで会話全体を通常価格で再読み込みさせる</strong>
            。長いセッションでの頻繁なモデル切り替えはコストを大きく押し上げる。
          </p>
          ...
          <div className={styles.tableScroll}>
            <table>
              ...
              <tbody>
                <tr>
                  <td>
                    <code>/compress</code>を早めに使う
                  </td>
                  <td>会話履歴を要約し、トークン数を大幅に削減</td>
                </tr>
                <tr>
                  <td>
                    <code>delegate_task</code>で並列調査
                  </td>
                  <td>各サブエージェントの最終要約のみが親のコンテキストに戻る</td>
                </tr>
                <tr>
                  <td>
                    <code>execute_code</code>でバッチ処理
                  </td>
                  <td>
                    中間ツール結果がコンテキストに入らず、<code>print()</code>のみ返る
                  </td>
                </tr>
                <tr>
                  <td>
                    cronの<code>enabled_toolsets</code>を絞る
                  </td>
          ...
          <p>
            curator・goal
            judge・memory/skillのバックグラウンドレビューは、いずれも独立した「補助タスク」スロットとして扱われ、メインモデルとは別のプロバイダ/モデルを指定できる。
          </p>
          <div className={styles.codeBlock}>
            <div className={styles.codeLine}>
            ...
            <table>
              ...
              <tbody>
                <tr>
                  <td>MCPツールが一覧に出ない</td>
                  <td>
                    サーバー未接続・フィルタで除外・<code>enabled: false</code>
                  </td>
                  <td>
                    <code>/reload-mcp</code>後にログ確認、<code>tools.include</code>を見直す
                  </td>
                </tr>
                <tr>
                  <td>Docker内でユーザーがpairing approveできない</td>
                  <td>
                    <code>docker exec</code>がデフォルトでrootになりファイル権限が合わない
                  </td>
                  <td>
                    <code>docker exec -u hermes ...</code>で実行する
                  </td>
                </tr>
                <tr>
                  ...
                  <td>
                    ジョブは意図通りフェイルクローズしている。明示的に<code>provider</code>/
                    <code>model</code>をピン留めする
                  </td>
                </tr>
                <tr>
                  <td>OAuth MCPサーバーでトークンが取得できているように見えるが実際は失敗</td>
                  <td>
                    一部プロバイダ（Google Drive等）が動的クライアント登録を拒否しつつ
                    <code>tools/list</code>は無認証で応答する
                  </td>
                  <td>
                    独自OAuthクライアントを作成し<code>oauth.client_id</code>/
                    <code>client_secret</code>を設定する
                  </td>
                </tr>
                <tr>
                  ...
                  <td>
                    モデル切り替え頻度を下げる、<code>/compress</code>を使う
                  </td>
                </tr>
                <tr>
                  ...
                  <td>
                    仕様通り。<code>terminal</code>ツールへのフォールバックを前提に設計する
                  </td>
                </tr>
              </tbody>
            </table>
          ...
          <p>
            本ガイドは以下の一次情報源（Hermes
            Agent公式ドキュメント・公式GitHubリポジトリ）および、国際的な技術メディア・著名開発者の発信に基づいて作成した。
          </p>
          <div className={styles.refGrid}>
            ...
            <div className={styles.refCard}>
              ...
              <ul>
                ...
                <li>
                  MarkTechPost「OpenClaw vs Hermes Agent: Why Nous Research's Self-Improving Agent
                  Now Leads OpenRouter's Global Rankings」:{" "}
            ...
            <div className={styles.refCard}>
              ...
              <ul>
                ...
                <li>
                  ...
                  <Ext href="https://github.com/sheeki03/tirith">
          ...
          <p>
            <em>
              本ガイドはHermes
              Agentの公開ドキュメント（2026年8月2日時点）を基に作成した。Hermesは開発の速いプロジェクトであり、設定キー名やデフォルト値は将来のリリースで変更される可能性がある。最新情報は上記の公式ドキュメントを参照されたい。
            </em>
          </p>
        </div>
      </div>
