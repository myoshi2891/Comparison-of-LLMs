    ...
    <SkillGuideClient>
      <div className={styles.hero}>
        ...
        <p className={styles.lead}>
          Claude Code を業務で使い込んでいる中級者〜上級者向けに、SKILL.md
          の設計思想・書き方・Claude
          Code固有のフロントマター・評価運用フローまでをステップバイステップで解説する。
        </p>
      ...
      <section id="intro" className={styles.section}>
        ...
        <p>
          ...
          という複数の選択肢がある。この中で Skills（
          <code className={styles.codeInline}>SKILL.md</code>
          ）は、2025年10月の発表以降、コミュニティで急速に採用が広がっている仕組みである。Django
          の共同開発者として知られる著名開発者 Simon Willison は、発表当日に「Claude Skills
          はすごい。MCP
          より大きなインパクトを持つかもしれない」という趣旨の考察を公開し、その理由として、SKILL.md
          が Markdown ファイル + YAML
          フロントマターというだけの、概念的に極めてシンプルな仕組みでありながら、トークン効率が非常に高い点を挙げている（出典は末尾を参照）。
        </p>
        <p>
          このガイドでは、公式ドキュメントの内容を土台にしながら、実務で SKILL.md
          を設計・運用するための手順を段階的に解説する。
        </p>
      ...
      <section id="architecture" className={styles.section}>
        <h2>
          <i aria-hidden="true" className="ti ti-stack-2" /> Progressive Disclosure —<br />
          Skillのアーキテクチャ
        </h2>
        <p>
          Skills
          を正しく設計する上で最も重要な前提は、すべての情報が常にコンテキストに乗るわけではないという点である。Claude
          Code は Skill の内容を3段階（Progressive Disclosure）で読み込む。
        </p>
        ...
        <div className={`${styles.callout} ${styles.calloutInfo}`}>
          ...
          <p>
            ...
            Willisonはこの点を「非常にトークン効率が良い」設計と評している。
          </p>
        </div>
      </section>
      ...
      <section id="step1" className={styles.section}>
        ...
        <ol>
          <li>
            Skillなしで、Claude(Claude
            Aと呼ぶ)に代表的なタスクを実施させ、具体的にどこで失敗するか・何度も同じ指示を与えているかを観察する
          </li>
      ...
      <section id="step2" className={styles.section}>
        ...
        <p>
          Skillは1つのディレクトリで、SKILL.md
          だけが必須である。それ以外は用途に応じて自由に追加できる。
        </p>
        ...
        <p>
          同名のSkillが複数レベルに存在する場合は、Enterprise &gt; Personal &gt; Project
          の順で優先される。モノレポでは{" "}
          <code className={styles.codeInline}>packages/frontend/.claude/skills/</code>{" "}
          のようにネストした場所にもSkillを置け、その配下のファイルを扱っているときだけ自動的に読み込まれる。
        </p>
      </section>
      ...
      <section id="step3" className={styles.section}>
        ...
        <div className={styles.codeBlock}>
          ...
          <div className={styles.codeLine}>
            description:
            PDFファイルからテキストや表を抽出し、フォーム入力やドキュメント結合を行う。PDF、フォーム、ドキュメント抽出についてユーザーが言及した場合に使用する。
          </div>
          <div className={styles.codeLine}>---</div>
        </div>

        <p>
          命名規則は「動名詞(-ing形)」が推奨されている。避けるべき命名の例:{" "}
          <code className={styles.codeInline}>helper</code> /{" "}
          <code className={styles.codeInline}>utils</code> /{" "}
          <code className={styles.codeInline}>tools</code> のような曖昧な名前、
          <code className={styles.codeInline}>documents</code> /{" "}
          <code className={styles.codeInline}>data</code> のような汎用すぎる名前。
        </p>
      </section>
      ...
      <section id="step4" className={styles.section}>
        ...
        <p>
          <code className={styles.codeInline}>description</code>{" "}
          はClaudeが「このSkillを今使うべきか」を判断する唯一の材料である。以下の原則を守る。
        </p>
        <ul>
          <li>
            必ず三人称で書く(description
            はシステムプロンプトに注入されるため、視点が一貫しないと発見精度が落ちる)
          </li>
      ...
      <section id="step5" className={styles.section}>
        ...
        <p>
          Level 2
          に読み込まれた本文は、会話履歴や他のコンテキストと同じ枠を奪い合う。公式ガイドの前提は「Claudeはすでに十分賢い」であり、Claudeがすでに知っていることの説明を書かないことが核心である。
        </p>
        <p>
      ...
      <section id="step6" className={styles.section}>
        ...
        <p>
          本文が長くなりそうな場合、以下の3パターンのいずれかで分割する。共通ルールは「参照は
          SKILL.md から1階層のみ」にすることである。ネストした参照(SKILL.md → advanced.md →
          details.md)は、Claudeが <code className={styles.codeInline}>head -100</code>{" "}
          のような部分読み込みで済ませてしまい、情報が欠落するリスクがある。
        </p>
        <div className={styles.tableWrap}>
          <table className={styles.table}>
            <thead>
            ...
            <tbody>
              ...
              <tr>
                ...
                <td>
                  <code className={styles.codeInline}>reference/finance.md</code>{" "}
                  のようにドメインごとにファイル分割
                </td>
              </tr>
              <tr>
      ...
      <section id="step8" className={styles.section}>
        ...
        <p>
          Claude Code は Agent Skills のオープン標準を拡張し、name / description
          以外にも多数のフロントマターフィールドをサポートしている(すべて任意項目)。
        </p>
        <div className={styles.tableWrap}>
          <table className={styles.table}>
            <thead>
            ...
            <tbody>
              <tr>
                <td>
                  <code className={styles.codeInline}>when_to_use</code>
                </td>
                <td>発火条件の補足(description と合算で1,536文字上限)</td>
              </tr>
              <tr>
                <td>
                  <code className={styles.codeInline}>argument-hint</code>
                </td>
                <td>オートコンプリート時に表示される引数のヒント(例: [issue-number])</td>
              </tr>
              <tr>
                <td>
                  <code className={styles.codeInline}>arguments</code>
                </td>
                <td>
                  <code className={styles.codeInline}>$name</code>{" "}
                  形式で参照できる名前付き引数のリスト
              ...
              <tr>
                <td>
                  <code className={styles.codeInline}>disable-model-invocation</code>
                </td>
                <td>true にするとClaudeによる自動発火を禁止し、/name での手動実行のみに限定</td>
              </tr>
              <tr>
                <td>
                  <code className={styles.codeInline}>user-invocable</code>
                </td>
                <td>false にすると / メニューから隠し、Claude専用の背景知識として扱う</td>
              </tr>
              <tr>
                <td>
                  <code className={styles.codeInline}>allowed-tools</code>
                </td>
                <td>そのSkillが発火したターンに限り、確認なしで使えるツールを許可</td>
              </tr>
              <tr>
                <td>
                  <code className={styles.codeInline}>disallowed-tools</code>
                </td>
                <td>そのターンの間、特定ツールを利用不可にする</td>
              </tr>
              <tr>
                <td>
                  <code className={styles.codeInline}>model</code> /{" "}
                  <code className={styles.codeInline}>effort</code>
                </td>
                <td>Skill実行中だけモデルや推論の強度を上書き</td>
              </tr>
              <tr>
                <td>
                  <code className={styles.codeInline}>context: fork</code>
                </td>
                <td>独立したサブエージェントのコンテキストで実行</td>
              </tr>
              <tr>
                <td>
                  <code className={styles.codeInline}>agent</code>
                </td>
                <td>context: fork 時に使うサブエージェントの種類</td>
              </tr>
              <tr>
                <td>
                  <code className={styles.codeInline}>paths</code>
                </td>
        ...
        <p>
          <code className={styles.codeInline}>disable-model-invocation</code> と{" "}
          <code className={styles.codeInline}>user-invocable</code>{" "}
          の組み合わせで、誰が呼び出せるかが変わる。
        </p>
        <div className={styles.tableWrap}>
          <table className={styles.table}>
            <thead>
            ...
            <tbody>
              ...
              <tr>
                <td>
                  <code className={styles.codeInline}>disable-model-invocation: true</code>
                </td>
                <td>可</td>
                <td>不可</td>
              ...
              <tr>
                <td>
                  <code className={styles.codeInline}>user-invocable: false</code>
                </td>
                <td>不可</td>
                <td>可</td>
                <td>説明は常時コンテキストにあり、発火時に本文がロードされる</td>
        ...
        <p>
          副作用を伴う操作(<code className={styles.codeInline}>/deploy</code> や{" "}
          <code className={styles.codeInline}>/commit</code> のような操作)には{" "}
          <code className={styles.codeInline}>disable-model-invocation: true</code>{" "}
          を付け、Claudeが自律的に実行してしまわないようにするのが定石である。
        </p>
      </section>
      ...
      <section id="step9" className={styles.section}>
        ...
        <ul>
          ...
          <li>
            マジックナンバーを避ける(
            <code className={styles.codeInline}>
              TIMEOUT = 30 # HTTPリクエストは通常30秒以内に完了する
            </code>{" "}
            のように理由をコメントで示す)
          </li>
          <li>
            実行するのか参照として読ませるのか、指示を明確にする(「analyze_form.py
            を実行せよ」と「analyze_form.py の抽出アルゴリズムを参照せよ」は別の指示)
          </li>
          <li>
            破壊的な操作や大量更新には「計画ファイルを作成→検証→実行」のパターンを使う(例:
            50件のPDFフォーム更新を一気に適用する前に changes.json を検証する)
          </li>
        </ul>
        <p>
      ...
      <section id="step11" className={styles.section}>
        ...
        <p>
          公式の推奨フローは「Claude A(Skillを設計する側)」と「Claude
          B(実際にSkillを使ってタスクをこなす側)」を役割分担させ、観察に基づいて改善を回すというものである。
        </p>
        ...
        <p>
          Claude Code には <code className={styles.codeInline}>skill-creator</code>{" "}
          プラグインが用意されており、テストケースを{" "}
          <code className={styles.codeInline}>evals/evals.json</code>{" "}
          に保存し、Skillあり/なしの合格率・トークン数・実行時間を自動比較できる。バージョン間のブラインドA/B比較や、発火条件(description)のチューニング支援機能も含まれている。
        </p>
        ...
        <div className={styles.tableWrap}>
          <table className={styles.table}>
            ...
            <tbody>
              <tr>
                <td>
                  Windowsスタイルのパス(<code className={styles.codeInline}>scripts\helper.py</code>
                  )
                </td>
                <td>Unix環境でエラーになる</td>
                <td>
                  常にスラッシュ区切り(<code className={styles.codeInline}>scripts/helper.py</code>)
                </td>
              </tr>
              <tr>
                <td>選択肢を並べすぎる</td>
              ...
              <tr>
                ...
                <td>
                  <code className={styles.codeInline}>ServerName:tool_name</code> の完全修飾名を使う
                </td>
              </tr>
            </tbody>
          </table>
      ...
      <section id="summary" className={styles.section}>
        ...
        <p>
          SKILL.md
          の設計は、突き詰めると「Claudeはすでに賢い」という前提のもとで、足りていない情報だけを、必要なタイミングでだけ渡すという一点に集約される。Progressive
          Disclosureのアーキテクチャを理解し、評価駆動で本文を磨き、Claude
          Code固有のフロントマター(disable-model-invocation や allowed-tools
          など)で発火・権限を制御することで、コンテキストを圧迫せずに再利用可能な専門知識を積み上げていくことができる。
        </p>
      </section>
      ...
      <section id="sources" className={styles.section}>
        ...
        <div className={`${styles.callout} ${styles.calloutWarning}`}>
          ...
          <p>
            ...
            code.claude.com/docs/en/skills)を直接参照すること。
          </p>
        </div>
      </section>

      <footer className={styles.pageFooter}>SKILL.md 実践ガイド — 情報基準日: 2026年7月26日</footer>
    </SkillGuideClient>
  );
}
