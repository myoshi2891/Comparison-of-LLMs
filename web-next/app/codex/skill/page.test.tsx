// Phase B-3 [Red] contract test. Expected to FAIL until Green phase
// implements app/codex/skill/page.tsx.

/**
 * Phase B-3 契約テスト (/codex/skill)。
 *
 * 固定する契約:
 * - `metadata` が export され、title に「マークダウンファイル」と「Codex」を含む
 * - `<h1>` が 1 つ存在し、`マークダウンファイル` を含む
 * - 12 個の section id が存在する (overview, directory, agents-md,
 *   agents-override, skill-md, prompt-md, sdd-files, config, openai-yaml,
 *   multi-agent, best-practices, sources)
 * - 12 個の TOC リンクが `#section-id` 形式で存在する
 * - 外部リンク (http/https) には全て `target="_blank"` かつ
 *   `rel="noopener noreferrer"` が付与されている
 * - `sources` セクション内に 20 件以上の外部リンクが存在する
 *   (legacy HTML は 23 件。余裕を持たせて 20 を下限に固定)
...
import CodexSkillPage, { metadata as rawMetadata } from "@/app/codex/skill/page";

const Page = CodexSkillPage as unknown as () => ReactElement;
...
describe("/codex/skill - metadata", () => {
  it("exports a metadata object with title containing マークダウンファイル and Codex", () => {
...
describe("/codex/skill - page structure", () => {
...
describe("/codex/skill - external link safety", () => {
  ...
  it("sources section contains at least 20 external links", () => {
    const { container } = render(<Page />);
...
describe("/codex/skill - static source safety", () => {
