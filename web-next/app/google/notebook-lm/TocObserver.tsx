...
export default function TocObserver() {
  useEffect(() => {
    ...
    if (links.length > 0) links[0].classList.add(styles.tocLinkActive ?? "active");
          ...
          if (entry.isIntersecting) {
            ...
            for (const l of links) {
              if (l.getAttribute("href") === `#${id}`) {
                l.classList.add(styles.tocLinkActive ?? "active");
              } else {
                l.classList.remove(styles.tocLinkActive ?? "active");
              }
            }
          }
        }
