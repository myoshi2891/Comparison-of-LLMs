...
describe("/local-llm/best-practices - metadata", () => {
  it("exports a metadata object with title containing 'ローカルLLM／セルフホスティング ベストプラクティスガイド'", () => {
...
describe("/local-llm/best-practices - page structure", () => {
  it("renders an <h1> containing 'ローカルLLM／セルフホスティング ベストプラクティスガイド'", () => {
    const { container } = render(<Page />);
    const h1 = container.querySelector("h1");
    expect(h1).not.toBeNull();
    expect(h1?.textContent).toContain("ローカルLLM／セルフホスティング");
    expect(h1?.textContent).toContain("ベストプラクティスガイド");
  });
...
describe("/local-llm/best-practices - external link safety", () => {
...
describe("/local-llm/best-practices - code block validation", () => {
  it("renders code blocks with correct language classes", () => {
    const { container } = render(<Page />);
    const codeBlock = container.querySelector("pre code");
    expect(codeBlock).not.toBeNull();
    expect(codeBlock?.className).toContain("language-yaml");
  });
});

describe("/local-llm/best-practices - static source safety", () => {
