...
describe("/mcp/mcp-best-practices-intermediate - metadata", () => {
  it("exports a metadata object with title containing 'MCP実践ベストプラクティス'", () => {
    expect(metadata).toBeDefined();
    const title =
      typeof metadata.title === "string"
        ? metadata.title
        : (metadata.title as { default?: string } | undefined)?.default;
    expect(title).toContain("MCP実践ベストプラクティス");
  });
...
describe("/mcp/mcp-best-practices-intermediate - page structure", () => {
  it("renders an <h1> containing 'MCP実践ベストプラクティス'", () => {
    const { container } = render(<Page />);
    const h1 = container.querySelector("h1");
    expect(h1).not.toBeNull();
    expect(h1?.textContent).toContain("MCP実践ベストプラクティス");
  });

  it("renders all 14 expected sections", () => {
    const { container } = render(<Page />);
...
describe("/mcp/mcp-best-practices-intermediate - external link safety", () => {
...
describe("/mcp/mcp-best-practices-intermediate - code block validation", () => {
