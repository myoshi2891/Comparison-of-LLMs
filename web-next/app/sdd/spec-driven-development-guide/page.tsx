      ...
      <main className={styles.main}>
        ...
        <section className={styles.chapter} id="sec1">
          ...
          <p className={styles.sectionLead}>
            仕様駆動開発（Spec-Driven Development, SDD）とは、コードではなく、実行可能でバージョン管理された「仕様（Spec）」を単一の真実の源（Single Source of Truth）とする開発手法です。開発チーム、あるいはAIコーディングエージェントは、まず「何を作るか」を詳細な仕様として書き下し、そこから実装計画を導出し、計画をアトミックなタスクに分解し、その後で初めてコードを生成します。
          </p>
          ...
          <ul>
            <li>
              <strong>意図のドリフト（Intent Drift）</strong>：会話を重ねるうちに、AIエージェントの出力が当初の意図から徐々にずれていく
            </li>
            <li>
              <strong>アーキテクチャの不整合</strong>：一貫した設計判断がされないまま機能が積み重なる
            </li>
            <li>
          ...
          <p>
            SDDはこの「バイブコーディング」への直接的な対抗策として2025年に登場し、2026年には GitHub Spec Kit、AWS Kiro、Claude Code、Cursor、OpenSpec、BMAD-METHOD、Tessl、Google Antigravity といった主要なAIコーディングツールがそれぞれ独自のSDD実装を提供するに至っています。DeepLearning.AI が2025年後半に専門コースを開講したことも、この手法が実験段階から主流へ移行したシグナルの一つとされています。
          </p>
        ...
        <section className={styles.chapter} id="sec2">
            ...
            <table className={styles.table}>
              ...
              <tbody>
                <tr>
                  ...
                  <td>人間 <strong>と</strong> AIエージェント（曖昧さを補完できない）</td>
                </tr>
                <tr>
                  <td>更新タイミング</td>
          ...
          <ul>
            <li>
              <strong>セキュリティ</strong>：LLMが生成するコードの脆弱性混入率はベンチマークによって9.8%〜42.1%と報告されており（詳細は第13章）、実行可能な仕様がこれに対する検証ゲートとして機能します。
            </li>
            <li>
              <strong>コンプライアンス</strong>：仕様が監査証跡（audit trail）として機能し、規制業界での証跡要件を満たします。
            </li>
            <li>
              <strong>チーム間の整合性</strong>：PM、エンジニア、AIエージェント、レビュアーの間で「仕様」という共通言語を持つことで、役割間の解釈のズレを減らします。
            </li>
          </ul>
          ...
          <ul className={styles.refs}>
            ...
            <li>
              <Ext href="https://arxiv.org/abs/2602.00180">Spec-Driven Development: From Code to Contract in the Age of AI Coding Assistants (arXiv:2602.00180)</Ext>
            </li>
            <li>
        ...
        <section className={styles.chapter} id="sec3">
          <div className={styles.chapterHeader}>
            <h2>
              <span className={styles.chip}>03</span>成熟度モデル：Spec-First / Spec-Anchored / Spec-as-Source
            </h2>
          </div>
          <p className={styles.sectionLead}>
            Thoughtworksのコンサルタント Birgitta Böckeler が提唱し、Piskalaの論文（arXiv, 2026）でも採用されている3段階の成熟度モデルが、実務上もっとも参照される分類です。
          </p>
            ...
            <table className={styles.table}>
              ...
              <tbody>
                ...
                <tr>
                  ...
                  <td>仕様がメインのソースファイルであり、人間は仕様のみを編集し、コードには触れない</td>
          ...
          <ul className={styles.refs}>
            ...
            <li>
              <Ext href="https://arxiv.org/abs/2602.00180">Spec-Driven Development: From Code to Contract (arXiv:2602.00180)</Ext>
            </li>
          </ul>
        </section>
          ...
          <div className={styles.tableWrap}>
            <table className={styles.table}>
              ...
              <tbody>
                ...
                <tr>
                  <td><strong>TDD</strong>（テスト駆動開発）</td>
                  <td>失敗するユニットテスト</td>
                ...
                <tr>
                  <td><strong>BDD</strong>（振る舞い駆動開発）</td>
                ...
                <tr>
                  <td><strong>SDD</strong>（仕様駆動開発）</td>
        ...
        <section className={styles.chapter} id="sec5">
          ...
          <div className={styles.callout}>
            ...
            <br />
            重要なのは、すべてのステップが必須ではないという点です。GitHub Spec Kitの公式ドキュメントでも、簡単な検証であれば <code>specify → plan → tasks → implement</code> の4ステップの「軽量パス」で十分とされ、本番機能や曖昧さが残る作業に対してのみ <code>clarify</code> <code>checklist</code> <code>analyze</code> を品質ゲートとして追加することが推奨されています。
          </div>
          ...
          <div className={styles.tableWrap}>
            <table className={styles.table}>
              ...
              <tbody>
                <tr>
                  <td>Constitution</td>
                  <td>チーム/プロジェクト共通の非交渉的な原則（言語、アーキテクチャ、品質基準）を固定</td>
        ...
        <section className={styles.chapter} id="sec6">
          ...
          <p className={styles.sectionLead}>
            SDDにおける「仕様」の質は、要求（Requirements）の書き方に大きく依存します。ここで業界標準になりつつあるのが <strong>EARS（Easy Approach to Requirements Syntax）</strong> 記法です。AWS Kiroもこの記法を <code>requirements.md</code> の標準フォーマットとして採用しています。
          </p>
          ...
          <div className={styles.codeWrap}>
            ...
            <div className={styles.codeBody}>
              <div className={styles.codeLine}>While [任意の事前条件], When [任意のトリガー], the [システム名] shall [システムの応答]</div>
            </div>
          </div>
          <p>
          ...
          <div className={styles.tableWrap}>
            <table className={styles.table}>
              ...
              <tbody>
                <tr>
                  <td><strong>Ubiquitous</strong>（普遍要求）</td>
                  <td>なし</td>
                  <td>常に真である基本的な性質を記述</td>
                  <td>「本システムはすべてのユーザー入力を検証しなければならない (shall)」</td>
                </tr>
                <tr>
                  <td><strong>Event-Driven</strong>（イベント駆動）</td>
                  <td>When</td>
                  <td>特定のイベント発生時のみ有効</td>
                  <td>「決済が完了した時 (When)、本システムは通知を送信しなければならない」</td>
                </tr>
                <tr>
                  <td><strong>Unwanted Behavior</strong>（望まない振る舞い）</td>
                  <td>If / Then</td>
                  <td>エラー・故障・異常系を扱う</td>
                  <td>「パスワードが誤って入力された場合 (If)、本システムはエラーメッセージを表示しなければならない」</td>
                </tr>
                <tr>
                  <td><strong>State-Driven</strong>（状態駆動）</td>
                  <td>While</td>
                  <td>特定の状態が継続している間有効</td>
                  <td>「決済処理中である間 (While)、本システムはキャンセルボタンを無効化しなければならない」</td>
                </tr>
                <tr>
                  <td><strong>Optional Feature</strong>（オプション機能）</td>
                  <td>Where</td>
                  <td>特定のオプション機能が存在する場合のみ有効</td>
                  <td>「多要素認証機能が有効な場合 (Where)、本システムは確認コードを要求しなければならない」</td>
                </tr>
              </tbody>
            </table>
          </div>
          ...
          <p>
            複数のキーワードを組み合わせた「複合要求（Complex requirements）」も定義されています。例えば事前条件とトリガーを両方含む形として、「航空機が地上にある間、逆推力が指令された時、エンジン制御システムは逆推力を有効化しなければならない」のように記述します。
          </p>
          ...
          <ul>
            ...
            <li>
              AWS Kiroの <code>requirements.md</code> は、ユーザーストーリーと受け入れ基準をEARS形式（特にWhen/Then構文）で記述する運用が公式に案内されています。
            </li>
            <li>
          ...
          <ul className={styles.refs}>
            <li>
              <Ext href="https://alistairmavin.com/ears/">Alistair Mavin - EARS: Easy Approach to Requirements Syntax | Official Guide</Ext>
            </li>
            <li>
            ...
            <li>
              <Ext href="https://www.linkedin.com/pulse/easy-approach-requirements-syntax-ears-chatgpt-rob-black">Easy Approach to Requirements Syntax (EARS) with ChatGPT - LinkedIn (Rob Black)</Ext>
            </li>
            <li>
              <Ext href="https://kiro.dev/docs/specs/">Specs - IDE - Docs - Kiro</Ext>
            </li>
            <li>
              <Ext href="https://repost.aws/articles/AROjWKtr5RTjy6T2HbFJD_Mw/%F0%9F%91%BB-kiro-agentic-ai-ide-beyond-a-coding-assistant-full-stack-software-development-with-spec-driven-ai">👻 Kiro Agentic AI IDE - AWS re:Post</Ext>
            </li>
          </ul>
        </section>
        ...
        <section className={styles.chapter} id="sec7">
            ...
            <table className={styles.table}>
              ...
              <tbody>
                <tr>
                  ...
                  <td><code>specify</code> CLI、30以上のAIエージェント統合、constitution機構</td>
                </tr>
                <tr>
                ...
                <tr>
                  <td><strong>Claude Code系（cc-sdd等）</strong></td>
                ...
                <tr>
                  <td><strong>Cursor (.cursor/rules)</strong></td>
                  <td>Cursor</td>
                  <td>軽量な規約ベース</td>
          ...
          <h3>7.2 併用パターン</h3>
          <p>
            複数の比較記事（Reenbit社、Reinvently社）は、プロジェクトのフェーズによってツールを乗り換える実例を報告しています。ブラウンフィールドの改修（OpenSpec）が軌道に乗り新規機能を積み上げる段階になると、デルタフォーマットだけでは薄く感じられるようになり、アーカイブ済みの仕様をBMADのArchitectエージェントへの入力として引き継ぐ、といった移行が語られています。また、スタートアップがシリーズAを迎えPMを初採用したタイミングで、Spec KitのconstitutionをBMADのマスターエージェントプロンプトへ移す、という移行パターンも紹介されています。
          </p>
          ...
          <ul className={styles.refs}>
            ...
            <li>
              <Ext href="https://reenbit.com/bmad-vs-spec-kit-vs-openspec-choosing-your-spec-driven-ai-framework/">BMAD vs Spec Kit vs OpenSpec - Reenbit</Ext>
            </li>
            <li>
            ...
            <li>
              <Ext href="https://redreamality.com/blog/-sddbmad-vs-spec-kit-vs-openspec-vs-promptx/">What Is SDD? BMAD vs spec-kit vs OpenSpec vs PromptX</Ext>
            </li>
          </ul>
        </section>
        ...
        <section className={styles.chapter} id="sec8">
          ...
          <p className={styles.sectionLead}>
            GitHub Spec Kitは、GitHubがOSS（MITライセンス）として提供する <code>specify</code> という名前のCLIツールで、2026年時点でもっとも広く採用されているSDDツールの一つです（報告によりスター数の数字に幅がありますが、2026年前半時点でおよそ88,000〜110,000以上のGitHubスターを獲得しています）。Claude Code、GitHub Copilot、Gemini CLI、Cursorなど30以上のAIエージェント統合をサポートしています。
          </p>

          <h3>8.1 インストールと初期化</h3>
          <p>
            <code>uv</code> パッケージマネージャーを使い、次のようにインストールし、プロジェクトを初期化します。Claude Codeでは <code>.claude/skills/</code> 配下、Codex CLIでは <code>.agents/skills/</code> 配下に <code>speckit-*</code> スキルを配置する形式が使われる点に注意してください。
          </p>
          ...
          <div className={styles.codeWrap}>
            ...
            <div className={styles.codeBody}>
              <div className={styles.codeLine}><span className={styles.cc}># インストール</span></div>
              <div className={styles.codeLine}><span className={styles.cv}>uv tool install specify-cli --from git+https://github.com/github/spec-kit.git</span></div>
              <div className={styles.codeLine}></div>
              <div className={styles.codeLine}><span className={styles.cc}># プロジェクト初期化（例：Claude Codeと統合）</span></div>
              <div className={styles.codeLine}><span className={styles.cv}>specify init my-project --integration claude</span></div>
            </div>
          </div>
          ...
          <p>
            constitutionは、そのプロジェクトにおいてAIエージェントがどんな理由があっても逸脱してはならない制約を定義するものです。例えば「TypeScriptのみ・strictモード」「外部の状態管理ライブラリを使わない」「すべての機能に結合テストを持たせる」「WCAG 2.1 AA準拠」「明示的なオプトインなしのテレメトリ禁止」といった原則を1回定義すれば、以降のすべてのspec/plan/tasksがこの原則に照らしてチェックされます。
          </p>

          <h3>8.4 実務上の注意点</h3>
          <ul>
            <li>
              公式のガイダンスでは、<code>/speckit.specify</code> の段階では技術スタックにできるだけ触れず、「何を」「なぜ」作るのかを先に明確にすることが推奨されています。
            </li>
            <li>
              <code>/speckit.analyze</code> は実装前の最後の防衛線であり、要件が複数箇所に異なる表現で重複していないか、要件同士が矛盾していないかを検出します。
            </li>
            <li>
              実務者のブログ（Den Delimarsky氏）は、30タスクのリストをいきなり無人で <code>/speckit.implement</code> させず、まず3〜5タスクから始めてレビューし、constitutionを調整してからスケールアップすることを推奨しています。
            </li>
            <li>
              Spec Kitは頻繁にCLIの仕様が変更されており（例えば <code>--ai</code> フラグ体系がv0.7.1で非推奨化され、v0.10.0で完全削除されて <code>--integration</code> 方式に置き換えられた）、2026年6月以前のチュートリアルのコマンドが動作しない場合があるため、常に公式ドキュメントを確認する必要があります。
            </li>
            <li>
          ...
          <div className={`${styles.callout} ${styles.calloutWarn}`}>
            ...
            <br />
            Spec Kitのコマンド体系・CLIフラグは変更が速いペースで行われています。実装前には必ず公式ドキュメント（github.github.com/spec-kit）で最新仕様を確認してください。
          </div>
        ...
        <section className={styles.chapter} id="sec9">
          ...
          <p className={styles.sectionLead}>
            AWS Kiroは、Amazon Q Developerの後継として登場したエージェント型IDEで、コード生成前に <code>requirements.md</code>・<code>design.md</code>・<code>tasks.md</code> の3文書を必須とする「spec mandate（仕様の義務化）」を特徴とします。
          </p>
          ...
          <p>
            Kiroの特徴的な機能として、<code>tasks.md</code> 内のタスクの依存関係グラフを自動構築し、依存関係のないタスクを同じ「Wave（波）」としてグループ化し、Wave内は並列実行、Wave間は順次実行するという方式があります。
          </p>

          <div className={styles.mermaidWrap}>
            <MermaidDiagram chart={MMD_8} />
            <div className={styles.diagramCaption}>図8：Kiroのタスク依存関係に基づくWave並列実行</div>
          </div>

          <h3>9.3 Steering Filesとの役割分担</h3>
          <p>
            <code>requirements.md</code> が「何を作るか」、<code>design.md</code> が「どう作るか」を定義するのに対し、Steering Filesは機能を横断してすべてのビルドに適用される制約（コーディング規約、セキュリティポリシーなど）を定義します。規制業界向けには、AWS GovCloudデプロイでコンプライアンス制約が事前設定されたSteering Filesを使う運用も紹介されています。
          </p>

          <h3>9.4 実際の事例：3週間での創薬エージェント構築</h3>
          <p>
            AWSの公式ブログでは、ライフサイエンス業界向けにKiroのspec駆動アプローチを用いた事例が報告されています。3名のソリューションアーキテクトが、他の会議やワークショップと並行しながら3週間で本番稼働するシステムを構築し、Kiroがビジネスロジックコードの95%以上を生成、開発時間にして80時間以上を節約したと報告されています。Agent Hooksによりコード変更時に自動でREADME.mdドキュメントを更新する仕組みも活用されました。
          </p>

          <div className={`${styles.callout} ${styles.calloutTip}`}>
            <span className={styles.calloutLabel}>同ブログの提言</span>
            <br />
            「仕様に前もって投資することはすぐに元が取れる（Invest in Specifications Upfront—It Pays Off Fast）」
          </div>
          ...
          <ul>
            ...
            <li>
              Kiroは既存コードベース（ブラウンフィールド）向けに、新規開発前に <code>structure.md</code>（コードベースのアーキテクチャ）・<code>tech.md</code>（技術スタックとパターン）・<code>product.md</code>（ビジネス文脈）の3文書を自動生成し、ベースラインの理解を確立する機能も持っています。
            </li>
          </ul>
        ...
        <section className={styles.chapter} id="sec10">
          ...
          <p className={styles.sectionLead}>
            Augment Code社のガイドで「もっとも活用されていないパターン」として紹介されているのが、実装を行うエージェント自身に自己検証させるのではなく、別のエージェントに検証させるというパターンです。
          </p>

          <div className={styles.mermaidWrap}>
            <MermaidDiagram chart={MMD_9} />
            <div className={styles.diagramCaption}>図9：Coordinator / Implementor / Verifier のマルチエージェント検証パターン</div>
          </div>
          ...
          <div className={styles.callout}>
            ...
            <br />
            arXivの「Bootstrapping Coding Agents: The Specification Is the Program」という論文では、3〜7名のエンジニアチームが5ヶ月かけて100万行規模のコードベースを、Codexを使い一切人手でコードを書かずに構築した事例が紹介されています。このチームは構造化された <code>docs/</code> ディレクトリを参照システムとして扱い、コードそのものではなく仕様を安定した成果物として位置づけています。
          </div>
        ...
        <section className={styles.chapter} id="sec11">
          ...
          <ul className={styles.refs}>
            ...
            <li>
              <Ext href="https://medium.com/@tojosphine/spec-driven-development-what-i-wish-i-knew-before-i-started-1213d485a244">Spec-Driven Development, What I Wish I Knew Before I Started - Josphine Job</Ext>
            </li>
            <li>
              <Ext href="https://blog.allegro.tech/2026/06/spec-driven-development-best-practices.html">Spec-Driven Development (SDD) — best practices (so far) - Allegro Tech</Ext>
            </li>
        ...
        <section className={styles.chapter} id="sec12">
          ...
          <div className={styles.tableWrap}>
            <table className={styles.table}>
              ...
              <tbody>
                <tr>
                  <td><strong>過剰仕様</strong>（Over-specification）</td>
                  <td>「良いAI生成体験を得るため」という理由で、開発着手前にアプリケーションのあらゆる側面を定義しようとし、管理不能なほど大量のファイルが生まれる</td>
                  <td>仕様の粒度は必要最小限に留め、annoyance testで都度判断する</td>
                </tr>
                <tr>
                  <td><strong>ビッグバンリリースへの偏り</strong></td>
                  <td>重厚な事前仕様化と、一括での大規模リリースに偏りがちになる</td>
                  <td>小さなタスク単位での段階的実装・レビューを徹底する</td>
                </tr>
                <tr>
                  <td><strong>AI生成コードへの慢心</strong>（complacency）</td>
                  <td>仕様に基づいて生成されたコードだからと過信し、人間のレビューを省略してしまう</td>
                  <td>Verifierパターンや Analyzeステップでの機械的検証と、人間レビューを併用する</td>
                </tr>
                <tr>
                  <td><strong>セマンティック拡散</strong>（Semantic diffusion）</td>
                  <td>「spec-driven development」「harness engineering」といった新語が定義の定まらないまま広まり、成熟したエンジニアリング手法なのか、単なる日常的なAIツール利用の言い換えなのかの境界が曖昧になる</td>
                  <td>自チーム内で用語の定義を明文化し、共通認識を持つ</td>
                </tr>
                <tr>
                  <td><strong>ツールのAPI・CLI変更への追従漏れ</strong></td>
                  <td>Spec Kitのようなツールは頻繁にCLI仕様が変更されており、古いチュートリアルのコマンドが動作しなくなることがある</td>
                  <td>常に公式ドキュメントを一次情報として参照する</td>
                </tr>
                <tr>
                  <td><strong>ブラウンフィールドでの過負荷</strong></td>
                  <td>複雑な既存システムに重厚な仕様駆動ツール（Kiro/BMAD）を適用すると、過剰投資になりがちである</td>
          ...
          <ul className={styles.refs}>
            <li>
              <Ext href="https://www.thoughtworks.com/en-us/radar/techniques/spec-driven-development">Spec-driven development | Technology Radar | Thoughtworks</Ext>
            </li>
            <li>
              <Ext href="https://www.thoughtworks.com/radar">Technology Radar | Thoughtworks</Ext>
            </li>
        ...
        <section className={styles.chapter} id="sec13">
          ...
          <p>
            これらのデータを踏まえ、Augment Code社のガイドでは「SDDの仕様は、まさにこうした失敗に対する実行可能な検証ゲートとして機能する」と位置づけています。同ガイドはさらに、いわゆる「Constitutional SDD」という発展系のアプローチにも言及しており、ガバナンス層・憲法的制約・監督チェックポイントを仕様駆動開発に追加するパターンとして、規制業界の監査要件、複数チームにまたがるサービス連携、AIが生成したコードに人間の承認を必須とする場面などで採用が進んでいるとされています。
          </p>
        ...
        <section className={styles.chapter} id="sec14">
          ...
          <p>
            Allegro Tech社のブログでは、外部ツールなしでもLLMとの対話だけで「今からSDD手法で機能Xを実装したい。ブラウンフィールド／グリーンフィールドかを踏まえ、どんなフェーズでどんな文書を作るべきか」と明示的に伝えるだけで練習セッションが始められるとしており、まずは道具に頼らずSDDの型を体で覚えることを勧めています。同社は社内実装として <code>PRODUCT-SPEC.md</code>（技術非依存のビジネス要求）と <code>TECHNICAL-SPEC.md</code>（技術・非機能要求）を分離する独自運用も紹介しています。
          </p>

          <div className={styles.callout}>
            <span className={styles.calloutLabel}>導入判断の参考データ</span>
            <br />
            2026年1月のJetBrains AI Pulse Survey（11,000人の開発者対象）では、90%が業務でAIを使用している一方、SDLC全体でAIを活用しているのはわずか13%にとどまるという調査結果が示されています。またStack Overflowの2025年調査では、84%の開発者がAIツールを利用中または利用予定である一方、その正確性を信頼しているのは33%にとどまり、ポジティブな感情は2023〜2024年の70%超から2025年には60%まで低下したと報告されています。これらのデータは、導入のボトルネックはツールの有無ではなく、AIエージェントの出力に対する「信頼」であることを示唆しており、SDDはこの信頼のギャップを埋めるための検証可能な仕組みとして位置づけられます。
          </div>
        ...
        <section className={styles.chapter} id="sec15">
          ...
          <p className={styles.sectionLead}>導入・実践の際に確認すべきチェックリストとして整理します。</p>
          <ul className={styles.checklist}>
            <li>プロジェクト／チーム共通の <strong>constitution（原則）</strong> を1つ定義したか</li>
            <li>仕様定義の初期段階で、技術スタックに触れず「何を」「なぜ」を明確化したか</li>
            <li>曖昧な要件は <strong>clarify</strong> フェーズで対話的に解消したか</li>
            <li>要求は <strong>EARS記法</strong>（Ubiquitous / Event-Driven / Unwanted Behavior / State-Driven / Optional Feature）で書かれているか</li>
            <li>成熟度モデルとして <strong>Spec-Anchored</strong> を目標に据えているか（Spec-as-Sourceに性急に飛びついていないか）</li>
