...
describe("Phase A - SiteHeader dropdown rendering", () => {
  it("renders 13 dropdowns (Claude/Google/Codex/Copilot/Code Review/Agent/MCP/Sandbox/IDE/Security/Local LLM/CI/CD/RAG) as .ch-dropdown <li>", () => {
    const { container } = render(<SiteHeader pathname="/" />);
    const dropdowns = container.querySelectorAll("li.ch-dropdown");
    expect(dropdowns.length).toBe(13);
  });
  ...
  it("each dropdown has a .ch-submenu <ul> with at least one child", () => {
    ...
    expect(submenus.length).toBe(13);
    submenus.forEach((ul) => {
